...
Moreover, finding a new block with the same blockID requires currently 2<sup>64</sup> tries in average. In each try, the attacker creates a block which includes creating the payload, signing the block header and computing the blockID. The signing step is computational wise the most expensive one. According to the designers of the signature scheme used in Lisk (Ed25519), a quad-core 2.4 GHz Westmere is able to sign [109,000 messages per second](https://link.springer.com/content/pdf/10.1007%2Fs13389-012-0027-1.pdf). Therefore, one would require 5.3 million years in average to find a new block with this hardware. This makes the attack economically very unattractive, even with more advanced hardware. Furthermore, the attacker needs to achieve that other users synchronise their chain with the faked chain where they copy at least the new block.
