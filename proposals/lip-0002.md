...
We propose to set a maximum block size of 15 KiB (15 * 1024 bytes). This size will maintain the yearly blockchain growth below 50GB, even when blocks are always at full capacity, and the active delegates do not miss any blocks. The summary of the numerical implications are as follows:
...
* The payload of a block must not weigh more than 15 KiB. If a delegate generates a bigger block, it should be deemed invalid.
...
8. Remove the redundant `payloadLength` check from blocks verification for historical data (up to 1048576 KiB), as it is already included in block's signature.
