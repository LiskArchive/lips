...
*   The block header properties in B satisfy `B.heightPrevious>0`, `B.heightPrevious`<`B.height` and `B.height-B.heightPrevious<=303`, but the block at height `heightPrevious` in the current chain is not forged by `D`. This means that the delegate recently forged a block on a different branch before.
