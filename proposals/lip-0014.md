...
We assume that the delegate forging a block adjusts the reward property in the block if and only if one of the conditions above is met. Any block without reduced reward property, but which meets one of the conditions above, is deemed invalid. The condition `B.height-B.maxHeightPreviouslyForged<=303` ensures that we only need to consider the recent 303 blocks for verifying this condition.
