...
In this section, we summarize the key properties following from the analysis in [8, Theorem 4.4]. In short, for a static set of 101 delegates the consensus algorithm has the following properties and adds the desired block finality and robustness to the Lisk consensus algorithm:
...
For a new child block `B`, which is supposed to be added to the current chain containing blocks up to height `B.height-1`, to be valid, the property `B.maxHeightPrevoted` needs to be the largest height of a block in the current chain up to height `B.height-1` that has at least 68 `prevotes` contained in the chain. Note that for this the `prevotes` implied by block `B` are not taken into account. If this number is incorrect, the block header is deemed invalid and discarded.
...
*   `maxHeightPrevoted`: largest height of an ancestor block with at least  68 `prevotes` as described above
...
getHeightNotPrevoted(newBlockheader) {
   ...
   let h=Math.max(newBlockheader.maxHeightPreviouslyForged, newBlockheader.height-302);
...
applyPrevotesPrecommits(newBlockheader) {
   ...
   const minPrevoteHeight = Math.max(newBlockheader.maxHeightPreviouslyForged+1,
                                     delegateMinHeightActive,
...
After updating the `prevoteCounts` and `precommitCounts` data structures as described above, we can easily compute the largest height with `precommits` by at least 68 delegates, denoted by `chainMaxHeightFinalized`, and we consider all blocks at this height and before finalized. This value should be stored persistently and never decreased so that even in the case of node crashes, blocks that are once deemed final are never reverted. Moreover, we also compute the largest height of a block with `prevotes` messages by at least 68 delegates, denoted by `chainMaxHeightPrevoted`, which is needed to verify the next block added to the chain. It is important to note that if `A` is the current block at the tip of the chain, then in general after computing the `prevotes` and `precommits` implied by `A`, the values `chainMaxHeightPrevoted` and `A.maxHeightPrevoted` are different, because `A.maxHeightPrevoted` is computed not taking the `prevotes` implied by `A` into account.
...
*   The block header properties in `B` satisfy `B.maxHeightPreviouslyForged`>=`B.height`. This means that the delegate does not constructively participate in the Lisk-BFT protocol as the block header does not imply any `prevote` or `precommit`.
*   The block header properties in B satisfy `B.maxHeightPreviouslyForged>0`, `B.maxHeightPreviouslyForged`<`B.height` and `B.height-B.maxHeightPreviouslyForged<=303`, but the block at height `maxHeightPreviouslyForged` in the current chain is not forged by `D`. This means that the delegate recently forged a block on a different branch before.

We assume that the delegate forging a block adjusts the reward property in the block if one of the conditions above is met. Any block without reduced reward property, but which meets one of the conditions above, is deemed invalid. The condition `B.height-B.maxHeightPreviouslyForged<=303` ensures that we only need to consider the recent 303 blocks for verifying this condition.
...
The `postBlockAnnouncement` event used to inform peers about a new applied block  (see [LIP 0004 "Introduce robust peer selection and banning mechanism"](https://github.com/LiskHQ/lips/blob/master/proposals/lip-0004.md) ) needs to get extended by the property `maxHeightPrevoted`.
...
*   After reverting one or more blocks, a node needs to update the `prevote` and `precommit` of the blocks and, in particular, needs to have the correct value of `chainMaxHeightPrevoted` to be able to verify new blocks. Assume one or more blocks have been deleted from the current chain and the current tip of the chain is now block `A` at height `A.height`. Then the `prevotes`, `precommits` and `chainMaxHeightPrevoted` are obtained as follows:
...
**2. Update `chainMaxHeightPrevoted`**

Note that the block of largest height with at least 68 `prevotes` that is contained in the current chain with tip A, is either the block at height `A.maxHeightPrevoted` or its height is at least `A.height-302`. The reason is that block `A` only implies `prevotes` for blocks of height at least `A.height-302` so either `chainMaxHeightPrevoted=A.maxHeightPrevoted` or the `prevotes` implied by `A` result in a new block of height at least `A.height-302` obtaining at least 68 `prevotes`.
Thus the value `chainMaxHeightPrevoted` can be updated as outlined in the following function.

```js
updateChainMaxHeightPrevoted(){
   chainMaxHeightPrevoted=A.maxHeightPrevoted;
   for(let h=A.height;h>=A.height-302;h--) {
      if(prevoteCounts[h]>=68) {
         chainMaxHeightPrevoted=h;
         break;
      }
   }
}
...
**3. Step: Revert to last common block**
  ...
  a)  The node requests the last common block `C` from `P`. If `P` does not provide a common block `C` after several tries, then `P` is banned and the block synchronization is restarted from Step 1.
...
Assuming that the migration is scheduled for height `l`, we propose that `chainMaxHeightFinalized` is initialized with the value `l-202`.
