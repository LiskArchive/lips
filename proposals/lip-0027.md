...
**Superseded in [LIP 0064][lip-0064]**: LIP 0064 requires that every property is _required_. Hence, a valid message cannot have missing properties. Therefore, all properties will be contained in the binary message, except for empty arrays.
...
**Superseded in [LIP 0064][lip-0064]**: LIP 0064 requires that every property is _required_. Therefore, valid binary messages cannot have missing values, except for properties of type array where the missing value corresponds to the empty array, and valid messages cannot have missing properties. This implies `encode(decode( binaryMsg )) == binaryMsg` for every valid binary message `binaryMsg` and `decode(encode( msg )) == msg` for every valid message `msg`. Hence, no special care must be taken anymore.
...
**Superseded in [LIP 0064][lip-0064]**: LIP 0064 requires additionally that every schema of type _object_ must use the `required` keyword, and every property of the object must be contained its value.
...
**Fixed in [Lisk SDK PR 8497][sdk-pr-8497]**: Varint encoding allows more than one way to encode an integer by appending zero-bytes to the encoding. For example, the integer 75 can be encoded as `0x4b`, but also as `0xcb00`. This was fixed in the mentioned PR for the Lisk SDK. This means that varint encoded values must always have the shortest form. A binary message violating this rule is considered invalid and decoding will fail for it.
...
**Fixed in [Lisk SDK PR 8497][sdk-pr-8497]**: Some unicode characters have a non unique representation. For instance, the character `ñ` can be represented as a single code point `\u00F1` or as a combination of two `\u006E\u0303` where `\u006E` represents `n` and `\u0303` the tilde character. Hence, also the UTF-8 encoding of the string `"ñ"` is non unique. This ambiguity was fixed in the mentioned PR by requiring that strings must be normalized using the [_Normalization Form Canonical Composition_](https://en.wikipedia.org/wiki/Unicode_equivalence#Normal_forms) transformation prior to encoding. A binary message violating this rule is considered invalid and decoding will fail for it.
...
**Superseded in [LIP 0064][lip-0064]**: Point 3 becomes obsolete as there are no non-required properties anymore.
...
**Superseded in [LIP 0064][lip-0064]**: Following LIP 0064, a valid binary message cannot have missing values except for properties of type array. Therefore, the definitions of default values become obsolete except for the one of arrays.

### Notation for Encoding and Decoding

In other LIPs, we will use the following notation for encoding and decoding objects without introducing the notation and referencing this LIP every time:
...
### Notation for Schema Validation

In other LIPs, we will use the following notation for validating if a JavaScript objects obeys a Lisk JSON schema without introducing the notation and referencing this LIP every time:

- `validateObjectSchema(schema, object)` has no return type, but throws an exception if `object` does not satisfy the Lisk JSON schema `schema`.
...
[sdk-pr-8497]: https://github.com/LiskHQ/lisk-sdk/pull/8497