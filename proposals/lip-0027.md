...
**Fixed in [Lisk SDK PR 8497][sdk-pr-8497]**: Varint encoding allows more than one way to encode an integer by appending zero-bytes to the encoding. For example, the integer 75 can encoded as `0x4b`, but also as `0xcb00`. This was fixed in the mentioned PR for the Lisk SDK. This means that varint encoded values must always have the shortest form. A binary message violating this rule is considered invalid and decoding will fail for it.
