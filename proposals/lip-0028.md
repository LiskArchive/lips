...
This LIP defines a generic serialization for transactions by specifying the appropriate JSON schema. The proposed serialization method is applied in two steps, first to the asset property, where type-specific properties are defined, and then to the transaction object. It will be used for signing but also for storing and transmitting transactions in the Lisk protocol. Further, this shall be the default serialization method for custom transactions created via the Lisk SDK.
...
1. The `trsData.asset` property is serialized to bytes according to the corresponding  `asset` property schema specified by the combination of `trsData.moduleID` and `trs.assetID` properties (see [below](#transactionSchema-schema)).
...
2. The bytes value of the `trsData.asset` property is deserialized according to the corresponding `asset` property schema specified by the combination of `trsData.moduleID` and `trs.assetID` properties.
...
1. `moduleID`: An integer identifying the module the transaction belongs in. Together with `assetID` specifies the JSON schema to be used to serialize-deserialize the `asset` property of the transaction.
2. `assetID`: An integer identifying the specific asset for the transaction in the module. Together with `moduleID` specifies the JSON schema to be used to serialize-deserialize the `asset` property of the transaction.
...
There is a unique `asset` property schema per transaction type, implied by the unique combination of the values in `moduleID` and `assetID` properties. All the properties defined in any `asset` schema are required.
