...
* Migrate every account, except the Genesis Account and any account without incoming transaction, as follows.
  ...
  * If the string representation of an account in Lisk Core 2.0 is non-standard (It is not `OL`, but starts with `0`. Or the decimal number is larger than `2^64-1`), the account corresponds to one unique 8-byte address `legacyAddress`, which are the 8 bytes signed in the transaction creating the account. Note that the account cannot have any outgoing transaction as the string representation is non-standard. The non-standard account `acc_non_standard` is migrated as follows:
    * If there is an account `acc_standard` with standard string representation corresponding to the same `legacyAddress`, then the balance of `acc_non_standard` is added to the balance of the migrated account `acc_standard`.
