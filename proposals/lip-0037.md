...
Every binary message must be tagged before being signed. This is done as specified by the function `tagMessage` in the pseudo code below: A specific ASCII-encoded tag and a chain identifier as specified in [LIP 0009](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0009.md#specification) are prepended to the message.
...
Chain identifiers are 4-byte values. The first byte is set to `CHAIN_ID_PREFIX_MAINNET` for chains running in the mainnet network and to `CHAIN_ID_PREFIX_TESTNET` for chains running in the testnet network. The other 3 bytes are unique to that chain.

The following table defines the chain-identifiers prefixes currently specified.

| Name                            | Type  | Value| Description                                                 |
|---------------------------------|-------|------|-------------------------------------------------------------|
| `CHAIN_ID_PREFIX_MAINNET`       | bytes | 0x00 | Chain-identifiers prefix for chains running in the mainnet. |
| `CHAIN_ID_PREFIX_TESTNET`       | bytes | 0x01 | Chain-identifiers prefix for chains running in the testnet. |

### Signing and Verifying with Ed25519

Let [`Sign`](https://tools.ietf.org/html/rfc8032#section-5.1.6) and [`Verify`](https://tools.ietf.org/html/rfc8032#section-5.1.7) be the signing and verifying functions of Ed25519 as specified in [RFC 8032](https://tools.ietf.org/html/rfc8032). Then the signature for a binary message `m` and a secret key `sk` is generated by `signMessage(sk, tag, chainID, m)` as defined below. `tag` must be the correct message tag for `m` and `chainID` the correct chain identifier for the chain. The resulting signature `sig` in combination with the message `m` and the matching public key `pk` is verified by `verifyMessageSig(pk, tag, chainID, m, sig)`.

```python
def signMessage(sk: bytes, tag: bytes, chainID: bytes, m: bytes) -> bytes:
    taggedMessage = tagMessage(tag, chainID, m)
    return Sign(sk, taggedMessage)
```

```python
def verifyMessageSig(pk: bytes, tag: bytes, chainID: bytes, m: bytes, sig: bytes) -> bool:
