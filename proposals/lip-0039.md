...
def verify(queryKeys: list[bytes], proof: Proof, root: bytes) -> bool:
    ...
    for q in proof.queries:
        ...
        # If the query is a duplicate, check that the two queries are consistent.
            ...
            duplicateQuery = _filter[q.binary_path]
            # Check that the value and bitmap are the same.
            if q.value != duplicateQuery.value or q.bitmap != duplicateQuery.bitmap:
                return False
            # If the value is empty, both queries point to an empty node.
            # The binary paths must match (the keys can be different).
            if q.value == EMPTY_VALUE:
                if q.binary_path != duplicateQuery.binary_path:
                    return False
            # Otherwise, the keys must be the same.
            else:
                if q.key != duplicateQuery.key:
                    return False
