...
We define a function to verify inclusion proofs, `verify_inclusion_proof`, and a function to verify non-inclusion proofs, `verify_non_inclusion_proof`. Both function just call the `verify_proof` function with a different `is_inclusion` flag.

```python
def verify_inclusion_proof(query_keys: list[bytes], proof: Proof, root: bytes) -> bool:
    return verify_proof(query_keys, proof, root, True)
```

```python
def verify_non_inclusion_proof(query_keys: list[bytes], proof: Proof, root: bytes) -> bool:
    return verify_proof(query_keys, proof, root, False)
```

```python
def verify_proof(query_keys: list[bytes], proof: Proof, root: bytes, is_inclusion: bool) -> bool:
    ...
    for key, query in zip(query_keys, proof.queries):
        ...
        if is_inclusion_proof(key, query) != is_inclusion:
            return False
