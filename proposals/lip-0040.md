...
Given `B` substores in a module store and `N` entries in the substore, the module subtree will contain on average `Log(B) + Log(N)` layers. The expected number of operations needed to validate a witness from a leaf to the state root is `8 * MODULE_ID_LENGTH` (for the module ID), plus `8 * STORE_PREFIX_ID_LENGTH` (for the store prefix), plus `Log(N)` for the module subtree. Empty hashes are not included in the witness, and the resulting witness size is therefore `Log(M) + Log(B) + Log(N)`, where `M` indicates the number of modules registered in the chain.

### Module ID

The module ID is calculated automatically from the first `MODULE_ID_LENGTH` bytes of the hash of the module name, where the first bit of the module ID is set to 0.
On the other hand, the Interoperability module ID is set to a constant, with the first bit set to 1.
...
When a chain is started and modules are registered, ID collisions between different modules are checked and must resolved (by changing module names).
...
|--------------------------|--------|-------|------------------------------------|
| `MODULE_ID_LENGTH`       | uint32 | 4     | Length in bytes of a module ID.    |
| `STORE_PREFIX_ID_LENGTH` | uint32 | 2     | Length in bytes of a store prefix. |
...
```python
leaf.key = moduleID + storePrefix + SHA256(storeKey),
...
The module ID is calculated from the module names as follows.

```python
def module_name_to_id(module_name: str) -> bytes:
    module_id = SHA256(module_name.encode())[:MODULE_ID_LENGTH]
