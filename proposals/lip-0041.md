...
| `NONCE_VERIFY_STATUS_FAIL` | int32 | -1 | Return value of nonce verification when transaction is nonce is invalid and will never become valid (smaller than account nonce). |
| `NONCE_VERIFY_STATUS_PENDING` | int32 | 0 | Return value of nonce verification when transaction is nonce currently invalid but could become valid in the future (larger than account nonce). |
| `NONCE_VERIFY_STATUS_OK` | int32 | 1 | Return value of nonce verification when transaction is nonce is valid (equal to account nonce). |
...
def isValidNonce(trs: Transaction) -> bool:
    ...
    return nonceStatus == NONCE_VERIFY_STATUS_OK
```
