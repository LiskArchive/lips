...
|`TYPE_ID_SIGNATURE_GROUP_REGISTERED` | bytes   | 0x0000      | Type ID of the signature group registered event. |
...
This event has `typeID = TYPE_ID_SIGNATURE_GROUP_REGISTERED`.
This event is emitted when a signature group is registered.
...
```java
signatureGroupRegisteredEventDataSchema = {
    "type": "object",
...
The schema `multisigRegAsset` contains 4 properties:
...
4. `signatures`: An array of signatures, one for each key in `mandatoryKeys` and `optionalKeys` in lexicographical order.

This schema supersedes the one defined in [LIP 0028](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0028.md#multisignature-registration).
...
def verify(trs: Transaction) -> None:
  # Union of all keys sorted lexicographically
  keys = sorted(trs.params.mandatoryKeys + trs.params.optionalKeys)
```
