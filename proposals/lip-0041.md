...
def execute(trs: Transaction) -> None:
    ...
    for signature, key in zip(trs.params.signatures, allKeys):
        if not verifyMessageSig(key, MESSAGE_TAG_MULTISIG_REG, chainID, message, signature):
            emitPersistentEvent(
                module = MODULE_NAME_AUTH,
...
We define the following auxiliary function to verify the validity of a signature. These specifications supersede those given in [LIP 0017](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0017.md#signatures-replaces-signature). The function `verifyMessageSig` is defined in [LIP 0062](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0062.md#specification) and `chainID` is the chain identifier of the chain.
...
def verifySignatures(trs: Transaction) -> None:
    ...
    if isMultisignatureAccount(senderAddress):
        ...
        for idx in range(numberOfMandatoryKeys):
            ...
            if not verifyMessageSig(key, MESSAGE_TAG_TRANSACTION, chainID, unsignedTrsBytes, trs.signatures[idx]):
                raise Exception(f'Invalid signature for public key {key.hex()}.')
    ...
    # Single signature account case.  
        ...
        if not verifyMessageSig(trs.senderPublicKey, MESSAGE_TAG_TRANSACTION, chainID, unsignedTrsBytes, trs.signatures[0]):
            raise Exception(f'Invalid signature for public key {key.hex()}.')
```
