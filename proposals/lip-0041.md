...
This function is used to retrieve information about the auth account corresponding to the input `address`. It returns `authAccount(address)`. If there is no entry corresponding to `address`, it returns an account object with all properties set to their default.
