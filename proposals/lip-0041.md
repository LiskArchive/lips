...
* `address: Address`: Address identifying the auth account.
...
This function is used to retrieve information about an auth account. It returns `authAccount(address: Address)`. If there is no auth account corresponding to `address`, it returns an empty object.
