...
def verify(trs: Transaction) -> None:
    ...
    let address be the address corresponding to trs.senderPublicKey
    if len(authAccount(address).mandatoryKeys) > 0 or len(authAccount(address).mandatoryKeys) > 0:
...
def execute(trs: Transaction) -> None:
    let address be the address corresponding to trs.senderPublicKey
    authAccount(address).numberOfSignatures = trs.params.numberOfSignatures
    authAccount(address).mandatoryKeys = trs.params.mandatoryKeys
    authAccount(address).optionalKeys = trs.params.optionalKeys
