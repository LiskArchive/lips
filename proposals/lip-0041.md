...
def verify(trs: Transaction) -> None:
    ...
    if len(allKeys) < 1 or len(allKeys) > 64:
...
def execute(trs: Transaction) -> None:
    ...
    for signature, key in zip(trs.params.signatures, allKeys):
        if verifyMessageSig(key, MESSAGE_TAG_MULTISIG_REG, networkIdentifier, message, signature) == False:
            emitPersistentEvent(
                moduleID = MODULE_ID_AUTH,
...
```python
def isMultisignatureAccount(address: bytes) -> bool:
...
This function returns `true` if the transaction object contains a valid nonce, `false` otherwise. This is done using the `isValidNonce(trs)` function defined [below](#transaction-verification).
...
def transactionVerification(trs: Transaction) -> None:
    ...
    if not isValidSignature(trs):
        raise Exception(f'Invalid signature for public key {trs.senderPublicKey.hex()}.')
```
...
def isValidSignature(trs: Transaction) -> bool:
    ...
    if isMultisignatureAccount(senderAddress):
        ...
        nonEmptySignatures = [sig if len(sig) > 0 for sig in trs.signatures]
    ...
    # Single signature account case  
        ...
        return verifyMessageSig(trs.senderPublicKey, MESSAGE_TAG_TRANSACTION, networkIdentifier, unsignedTrsBytes, trs.signatures[0])
```
