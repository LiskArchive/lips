...
The following function initializes block generator account for `TOKEN_ID_REWARD`, if it has not been previously initialized. The `TOKEN_ID_REWARD` is the token ID of the token used for the reward system of the blockchain. 

We refer to the `userSubstoreExists` and `initializeUserAccountForBlockGenerator` functions exposed by the [Token module][token-module].

```python
def beforeTransactionsExecute(b: Block) -> None:
    (blockReward, reduction) = getBlockReward(b.header)
    if blockReward > 0 and not Token.userSubstoreExists(b.header.generatorAddress, TOKEN_ID_REWARD):
        Token.initializeUserAccountForBlockGenerator(b.header.generatorAddress, TOKEN_ID_REWARD)
```
