...
| `EVENT_TYPE_REWARD`           | bytes    |       0x0001                              |
...
Event is emitted when the reward is minted, it carries information about reward reduction. This event has `typeID = EVENT_TYPE_REWARD`.
...
```python
getBlockReward(blockHeader: BlockHeader) -> tuple[uint64, uint32]:
...
eventData = {
    "amount": blockReward,
    "reduction": reduction
} serialized with rewardMintedDataSchema
emitEvent(MODULE_ID_REWARD, EVENT_TYPE_REWARD, eventData, [b.header.generatorAddress])
```
