...
```python
getBlockReward(blockHeader: BlockHeader) -> uint64:
...
Here `isValidSeedReveal` is the function exposed by the [random module][research:random-module] and `impliesMaximalPrevotes` is the function exposed by the [BFT module][research:bft-module#specs].
