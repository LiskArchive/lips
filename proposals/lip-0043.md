...
The identifier of the chain. The first byte should be set to the correct value [depending on the network on which the chain is running][lip-0037#chainID].
...
The chain ID is a 4-byte constant set in the chain configuration. Chain IDs serve two purposes: They are [prepended to the input of the signing function of every transaction, block, or message of the chain][lip-0037] to avoid transaction replays between different chains in the ecosystem; 
...
| `MAINCHAIN_CHAIN_ID`                | bytes  | TBD                  | Chain identifier of the Lisk mainchain.          |
...
| `SUBSTORE_PREFIX_CHAIN_VALIDATORS`       | bytes  | 0xb000               | Substore prefix of the chain validators substore.     |
...
| `MAX_CHAIN_NAME_LENGTH`                     | uint32 | 40                   | Maximum allowed name for a sidechain.              |
...
def execute(trs: Transaction) -> None:
    ...
    if verifyWeightedAggSig(blsKeys, MESSAGE_TAG_CHAIN_REG, trs.params.ownChainID, trs.params.aggregationBits, trs.params.signature, bftWeights, certificateThreshold, message) == VALID:
        emitPersistentEvent(
            module = MODULE_NAME_INTEROPERABILITY,
            name = EVENT_NAME_CHAIN_REGISTRATION_FAILED,
            data = trs.params,
