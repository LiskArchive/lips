...
* The properties `trs.params.aggregationBits` and `trs.params.signature` are validated as follows:
  * The function `verifyWeightedAggSig(keyList, tag, netID, aggregationBits, signature, weights, certificateThreshold, message)` specified in the [LIP 0038][BLSkeys], must return `VALID` where:
    * Let `validators` be the array of objects as 4th output of the `getBFTParameters` function exposed by the [BFT module][BFTModule], i, e., `( , , , validators, ) = getBFTParameters(currentHeight)`, where `currentHeight` is the current height of the chain as defined for the BFT module. Then:
       *  The `keyList` argument is an array containing `getValidatorAccount(address).blsKey`, for every `address` element in `validators`. The `getValidatorAccount` function is exposed by the [Validators module][validatorsModule].
       *  The `weights` argument is an array containing every `bftWeight` element in `validators`.
    ...
    * The `certificateThreshold` argument is equal to the third output of the `getBFTParameters` function exposed by the [BFT module][BFTModule], i, e., `( , , certificateThreshold, , ) = getBFTParameters(currentHeight)`.
