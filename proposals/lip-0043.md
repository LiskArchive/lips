...
For a general picture of the Lisk interoperability architecture, please refer to the [Introduce Interoperability module LIP][research:interop-module].
...
Once the sidechain registration command is processed, the sidechain account status is set to _registered_. In this state, the cross-chain channel is still not active, so the users on the mainchain or other chains cannot send [cross-chain messages (CCMs)][research:ccm] to this sidechain yet. Moreover, the [liveness requirement][research:interop-module#liveness-condition] to maintain the channel is not enforced, this means that there is no specific time requirement for a sidechain to be activated on the mainchain, it can stay in the registered status for any period of time.  When a valid cross-chain update command from this sidechain containing a valid [registration CCM][research:ccm] is processed, the sidechain status is changed to _active,_ making it active in the ecosystem. Now it is possible to send CCMs to the sidechain and the liveness condition is enforced.

#### Sidechain Structures on Mainchain

As mentioned above, when a new sidechain is registered on the mainchain via a registration command, a sidechain account is created in the Lisk mainchain state.  Specifically, this implies that the corresponding key-value entries for the sidechain are added to the interoperability module store (see Figure 1). The values of these entries are initialized as specified in the [Introduce Interoperability module LIP][research:interop-module], and concretely the `name`, `networkID`, and `initValidators` properties are computed from the sidechain registration command.
...
This property defines the set of eligible [BLS public keys][lip-0038] with their respective [BFT weights][research:add-weights] required to sign the first certificate from the sidechain.
...
Calling a function `fct` from a module `module` is represented by `module.fct(required inputs)`. In this LIP, functions from the [Validators module][research:validators-module] and the [BFT module][research:bft-module] are used.
...
* An entry in the chain data substore as specified in the [Introduce Interoperability module LIP][research:interop-module#module-store] is created as:
  * `storePrefix`: `STORE_PREFIX_CHAIN_DATA`
  * `storeKey`: `uint32be(chainID)`.
  * `storeValue`: `sidechainAccount` where `sidechainAccount` is the chain account object, serialized according to the `chainAccountSchema` schema as defined in the [Introduce Interoperability module LIP][research:interop-module], and initialized as:
    ...
    * The following [registration CCM][research:ccm#registration-message] is added to `sidechainAccount.outbox` by calling the `addToOutbox(chainID, registrationCCM)` logic as specified in the [Introduce Interoperability module LIP][research:interop-module]:
    ...
    ```
  * The rest of the properties, as specified in the [Introduce Interoperability module LIP][research:interop-module], are initialized to their default values.
* An entry in the outbox root substore as specified in the [Introduce Interoperability module LIP][research:interop-module] is created as:
...
* An entry in the registered names substore as specified in the [Introduce Interoperability module LIP][research:interop-module] is created as:
...
* An entry in the registered network IDs substore as specified in the [Introduce Interoperability module LIP][research:interop-module] is created as:
  * `storePrefix`: `STORE_PREFIX_NETWORK_ID`.
  * `storeKey`: `sidechainAccount.networkID `serialized as bytes.
  * `storeValue`: `chainID` serialized according to `chainIDSchema` schema as defined in the [Introduce Interoperability module LIP][research:interop-module].
...
When a mainchain registration command is applied, new entries of the Interoperability module are created in the sidechain state for the interoperability account, outbox root and own chain stores as specified in the [Introduce Interoperability module LIP][research:interop-module]. In particular, let `trs` be a transaction with module ID `MODULE_ID_INTEROPERABILITY` and command ID `COMMAND_ID_MAINCHAIN_REG` to be executed. Then:

* An entry in the chain data substore as specified in the [Introduce Interoperability module LIP][research:interop-module] is created as:
  * `storePrefix`: `STORE_PREFIX_CHAIN_DATA`.
  * `storeKey`: `uint32be(MAINCHAIN_ID)`.
  * `storeValue`: `mainchainAccount` where `mainchainAccount` is the chain account object, serialized according to the `chainAccountSchema` schema as defined in the [Introduce Interoperability module LIP][research:interop-module], and initialized as:
    ...
    * The following [registration CCM][research:ccm#registration-message] is added to `mainchainAccount.outbox` by calling the `addToOutbox(MAINCHAIN_ID, registrationCCM)` logic as specified in the [Introduce Interoperability module LIP][research:interop-module]:
...
* An entry in the own chain account data substore as specified in the [Introduce Interoperability module LIP][research:interop-module] is created as:
  * `storePrefix` : `STORE_PREFIX_CHAIN_DATA`
  * `storeKey`: `uint32be(0)`.
...
[research:ccm#registration-message]: https://research.lisk.com/t/introduce-cross-chain-messages/299#registration-message-18
