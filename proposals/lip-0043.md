...
That is the entry with store prefix equal to `STORE_PREFIX_NETWORK_ID` and store key equal to `netID` does not exist in the store.
...
* `trs.params.extraCommandFee == REGISTRATION_FEE`.
...
* `token.burn(senderAddress, TOKEN_ID_LSK_MAINCHAIN, REGISTRATION_FEE)` is called.
