...
| `SUBSTORE_PREFIX_VALIDATORS_PARAMS` | bytes | 0x4000 | Substore prefix of the validator params substore. |
...
This function allows to set the precommit threshold, certificate threshold, validators and associated BFT weights to be used from the next height onward. The information is not validated and simply stored in the Validators module. At the end of the function, the information is further forwarded from the application domain to the consensus domain where all validations are performed, see [LIP 0058][lip-0058] for details.
