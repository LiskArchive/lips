...
`MAINCHAIN_ID`                 | uint32  | Value is specified in [LIP 0045][lip-0045#constants] | The chain ID of the mainchain as part of the interoperability module.
...
```python
registerValidatorWithoutBLSKey(validatorAddress, generatorKey):
    # If the chain is not the mainchain, let the registration fail
    if interoperabilityModule.getOwnChainAccount.ID != MAINCHAIN_ID:
        return false
