...
This event has `typeID = TYPE_ID_GENERATOR_KEY_REGISTRATION`. This event is emitted when a generator key is registered. The event data contains the generator key `generatorKey` and the result of the registration `result`.
...
This event has `typeID = TYPE_ID_BLS_KEY_REGISTRATION`. This event is emitted when a BLS key is registered. The event data contains the BLS key `blsKey` and the result of the registration `result`.
