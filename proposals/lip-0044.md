...
`INVALID_BLS_KEY`              | bytes   | `BLS_KEY_LENGTH` bytes all set to 0x00                             | An invalid BLS key, used as a placeholder before a valid BLS key is registered.
...
* `generatorKey: ED25519_PUBLIC_KEY_LENGTH`: The public key whose corresponding private key is used to sign blocks generated by the validator.
* `blsKey: BLSKey`: The validator BLS key is the public BLS key whose corresponding private key is used to sign certificates.
...
This function returns the address that registered the input BLS key `blsKey: BLSKey`.
It checks if an entry with store key equal to `blsKey` exists in the registered BLS keys substore. 
If this is the case, it returns the stored `address: Address`, else it throws an error.
...
This function updates the value of the generator list to the input `generatorAddresses: list[Address]`.
...
Otherwise, it updates the value of `generatorList.addresses` to `generatorAddresses`.
