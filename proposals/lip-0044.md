...
* `success`: A boolean, indicating success or failure of the execution.

##### Execution

This function checks that there is no account already registered for the input `validatorAddress`. Then, it creates a new validator account in the validators data substore, where `blsKey` is set to `INVALID_BLS_KEY`. If this function is called on a chain that is not the mainchain, then this registration fails.
...
def registerValidatorWithoutBLSKey(validatorAddress: Address, generatorKey: PublicKeyEd25519) -> None:
    ...
    if the chain is not the mainchain:
        raise('A validator can be registered without a BLS key on on the mainchain.')
