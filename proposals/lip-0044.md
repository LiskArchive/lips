...
`KEY_REG_STATUS_SUCCESS` | uint32   | 0      | Success status code of key registration events. |
`KEY_REG_STATUS_NO_VALIDATOR` | uint32   | 1      | Failure status code of key registration events: address not registered as validator. |
`KEY_REG_STATUS_ALREADY_VALIDATOR` | uint32   | 2      | Failure status code of key registration events: address already registered as validator. |
`KEY_REG_STATUS_NO_MAINCHAIN` | uint32   | 3      | Failure status code of key registration events: chain is not the mainchain. |
`KEY_REG_STATUS_DUPLICATE_BLS_KEY` | uint32   | 4      | Failure status code of key registration events: BLS key already registered in the chain. |
`KEY_REG_STATUS_NO_UPDATE_BLS_KEY` | uint32   | 5      | Failure status code of key registration events: BLS keys cannot be updated. |
`KEY_REG_STATUS_INVALID_POP` | uint32   | 6      | Failure status code of key registration events: invalid proof of possession. |

### Type Definition

| Name               | Type    | Validation            | Description                     |
|--------------------|---------|-----------------------|---------------------------------|
| `Address` | bytes   | Must be of length 20. | Address of an account.   |
| `PublicKeyEd25519` | bytes | Must be of length 32. | Used for Ed25519 public keys. |
| `BLSKey` | bytes | Must be of length 48. | Used for BLS keys. |
| `ProofOfPossession` | bytes | Must be of length 96. | [The proof of possession associated with a BLS key][lip-0038#public-key-registration]. |
        ...
        emitPersistentEvent(
            ...
            data = encode(
                ...
                object = {
                    ...
                    "status": KEY_REG_STATUS_NO_VALIDATOR
                }
            ),
    ...
    if there exists an entry in the registered BLS keys substore with storeKey == blsKey:
        emitPersistentEvent(
            ...
            data = encode(
                ...
                object = {
                    "blsKey": blsKey,
                    "status": KEY_REG_STATUS_DUPLICATE_BLS_KEY
                }
            ),
            topics=[validatorAddress]
        )
    ...
    if PopVerify(blsKey, proofOfPossession) != VALID:
        emitPersistentEvent(
            ...
            data = encode(
                ...
                object = {
                    "blsKey": blsKey,
                    "status": KEY_REG_STATUS_INVALID_POP
                }
            ),
            topics=[validatorAddress]
        )
    ...
    if the chain is not the mainchain:
        emitPersistentEvent(
            ...
            data = encode(
                ...
                object = {
                    "generatorKey": generatorKey,
                    "status": KEY_REG_STATUS_NO_MAINCHAIN
                }
            ),
            topics=[validatorAddress]
        )
    ...
    if there exists an entry in the validators data substore with storeKey == validatorAddress:
        emitPersistentEvent(
            ...
            data = encode(
                ...
                object = {
                    "generatorKey": generatorKey,
                    "status": KEY_REG_STATUS_ALREADY_VALIDATOR
                }
            ),
            topics=[validatorAddress]
        )
    ...
    if validatorAccount(validatorAddress).blsKey != INVALID_BLS_KEY:
        emitPersistentEvent(
            ...
            data = encode(
                ...
                object = {
                    "blsKey": blsKey,
                    "status": KEY_REG_STATUS_NO_UPDATE_BLS_KEY
                }
            ),
            topics=[validatorAddress]
        )
