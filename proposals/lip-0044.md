...
* Notation: For the rest of this proposal let `validatorKeys(address)` be an entry in the validators keys substore identified by the store key `address`.
...
* Notation: For the rest of this proposal let `validatorParamsStore` be the entry in the validator params substore.
...
This substore stores the current validator parameters:
...
* `validators`: This property stores an array of objects, each corresponding to a validator with the following properties:
  ...
  * `generatorKey`: The validator public key whose corresponding private key is used to sign blocks generated by the validator.
  * `blsKey`: The validator BLS whose corresponding private key is used to sign certificates.
...
def getGeneratorsBetweenTimestamps(startTimestamp: uint32, endTimestamp: uint32) -> dict[Address, uint32]:
    ...
    if startTimestamp < genesisDataStore.timestamp:
        raise Exception('Start timestamp cannot be smaller than genesis block timestamp.')

    result = {}

    startSlotNumber = floor((startTimestamp - genesisDataStore.timestamp) / BLOCK_TIME) + 1
    endSlotNumber = floor((endTimestamp - genesisDataStore.timestamp) / BLOCK_TIME) - 1
...
The order in this array determines the block generation order
an array of `Address` values, corresponding to the list of eligible generators for the current round. Note that a position in the generator list does NOT reflect the position in a round. The assignment of a generator for a block slot is defined by the function []().
...
* Create the entry in the genesis data substore, setting `genesisDataStore.timestamp` to `b.header.timestamp`.
