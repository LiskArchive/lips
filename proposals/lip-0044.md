...
`KEY_REG_RESULT_NO_UPDATE_BLS_KEY` | uint32   | 4      | Failure result code of key registration events: BLS keys cannot be updated. |
`KEY_REG_RESULT_INVALID_POP` | uint32   | 5      | Failure result code of key registration events: invalid proof of possession. |
...
This function returns the address that registered the input BLS key `blsKey: PublicKeyBLS`.
