...
**Note**: For blockchains build with the SDK v6, this function should be not be called except from the [Legacy module](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0050.md#register-keys) used on the Lisk Mainchain. Otherwise, the security of interoperability may brake.
...
def getGeneratorsBetweenTimestamps(startTimestamp: uint32, endTimestamp: uint32) -> dict[Address, uint32]:
    ...
    startSlotNumber = floor(startTimestamp / BLOCK_TIME) + 1
    endSlotNumber = floor(endTimestamp / BLOCK_TIME) - 1
