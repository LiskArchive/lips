...
| `ValidatorKeys` | object | Must follow the `validatorKeysSchema` schema. | An object representing a validator account. |
...
* `validatorList`: The validators and their associated BFT weight to be used from the next height onward. The value must be an array of objects with an `address` property containing the 20-byte address of the validator and a `bftWeight` property containing the BFT weight of the validator as 64-bit unsigned integer. Note that this function fails if the number of provided validators is more than `LSK_BFT_BATCH_SIZE`.
...
```python
setBFTParameters(precommitThreshold: uint64, certificateThreshold: uint64, validatorList: list[object]):
    ...
    params.prevoteThreshold = floor(2/3*aggregateBFTWeight)+1
    ...
    for validator in validatorList:
        ...
        newValidator.blsKey =  validatorKeys.generatorKey
        newValidators.append(newValidator)
    params.validators = newValidators

    # compute the validators hash
    sortedValidators = newValidators.copy()    
    sort elements in sortedValidators lexicographically by blsKey property
    params.validatorsHash = computeValidatorsHashInternal(sortedValidators, certificateThreshold)
