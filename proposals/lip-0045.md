...
| `EVENT_NAME_CCM_SENT`                         | string | "ccmSent"            | Name of the cross-chain message sent event.                      |
...
This event has `name = EVENT_NAME_CCM_SENT`. This event is emitted when a CCM is sent.
The `result` property is set to `CCM_SENT`.
The `code` property of the data schema can take any of the values in the "Cross-chain Message Codes" section of the constants table.
...
This event uses the `ccmProcessedDataSchema` schema given above.
