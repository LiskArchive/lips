...
| `MERKLE_ROOT_LENGTH`                 | uint32  | 32      | Length in bytes of type `MerkleRoot`. |
...
```python
def sendInternal(feeAddress: bytes, moduleID: ModuleID, crossChainCommandID: CrossChainCommandID, receivingChainID: ChainID, fee: uint64, status: uint32, params: bytes) -> None:
    # Processing on the mainchain
    if ownChainAccount.ID == MAINCHAIN_ID:
...
```python
def createTerminatedOutboxAccount(chainID: ChainID, outboxRoot: MerkleRoot, outboxSize: uint64, partnerChainInboxSize: uint64) -> None:
    terminatedOutbox = {
...
```python
def send(feeAddress: Address, moduleID: ModuleID, crossChainCommandID: CrossChainCommandID, receivingChainID: ChainID, fee: uint64, status: uint32, params: bytes) -> None:
    return sendInternal(
        feeAddress,
        moduleID,
        crossChainCommandID,
