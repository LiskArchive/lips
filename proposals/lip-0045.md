...
| **Cross-chain Message Status and Errors**     |        |                      |                                                                  |
| `CCM_OK`                                      | uint32 | 0                    | Status code: ok. New CCMs have this initial status.              |
| `CCM_FORWARDED`                               | uint32 | 1                    | Status code for CCM processed event: forwarded.                  |
| `CCM_BOUNCED`                                 | uint32 | 2                    | Status code for CCM processed event: bounced.                    |
| `CCM_DISCARDED`                               | uint32 | 3                    | Status code for CCM processed event: discarded.                  |
| `CCM_APPLIED`                                 | uint32 | 4                    | Status code for CCM processed event: invalid.                    |
| `ERROR_CODE_CHANNEL_UNAVAILABLE`              | uint32 | 5                    | Error code: channel unavailable.                                 |
| `ERROR_CODE_MODULE_NOT_SUPPORTED`             | uint32 | 6                    | Error code: module not supported.                                |
| `ERROR_CODE_CROSS_CHAIN_COMMAND_NOT_SUPPORTED`| uint32 | 7                    | Error code: cross-chain command not supported.                   |
| `ERROR_CODE_FAILED_CCM`                       | uint32 | 8                    | Error code: failed.                                              |
| `ERROR_CODE_INVALID_CCM`                      | uint32 | 9                    | Error code: invalid CCM.                                         |
| `ERROR_CODE_INVALID_FORMAT`                   | uint32 | 10                   | Error code: invalid format.                                      |
| `ERROR_CODE_MESSAGE_FEE_ERROR`                | uint32 | 11                   | Error code: message fee error.                                   |
| **Message Tags**                              |        |                      |                                                                  |
...
In this case, the fee property is set to 0 and the message is returned only if the fee was larger than a minimum return fee and the status of the CCM is `CCM_OK`.
`bounce` is called if an error occurs in any other stage.
In this case, the fee is not changed and the message is returned if the status of the CCM is `CCM_OK`.
...
def bounce(ccm: CCM, errorCode: uint32) -> None:
    ...
    emitEvent(
        ...
        data = {"ccmID": ccmID, "result": errorCode},
        topics = [ccm.sendingChainID, ccm.receivingChainID]
    )

    if ccm.status == CCM_OK:
        ...
        emitEvent(
            module = MODULE_NAME_INTEROPERABILITY,
            name = EVENT_NAME_CCM_PROCESSED,
            data = {"ccmID": newCcmID, "result": ERROR_CODE_BOUNCED},
            topics = [ccm.sendingChainID, ccm.receivingChainID]
        )
    else:
        emitEvent(
...
```
```python
def bounceWithoutFee(ccm: CCM, errorCode: uint32) -> None:
    ccmID = sha256(encode(crossChainMessageSchema, ccm))
    emitEvent(
        module = MODULE_NAME_INTEROPERABILITY,
        name = EVENT_NAME_CCM_PROCESSED,
    ...
    if ccm.status == CCM_OK and ccm.fee >= MIN_RETURN_FEE * len(encode(crossChainMessageSchema, ccm)):
        ccm.status = errorCode
        ccm.sendingChainID, ccm.receivingChainID = ccm.receivingChainID, ccm.sendingChainID
...
The `result` property of the data schema can take one of the following values:

1. `CCM_OK`: The CCM was sent to the receiving chain succesfully.
2. `ERROR_CODE_CHANNEL_UNAVAILABLE`: The channel to the receiving chain is not available.
3. `ERROR_CODE_INVALID_FORMAT`: The CCM is not well-formed or the data property is too large.
4. `ERROR_CODE_MESSAGE_FEE_ERROR`: The message fee could not be paid.
...
The `result` property of the data schema can take any of the values in the "Cross-chain Message Status and Errors" section of the constants table.
...
Notice that the CCM ID is automatically included as a default topic.
...
# Create cross-chain message
    ...
    try:
        ...
        emitPersistentEvent(
            ...
            data = {"ccmID": ccmID, "result": ERROR_CODE_INVALID_FORMAT},
    ...
    if ownChainAccount.chainID == CHAIN_ID_MAINCHAIN:
        ...
        if chainAccount(receivingChainID) does not exists:
            emitPersistentEvent(
                module = MODULE_NAME_INTEROPERABILITY,
                name = EVENT_NAME_CCM_SENT,
                data = {"ccmID": ccmID, "result": ERROR_CODE_CHANNEL_UNAVAILABLE},
    ...
    if fee > 0:
        try:
            ...
            emitPersistentEvent(
                module = MODULE_NAME_INTEROPERABILITY,
                name = EVENT_NAME_CCM_SENT, 
                data = {"ccmID": ccmID, "result": ERROR_CODE_MESSAGE_FEE_ERROR},
                topics = [ccm.sendingChainID, ccm.receivingChainID, ccmID]
            )
            raise e
    ...
    emitEvent(
        module = MODULE_NAME_INTEROPERABILITY,
        name = EVENT_NAME_CCM_SENT,
        data = {"ccmID": ccmID, "result": CCM_OK},
        topics = [ccm.sendingChainID, ccm.receivingChainID, ccmID]
    )
```
