...
| `CCM_PROCESSED_RESULT_APPLIED`                | uint32 | 0                    | Value of `result` property of CCM processed event if CCM is applied.   |
| `CCM_PROCESSED_RESULT_FORWARDED`              | uint32 | 1                    | Value of `result` property of CCM processed event if CCM is forwarded. |
| `CCM_PROCESSED_RESULT_BOUNCED`                | uint32 | 2                    | Value of `result` property of CCM processed event if CCM is bounced.   |
| `CCM_PROCESSED_RESULT_DISCARDED`              | uint32 | 3                    | Value of `result` property of CCM processed event if CCM is discarded. |
| **Chain Status**                              |        |                      |                                                                  |
...
| `STATUS_CODE_OK`                              | uint32 | 0                    | Value of `status` property of a new CCM which is not a response due do an error.            |
| `STATUS_CODE_CHANNEL_UNAVAILABLE`             | uint32 | 1                    | Value of `status` property of returned CCM due to error: channel unavailable.               |
| `STATUS_CODE_MODULE_NOT_SUPPORTED`            | uint32 | 2                    | Value of `status` property of returned CCM due to error: module not supported.              |
| `STATUS_CODE_CROSS_CHAIN_COMMAND_NOT_SUPPORTED`| uint32| 3                    | Value of `status` property of returned CCM due to error: cross-chain command not supported. |
| `STATUS_CODE_FAILED_CCM`                      | uint32 | 4                    | Value of `status` property of returned CCM due to error: failed ccm execution.              |
| `STATUS_CODE_INVALID_CCM`                     | uint32 | 5                    | Value of `status` property of returned CCM due to error: invalid CCM.                       |
| `STATUS_CODE_INVALID_FORMAT`                  | uint32 | 6                    | Value of `status` property of returned CCM due to error: invalid format.                    |
| `STATUS_CODE_MESSAGE_FEE_ERROR`               | uint32 | 7                    | Value of `status` property of returned CCM due to error: message fee error.                 |
| **Message Tags**                              |        |                      |                                                                  |
...
def send(sendingAddress: Address,
    ...
    if not isLive(receivingChainID):
        emitPersistentEvent(
            module = MODULE_NAME_INTEROPERABILITY,
            name = EVENT_NAME_CCM_PROCESSED,
            data = {"ccmID": ccmID, "result": STATUS_CODE_CHANNEL_UNAVAILABLE},
    ...
    try:
        ...
        emitPersistentEvent(
            module = MODULE_NAME_INTEROPERABILITY,
            name = EVENT_NAME_CCM_PROCESSED,
            data = {"ccmID": ccmID, "result": STATUS_CODE_INVALID_FORMAT},
    ...
    if fee > 0:
        try:
            ...
            emitPersistentEvent(
                module = MODULE_NAME_INTEROPERABILITY,
                name = EVENT_NAME_CCM_PROCESSED,
                data = {"ccmID": ccmID, "result": STATUS_CODE_MESSAGE_FEE_ERROR},
                topics = [ccm.sendingChainID, ccm.receivingChainID, ccmID]
            )
            raise e
    ...
    emitEvent(
        module = MODULE_NAME_INTEROPERABILITY,
        name = EVENT_NAME_CCM_SENT,
        data = {"ccmID": ccmID, "result": CCM_SENT, "result": STATUS_CODE_OK},
        topics = [ccm.sendingChainID, ccm.receivingChainID, ccmID]
    )
```
