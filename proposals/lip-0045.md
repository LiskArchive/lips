...
```python
def appendToInboxTree(chainID: bytes, appendData: bytes) -> None:
...
```python
def appendToOutboxTree(chainID: bytes, appendData: bytes) -> None:
    # Update root, appendPath, and size
...
```python
def addToOutbox(chainID: bytes, ccm: CCM) -> None:
...
```python
def sendInternal(feeAddress: bytes, moduleID: bytes, crossChainCommandID: bytes, receivingChainID: bytes, fee: uint64, status: uint32, params: bytes) -> None:
    # Processing on the mainchain
    if ownChainAccount.ID == MAINCHAIN_ID:
...
```python
def terminateChainInternal(chainID: bytes) -> None:
    sendInternal(
        EMPTY_FEE_ADDRESS,
...
```python
def createTerminatedStateAccount(chainID: bytes, : bytes | None = None) -> None:
    if stateRoot:
...
```python
def createTerminatedOutboxAccount(chainID: bytes, outboxRoot: bytes, outboxSize: uint64, partnerChainInboxSize: uint64) -> None:
    terminatedOutbox = {
...
```python
def send(feeAddress: bytes, moduleID: bytes, crossChainCommandID: bytes, receivingChainID: bytes, fee: uint64, status: uint32, params: bytes) -> None:
    return sendInternal(
        feeAddress,
        moduleID,
        crossChainCommandID,
...
```python
def terminateChain(chainID: bytes) -> None:
    if terminatedStateAccount(chainID) exists:
