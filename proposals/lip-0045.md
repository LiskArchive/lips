...
| `CCM_PROCESSED_CODE_FAILED_CCM`                               | uint32 | 4    | Value of `code` of CCM processed event if processing failed due to: failure of cross-chain command execution.           |
| `CCM_PROCESSED_CODE_INVALID_CCM_VALIDATION_EXCEPTION`         | uint32 | 5    | Value of `code` of CCM processed event if processing failed due to: failure of validation .                             |
| `CCM_PROCESSED_CODE_INVALID_CCM_VERIFY_CCM_EXCEPTION`         | uint32 | 6    | Value of `code` of CCM processed event if processing failed due to: failure of CCM verification.                        |
| `CCM_PROCESSED_CODE_INVALID_CCM_VERIFY_EXCEPTION`             | uint32 | 7    | Value of `code` of CCM processed event if processing failed due to: failure of cross-chain command verification.        |
| `CCM_PROCESSED_CODE_INVALID_CCM_BEFORE_CCM_EXECUTE_EXCEPTION` | uint32 | 8    | Value of `code` of CCM processed event if processing failed due to: failure of before cross-chain command verification. |
| `CCM_PROCESSED_CODE_INVALID_CCM_AFTER_CCM_EXECUTE_EXCEPTION`  | uint32 | 9    | Value of `code` of CCM processed event if processing failed due to: failure of after cross-chain command verification.  |
