...
```python
def isChainIDAvailable(chainName: str) -> bool:
    if if not re.match(r"^[a-z0-9!@$&_.]+$", chainName):
        return False
    
    if len("chainName") < minLength or len("chainName") > maxLength:
       return False
       
    if if there exists an entry in the registered names substore with store key equal to chainName:
       return False
