...
| `CROSS_CHAIN_COMMAND_ID_SIDECHAIN_TERMINATED` | bytes | 0x0002                    | Cross-chain command ID of sidechain terminated CCM.              |
| **Chain Status**                              |        |                      |                                                                  |
...
def addToOutbox(chainID: ChainID, ccm: CCM) -> None:
    serializedMessage = encode(crossChainMessageSchema, ccm)
...
The `bounce` function is used when a CCM cannot be forwarded. It returns the CCM back to the sending chain, with a `CCM_STATUS_CHANNEL_UNAVAILABLE` error status. This function is only called on the mainchain when processing a CCU. It is specified in [LIP 0049][lip-0049].
