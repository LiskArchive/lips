...
| `RECOVERED_STORE_VALUE` | bytes | `\x00 * 32` | The constant value used for recovered entries of the state recovery command. Since there is no known sha256 pre-image for this constant, the same store entry cannot be recovered twice. |
| **Length Constants** | | | |
