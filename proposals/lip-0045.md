...
| `NUM_ACTIVE_VALIDATORS_MAINCHAIN` | uint32 | 101 | The maximum number of mainchain validators. |
...
| `COMMAND_RECOVER_STATE` | string | "recoverState" | Name of state recovery command. |
...
| `COMMAND_INITIALIZE_STATE_RECOVERY` | string | "initializeStateRecovery" | Name of state recovery initialization command. |
| `COMMAND_INITIALIZE_MESSAGE_RECOVERY` | string | "initializeMessageRecovery" | Name of message recovery initialization command. |
...
The `isChannelActive` function checks that a chain (or the mainchain if the chain account does not exist) has status `CHAIN_STATUS_ACTIVE`. On the mainchain, it also checks the liveness condition for the partner chain (which would imply termination if violated).
...
def isChannelActive(chainID: ChainID) -> bool:
    ...
    if ownChainAccount.chainID == getMainchainID():
        ...
        if not chainAccount(chainID).status == CHAIN_STATUS_ACTIVE:
            return False
        
        # Check liveness condition for active chains.
        timestamp = timestamp of the block where this state transition is processed
        if timestamp - chainAccount(chainID).lastCertificate.timestamp > LIVENESS_LIMIT:
            return False
