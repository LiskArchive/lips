...
| `LIVENESS_LIMIT`                              | uint32 | 30 * 24 * 3600           | The maximum time interval for the liveness condition.            |
...
* For each entry `channelData` in `interoperabilityAsset.channelDataSubstore`, let `channel = channelData.storeValue`:
  * Either `channel.messageFeeTokenID={chainID=1, localID=0}` (corresponding to the LSK token), or `channel.messageFeeTokenID.chainID` must be equal to `channelData.storeKey` or `ownChainAccount.chainID` (the message fee token must be a native token of either chains).
