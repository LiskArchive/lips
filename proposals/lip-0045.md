...
| **General Constants**                         |        |                      |                                                                  |
| `OWN_CHAIN_ID`                                | bytes  |                      | The chain ID of the chain udner consideration.                   |
...
The `isLive` function checks that a partner chain has not been terminated. On the mainchain, it also checks the liveness requirement for the partner chain (which would imply termination).
...
The `bounce` function returns a CCM that errored during the CCM execution stages to the sending chain if the message fee is larger than a minimum return fee (that depends on the message length) and the status of the CCM is `CCM_OK`.
If the function is called during the cross-chain command execution (see [`apply` function][lip-0049#apply]), the error code will be `ERROR_CODE_FAILED_CCM`. 
...
def bounce(ccm: CCM, errorCode: uint32) -> None:
    ...
    if ccm.status == CCM_OK and ccm.fee >= minimumFee:
        ...
        if errorCode == ERROR_CODE_FAILED_CCM:
            ccm.fee = 0
        else:
            ccm.fee -= minimumFee     
        ...
        ccm.status = CCM_DISCARDED
        newCcmID = sha256(encode(crossChainMessageSchema, ccm))
        emitEvent(
            module = MODULE_NAME_INTEROPERABILITY,
            name = EVENT_NAME_CCM_PROCESSED,
            data = {"ccmID": newCcmID, "result": CCM_DISCARDED},
            topics = [ccm.sendingChainID, ccm.receivingChainID]
        )
```
