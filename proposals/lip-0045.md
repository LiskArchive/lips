...
* For every `chainInfo` in `chainInfos` with empty `chainInfo.terminatedState` add an entry to the outbox root substore with key `chainInfo.chainID` and value `chainInfo.channelData.outbox.root`.
