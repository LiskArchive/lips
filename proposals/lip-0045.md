...
* For each entry `chainInfo` in `chainInfos`, let `chainData = chainInfo.chainData`, then check:
  ...
  * all `chainData.name` are from the character set `a-z0-9!@$&_.` and length between `MIN_CHAIN_NAME_LENGTH` and `MAX_CHAIN_NAME_LENGTH`;
...
* If `ownChainName` is not the empty string and `ownChainNonce != 0`, add an entry to the own chain substore with key set to `EMPTY_BYTES` and value set to `{"name": ownChainName, "chainID": OWN_CHAIN_ID, "nonce": ownChainNonce}`.
