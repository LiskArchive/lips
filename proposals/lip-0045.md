...
| `EVENT_NAME_CCM_PROCESSED`                         | string | "ccmSent"            | Name of the cross-chain message processed event.                 |
...
| **Cross-chain Message Results**               |        |                      |                                                                  |
| `CCM_APPLIED`                                 | uint32 | 0                    | Result for CCM processed event: applied.                         |
| `CCM_FORWARDED`                               | uint32 | 1                    | Result for CCM processed event: forwarded.                       |
| `CCM_BOUNCED`                                 | uint32 | 2                    | Result for CCM processed event: bounced.                         |
| `CCM_DISCARDED`                               | uint32 | 3                    | Result for CCM processed event: discarded.                       |
| **Cross-chain Message Codes**                 |        |                      |                                                                  |
| `CODE_OK`                                     | uint32 | 0                    | Code: ok. New CCMs have this initial status.                     |
| `ERROR_CODE_CHANNEL_UNAVAILABLE`              | uint32 | 1                    | Error code: channel unavailable.                                 |
| `ERROR_CODE_MODULE_NOT_SUPPORTED`             | uint32 | 2                    | Error code: module not supported.                                |
| `ERROR_CODE_CROSS_CHAIN_COMMAND_NOT_SUPPORTED`| uint32 | 3                    | Error code: cross-chain command not supported.                   |
| `ERROR_CODE_FAILED_CCM`                       | uint32 | 4                    | Error code: failed ccm execution.                                |
| `ERROR_CODE_INVALID_CCM`                      | uint32 | 5                    | Error code: invalid CCM.                                         |
| `ERROR_CODE_INVALID_FORMAT`                   | uint32 | 6                    | Error code: invalid format.                                      |
| `ERROR_CODE_MESSAGE_FEE_ERROR`                | uint32 | 7                    | Error code: message fee error.                                   |
| **Message Tags**                              |        |                      |                                                                  |
...
The `bounce` function returns a CCM that errored during the CCM execution stages to the sending chain if the message fee is larger than a minimum return fee (that depends on the message length) and the status of the CCM is `CODE_OK`.
...
def bounce(ccm: CCM, errorCode: uint32) -> None:
    ...
    if ccm.status == CODE_OK and ccm.fee >= minimumFee:
        emitEvent(
            module = MODULE_NAME_INTEROPERABILITY,
            name = EVENT_NAME_CCM_PROCESSED,
            data = {"ccmID": ccmID, "result": CCM_BOUNCED, "code": errorCode},
        ...
        emitEvent(
            module = MODULE_NAME_INTEROPERABILITY,
            name = EVENT_NAME_CCM_PROCESSED,
            data = {"ccmID": newCcmID, "result": CCM_SENT, "code": errorCode},
        ...
        emitEvent(
            module = MODULE_NAME_INTEROPERABILITY,
            name = EVENT_NAME_CCM_PROCESSED,
            data = {"ccmID": ccmID, "result": CCM_DISCARDED, "code": errorCode},
            topics = [ccm.sendingChainID, ccm.receivingChainID]
        )
```
...
Notice that the CCM ID is automatically included as a default topic (as any event emitted as part of the processing of a CCM). 
When the `result` property is `CCM_SENT`, there is an additional topic set to the ID of the CCM that was sent. 
In this case, the default topic may be missing.
...
def send(sendingAddress: Address, 
    ...
    emitEvent(
        module = MODULE_NAME_INTEROPERABILITY,
        name = EVENT_NAME_CCM_PROCESSED,
        data = {"ccmID": ccmID, "result": CCM_SENT, "result": CODE_OK},
        topics = [ccm.sendingChainID, ccm.receivingChainID, ccmID]
    )
```
