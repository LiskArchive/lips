...
def sendInternal(sendingAddress: Address,
    ...
    try:
        ...
        emitCCMFailedEvent(ccm, CCM_SENT_FAILED_CODE_INVALID_FORMAT)
        raise e
    ...
    if receivingChainID == ownChainAccount.chainID:
        emitCCMFailedEvent(ccm, CCM_SENT_FAILED_CODE_INVALID_RECEIVING_CHAIN)
    ...
    if not isLive(receivingChainID):
        emitCCMFailedEvent(ccm, CCM_SENT_FAILED_CODE_CHANNEL_UNAVAILABLE)
    ...
    if fee > 0:
        try:
            ...
            emitCCMFailedEvent(ccm, CCM_SENT_FAILED_CODE_MESSAGE_FEE_EXCEPTION)
            raise e
...
def emitCCMFailedEvent(ccm: CCM, code: uint32):
    emitPersistentEvent(
        module = MODULE_NAME_INTEROPERABILITY,
