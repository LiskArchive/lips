...
| `CCM_SENT_FAILED_CODE_INVALID_PARAMS` | uint32 | 12 | Value of `code` of CCM Sent Failed Event if sending failed due to invalid params property. |
| `CCM_SENT_FAILED_CODE_INVALID_FORMAT` | uint32 | 13 | Value of `code` of CCM Sent Failed Event if sending failed due to invalid message format. |
| `CCM_SENT_FAILED_CODE_INVALID_RECEIVING_CHAIN` | uint32 | 14 | Value of `code` of CCM Sent Failed Event if sending failed due to invalid receiving chain. |
| **Chain Status** | | | |
...
| `EMPTY_CCM` | CCM | An empty object following the [`crossChainMessageSchema`][lip-0049#ccmschema] schema. | The empty ccm object. |
...
| `MAX_CCM_SIZE` | uint32 | 10*1024 | The maximum size of a cross-chain message in bytes. |
| **Length Constants** | | | |
...
| `ChainAccount` | object | | An entry in the chain data substore, must follow the `chainDataSchema` schema. |
| `Channel` | object | | An entry in the channel substore, must follow the `channelDataSchema` schema. |
...
* Notation: For the rest of this proposal let `chainAccount(chainID)` be the entry in the chain data substore identified by the store key `chainID`.
...
* Notation: For the rest of this proposal let `channel(chainID)` be the entry in the channel data substore identified by the store key `chainID`.
...
* Notation: For the rest of this proposal let `validators(chainID)` be the entry in the chain validators substore identified by the store key `chainID`.
...
* The store value is an object with `chainID == CHAIN_ID_MAINCHAIN`, serialized using the JSON schema `registeredNamesSchema`.
...
The `calculateNewActiveValidators` function calculates the new active validators of a chain account based on the `newBLSKeys` and `newBFTWeights` properties of a CCU. It is specified in [LIP 0053][lip-0053].
...
* For each entry `chainValidators` in `interoperabilityAsset.chainValidatorsSubstore`, let `activeValidators = chainValidators.storeValue.activeValidators` and let `certificateThreshold = chainValidators.storeValue.certificateThreshold`:
  ...
  * Check that `totalWeight//3 + 1 <= certificateThreshold <= totalWeight`.
