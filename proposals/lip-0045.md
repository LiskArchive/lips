...
| **Events**                                    |        |                      |                                                                  |
| `TYPE_ID_CHAIN_ACCOUNT_UPDATED`                | bytes  | 0x0001               | Type ID of the chain account updated event.                       |
...
| `CCM_PROCESSED_STATUS_FAILED`                 | uint32 | 6                    | Status code for CCM processed event: failed.                     |
...
| `CCMID` | bytes   | Must be of length `HASH_LENGTH`. | ID of a cross-chain message.   |
...
def sendInternal(sendingAddress: Address, 
    ...
    topics = [ccmID, ccm.sendingChainID, ccm.receivingChainID]
    if len(originalCCMID) == CCM_ID_LENGTH:
        topics.append(originalCCMID)
    emitEvent(
        moduleID = MODULE_ID_INTEROPERABILITY,
        typeID = TYPE_ID_CCM_PROCESSED,
        data = {"ccmID": ccmID, "status": CCM_PROCESSED_STATUS_SENT},
        topics=topics
    )
```
...
This event has `typeID = TYPE_ID_CHAIN_ACCOUNT_UPDATED`. 
...
This event has `typeID = TYPE_ID_CCM_PROCESSED`. This event is emitted when a CCM is sent or processed.
The status property of the data schema can take one of the following values:

1. `CCM_PROCESSED_STATUS_SENT`: The CCM was sent to the receiving chain.
2. `CCM_PROCESSED_STATUS_CHANNEL_UNAVAILABLE`: The receiving chain is not available.
3. `CCM_PROCESSED_STATUS_MODULE_NOT_SUPPORTED`: The receiving chain does not support the module.
4. `CCM_PROCESSED_STATUS_COMMAND_NOT_SUPPORTED`: The receiving chain does not support the command.
5. `CCM_PROCESSED_STATUS_FORWARDED`: The CCM was forwarded to the receiving chain.
6. `CCM_PROCESSED_STATUS_APPLIED`: The CCM was applied on the receiving chain.
7. `CCM_PROCESSED_STATUS_DISCARDED`: The CCM was discarded.
8. `CCM_PROCESSED_STATUS_RECOVERY_FORWARDED`: The CCM was forwarded to the original sending chain for recovery.
9. `CCM_PROCESSED_STATUS_RECOVERY_APPLIED`: The CCM was recovered and applied on the original sending chain.
10. `CCM_PROCESSED_STATUS_RECOVERY_DISCARDED`: The CCM was ricovered and discarded.
...
- `ccmID`: The ID of the CCM that was processed.
...
def send(sendingAddress: Address, 
    ...
    status: uint32, 
    params: bytes,
    originalCCMID: bytes = EMPTY_BYTES) -> None:
    return sendInternal(
        sendingAddress,
        moduleID,
        crossChainCommandID,
...
```python
def error(ccm: CCM, errorStatus: uint32, relatedCCMIDs: list[bytes]) -> None:
    ...
    originalCCMID = SHA-256(encode(crossChainMessageSchema, ccm))
    return sendInternal(
        EMPTY_FEE_ADDRESS,
        ccm.moduleID,
