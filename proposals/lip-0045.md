...
| `CCU` | object | Must follow the `transactionSchema` schema, except that the `params` property satisfies `crossChainUpdateTransactionParams` instead of being of type `byte`. | The type of a cross-chain update transaction with deserialized `params` property. |
