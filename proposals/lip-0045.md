...
| `RECOVERED_STORE_VALUE` | bytes | 32 bytes all set to 0 | The constant value used for recovered entries of the state recovery command. Since there is no known sha256 pre-image for this constant, the same store entry cannot be recovered twice. |
| **Length Constants** | | | |
...
- https://github.com/LiskHQ/lisk-sdk/tree/development/framework/src/modules/interoperability