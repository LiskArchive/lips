...
| `CCM_STATUS_CHANNEL_UNAVAILABLE`              | uint32 | 1                    | Status assigned to CCMs when channel is unavailable.             |
| `CCM_STATUS_MODULE_NOT_SUPPORTED`             | uint32 | 2                    | Status assigned to CCMs when module is not supported.            |
| `CCM_STATUS_CROSS_CHAIN_COMMAND_NOT_SUPPORTED`| uint32 | 3                    | Status assigned to CCMs when cross-chain command is not supported. |
| `CCM_STATUS_FAILED`                           | uint32 | 5                    | Status assigned to CCMs when cross-chain command execution fails.  |
| `CCM_STATUS_INVALID`                          | uint32 | 6                    | Status assigned to CCMs when invalid.                            |
| `CCM_STATUS_RECOVERED`                        | uint32 | 11                   | Status assigned to CCMs after recovery.                          |
| **Events**                                    |        |                      |                                                                  |
...
| `CCM_PROCESSED_STATUS_APPLIED`                | uint32 | 0                    | Status code for CCM processed event: applied.                    |
...
| `CCM_PROCESSED_CROSS_CHAIN_COMMAND_NOT_SUPPORTED` | uint32 | 3                | Status code for CCM processed event: command not supported.      |
...
| `CCM_PROCESSED_STATUS_BOUNCED`                | uint32 | 7                    | Status code for CCM processed event: bounced.                    |
| `CCM_PROCESSED_STATUS_DISCARDED`              | uint32 | 8                    | Status code for CCM processed event: discarded.                  |
| `CCM_PROCESSED_STATUS_INTEROPERABILITY_VIOLATION` | uint32 | 9                | Status code for CCM processed event: interoperability violation. |
| `CCM_PROCESSED_STATUS_RECOVERY_FORWARDED`     | uint32 | 10                   | Status code for CCM processed event: recovered message forwarded.|
| `CCM_PROCESSED_STATUS_RECOVERY_APPLIED`       | uint32 | 11                   | Status code for CCM processed event: recovered message applied.  |
| `CCM_PROCESSED_STATUS_RECOVERY_FAILED`        | uint32 | 12                   | Status code for CCM processed event: recovered message failed.   |
| `CCM_PROCESSED_STATUS_RECOVERY_DISCARDED`     | uint32 | 13                   | Status code for CCM processed event: recovered message discarded.|
| **Message Tags**                              |        |                      |                                                                  |
...
#### bounce and bounceWithoutFee

The `bounce` and `bounceWithoutFee` functions return a CCM that errored during the CCM execution stages to the sending chain. 
`bounceWithoutFee` is called if an error occurs during the cross-chain command execution (see [`apply` function][lip-0049#apply]) in the receiving chain. 
In this case, the fee property is set to 0 and the message is returned only if the fee was larger than a minimum return fee and the status of the CCM is `CCM_STATUS_OK`.
`bounce` is called if an error occurs in any other stage.
In this case, the fee is not changed and the message is returned if the status of the CCM is `CCM_STATUS_OK`.

```python
def bounce(ccm: CCM, errorStatus: uint32, eventStatus: uint32) -> None:
    ...
    if ccm.status == CCM_STATUS_OK:
        ...
        emitEvent(
            module = MODULE_NAME_INTEROPERABILITY,
            name = EVENT_NAME_CCM_PROCESSED,
            data = {"ccmID": newCcmID, "status": CCM_PROCESSED_STATUS_BOUNCED},
            topics = [ccm.sendingChainID, ccm.receivingChainID]
        )
    else:
        emitEvent(
...
```
```python
def bounceWithoutFee(ccm: CCM, errorStatus: uint32, eventStatus: uint32) -> None:
    ccmID = sha256(encode(crossChainMessageSchema, ccm))
    emitEvent(
        module = MODULE_NAME_INTEROPERABILITY,
        name = EVENT_NAME_CCM_PROCESSED,
    ...
    if ccm.status == CCM_STATUS_OK and ccm.fee >= MIN_RETURN_FEE * len(encode(crossChainMessageSchema, ccm)):
        ccm.status = errorStatus
        ccm.sendingChainID, ccm.receivingChainID = ccm.receivingChainID, ccm.sendingChainID
        # The fee was already assigned to the relayer, set it to 0 in the CCM.
        ccm.fee = 0
        addToOutbox(ccm.receivingChainID, ccm)
        newCcmID = sha256(encode(crossChainMessageSchema, ccm))
        emitEvent(
...
# Create cross-chain message
    ...
    try:
        ...
        emitPersistentEvent(
            ...
            topics = [ownChainAccount.chainID, receivingChainID, EMPTY_BYTES]
        )
        raise e
...
def getMessageFeeTokenID(chainID: ChainID) -> TokenID:
    # Check for direct channel
    if chainAccount(chainID) does not exists:
