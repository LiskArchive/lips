...
#### [Sidechain Registration Command][research:chain-registration]
...
#### [Mainchain Registration Command][research:chain-registration]
...
#### [Cross-Chain Update Commands][research:ccu]
...
#### [Recovery Initialization Command][research:sidechain-recovery]
...
#### [State Recovery Command][research:sidechain-recovery]
...
#### [Message Recovery Command][research:sidechain-recovery]
...
A sidechain registers on the mainchain with a [sidechain registration command][research:chain-registration]. This command creates the sidechain account on the mainchain, with initial status set to ''registered''. Thereafter, the mainchain account is created on a sidechain with a [mainchain registration command][research:chain-registration], with initial status set to ''registered''.
...
<img src="lip-0045/store.png" width="80%">
...
In the Lisk ecosystem, the Lisk mainchain plays a central role, distinct from any other chain. It acts as an intermediary chain, [relaying cross-chain messages between sidechains][research:ccu]. This has a few notable advantages:
...
The `createCrossChainMessage` function creates and returns a new CCM. It is specified in the ["Cross-chain Messages" LIP][research:ccm].

#### validateFormat

The `validateFormat` function checks that a CCM follows the correct schema and does not exceed a size limit of 10KB. It is specified in the ["Cross-chain Messages" LIP][research:ccm].

#### process

The `process` function applies or forwards CCMs, including the handling of the different error cases. It is specified in the ["Cross-chain Messages" LIP][research:ccm].
...
* [Sidechain registration command][research:chain-registration]
* [Mainchain registration command][research:chain-registration]
* [Cross-Chain update command][research:ccu]
* [Recovery initialization command][research:sidechain-recovery]
* [State recovery command][research:sidechain-recovery]
* [Message recovery command][research:sidechain-recovery]
...
This proposal, together with [LIP "Chain registration"][research:chain-registration], [LIP "Cross-chain messages"][research:ccm], [LIP "Introduce cross-chain update transactions"][research:ccu], and [LIP "Sidechain recovery transactions"][research:sidechain-recovery], is part of the Interoperability module. Chains adding this module will need to do so with a hard fork.
...
[research:chain-registration]: https://research.lisk.com/t/chain-registration/291/2
