...
| **Interoperability Commands**                 |        |                      |                                                                  |
| `COMMAND_SIDECHAIN_REG`                       | string | "sidechainRegistration" | Name of sidechain registration command.                       |
| `COMMAND_MAINCHAIN_REG`                       | string | "mainchainRegistration" | Name of mainchain registration command.                       |
| `COMMAND_SIDECHAIN_CCU`                       | string | "sidechainCrossChainUpdate" | Name of sidechain cross-chain update command.             |
| `COMMAND_MAINCHAIN_CCU`                       | string | "mainchainCrossChainUpdate" | Name of mainchain cross-chain update command.             |
| `COMMAND_STATE_RECOVERY`                      | string | "stateRecovery" | Name of state recovery initialization command.                        |
| `COMMAND_MESSAGE_RECOVERY`                    | string | "messageRecovery" | Name of message recovery command.                                   |
| `COMMAND_STATE_RECOVERY_INITIALIZATION`       | string | "stateRecoveryInitialization" | Name of message recovery initialization command.        |
| `COMMAND_MESSAGE_RECOVERY_INITIALIZATION`     | string | "messageRecoveryInitialization" | Name of initiate recovery command.                    |
...
| `EVENT_NAME_CCM_SEND_SUCCESS`                 | string | "ccmSendSucess"      | Name of the event emitted when successfully sending a CCM.       |
| `EVENT_NAME_CCM_SEND_FAILED`                  | string | "ccmSendFailed"      | Name of the event emitted when sending a CCM fails.              |
...
| `CCM_PROCESSED_CODE_INVALID_CCM_VALIDATION_EXCEPTION`           | uint32 | 5    | Value of `code` of CCM Processed Event if processing failed due to: exception in validation.                             |
| `CCM_PROCESSED_CODE_INVALID_CCM_VERIFY_CCM_EXCEPTION`           | uint32 | 6    | Value of `code` of CCM Processed Event if processing failed due to: exception in CCM verification.                        |
| `CCM_PROCESSED_CODE_INVALID_CCM_VERIFY_EXCEPTION`               | uint32 | 7    | Value of `code` of CCM Processed Event if processing failed due to: exception in cross-chain command verification.        |
| `CCM_PROCESSED_CODE_INVALID_CCM_BEFORE_CCC_EXECUTION_EXCEPTION` | uint32 | 8    | Value of `code` of CCM Processed Event if processing failed due to: exception in before cross-chain command execution.    |
| `CCM_PROCESSED_CODE_INVALID_CCM_AFTER_CCC_EXECUTION_EXCEPTION`  | uint32 | 9    | Value of `code` of CCM Processed Event if processing failed due to: exception in after cross-chain command execution.     |
| `CCM_PROCESSED_CODE_INVALID_CCM_BEFORE_CCC_FORWARDING_EXCEPTION`| uint32 |10    | Value of `code` of CCM Processed Event if processing failed due to: exception in before cross-chain command forwarding.   |
| `CCM_SEND_FAILED_CODE_CHANNEL_UNAVAILABLE`    | uint32 | 1                    | Value of `code` of CCM Sent Failed Event if sending failed due to the receiving chain not being active.                   |
| `CCM_SEND_FAILED_CODE_MESSAGE_FEE_EXCEPTION`  | uint32 | 2                    | Value of `code` of CCM Sent Failed Event if sending failed due to the fee payment failing.                                |
| `CCM_SEND_FAILED_CODE_INVALID_FORMAT`         | uint32 | 3                    | Value of `code` of CCM Sent Failed Event if sending failed due to invalid message format.                                 |
| **Chain Status**                              |        |                      |                                                                  |
...
The `bounce` function returns a CCM that errored during the CCM execution stages to the sending chain if the message fee is larger than a minimum return fee (that depends on the message length) and the status of the CCM is `CCM_STATUS_CODE_OK`.
If the function is called during the cross-chain command execution (see [`apply` function][lip-0049#apply]), the status of the returned message is `CCM_STATUS_CODE_FAILED_CCM`.
...
def bounce(ccm: CCM, newCCMStatus: uint32, ccmProcessedEventCode: uint32) -> None:
    ...
    if ccm.status == CCM_STATUS_CODE_OK and ccm.fee >= minimumFee:
        emitEvent(
            ...
            data = {"ccmID": ccmID, "result": CCM_PROCESSED_RESULT_BOUNCED, "code": ccmProcessedEventCode},
        ...
        emitEvent(
            module = MODULE_NAME_INTEROPERABILITY,
            name = EVENT_NAME_CCM_PROCESSED,
            data = {"ccmID": ccmID, "result": CCM_PROCESSED_RESULT_DISCARDED, "code": ccmProcessedEventCode},
            topics = [ccm.sendingChainID, ccm.receivingChainID]
        )
```
...
The `result` property of the data schema can take any of the constants with prefix `CCM_PROCESSED_RESULT_` in the "Events" section of the constants table as value.
The `code` property of the data schema can take any of the constants with prefix `CCM_PROCESSED_CODE_` in the "Events" section of the constants table as value.
...
#### CrossChainMessageSendSuccess

This event has `name = EVENT_NAME_CCM_SEND_SUCCESS`. This event is emitted when a CCM is sent successfully.
...
def send(sendingAddress: Address,
    ...
    try:
        ...
        ccm.params = empty bytes
        emitPersistentEvent(
            module = MODULE_NAME_INTEROPERABILITY,
            name = EVENT_NAME_CCM_SEND_FAILED,
            data = {"ccm": ccm, "code": CCM_SEND_FAILED_CODE_INVALID_FORMAT},
            topics = []
        )
        raise e
    ...
    if not isLive(receivingChainID):
        ...
        emitPersistentEvent(
            module = MODULE_NAME_INTEROPERABILITY,
            name = EVENT_NAME_CCM_SEND_FAILED,
            data = {"ccm": ccm, "code": CCM_SEND_FAILED_CODE_CHANNEL_UNAVAILABLE},
            topics = []
        )
    ...
    if fee > 0:
        try:
            ...
            emitPersistentEvent(
                module = MODULE_NAME_INTEROPERABILITY,
                name = EVENT_NAME_CCM_SEND_FAILED,
                data = {"ccm": ccm, "code": CCM_SEND_FAILED_CODE_MESSAGE_FEE_EXCEPTION},
                topics = []
            )
            raise e
