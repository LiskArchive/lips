...
def sendInternal(sendingAddress: Address, 
    ...
    paramsSchema = JSON schema associated with (moduleID, crossChainCommandID)
    ccm = {
...
- `ccmID`: The ID of the CCM that was processed. This topic is present only if this value differs from the ID of the CCM that triggered the event.
