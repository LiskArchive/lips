...
```python
sendInternal(feeAddress, moduleID, crossChainCommandID, receivingChainID, fee, status, parameters):
    # Processing on the mainchain
    if ownChainAccount.ID == MAINCHAIN_ID:
...
```python
send(feeAddress, moduleID, crossChainCommandID, receivingChainID, fee, status, parameters):
    return sendInternal(
        feeAddress, 
        moduleID, 
        crossChainCommandID, 
...
[lip-0049]: https://github.com/LiskHQ/lips/blob/master/proposals/lip-0049.md
