...
def terminateChain(chainID: ChainID) -> None:
    ...
    sendInternal(
        ...
        0,
        CCM_STATUS_CODE_OK,
        { "inboxSize": channel(chainID).inbox.size }
    )
...
The `send` function is used to create and add a message to the outbox of a partner chain. The ccm status is automatically set to `CCM_STATUS_OK`.
