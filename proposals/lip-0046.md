...
```python
isSeedValidInput(generatorAddress: bytes, seedReveal: bytes) -> bool:
...
```python
isSeedRevealValid(generatorAddress: bytes, seedReveal: bytes) -> bool:
    let seedObject be an element in validatorReveals array
    let lastSeed be the seedObject with the largest seedObject.height and seedObject.generatorAddress == generatorAddress
    if not lastSeed:
        return True
...
```python
getRandomBytes(height: int, numberOfSeeds: int) -> bytes:
