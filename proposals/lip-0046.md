...
| `MAX_LENGTH_VALIDATOR_REVEALS` | uint32   | 206             | Maximum length of the `validatorReveals` array. It should be set as twice the maximum length of the chains validator set. |
...
This function assesses the validity of the `seedReveal` property of a block. `seedReveal` is a `SEED_LENGTH` bytes value with the seed revealed by the generator of a certain block.
...
* While the size of the `validatorReveals` array is equal to `MAX_LENGTH_VALIDATOR_REVEALS`, delete the element of `validatorReveals` array with the smallest value of `height`.
