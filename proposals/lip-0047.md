...
* The `trs.params.name` property has to be unique with respect to the set of already registered authority names in the blockchain state, such that the entry with store prefix equal to `STORE_PREFIX_NAME` and store key equal to `trs.params.name` (serialized as a utf-8 encoded string) does not exist in the store.
...
Let `genesisBlockAssetBytes` be the bytes included in the block assets for the PoA module and let `genesisBlockAssetObject` be the deserialization of `genesisBlockAssetBytes` according to the `genesisPoAStoreSchema` schema, then: 

* Checks for the `genesisBlockAssetObject.validatorSubstore`: 
  * Check that the `address` properties of all entries in the `genesisBlockAssetObject.validatorSubstore.validators` array are pairwise distinct.
  * Check that the `name` property of every entry in the `genesisBlockAssetObject.validatorSubstore.validators` array contains only characters from the set `[a-z0-9!@$&_.]`, is not empty and is at most `MAX_LENGTH_NAME` characters long.
  * Check that the `name` property of all entries in the `genesisBlockAssetObject.validatorSubstore.validators` array are pairwise distinct. 
  * Check that the `generatorKey` property of every entry in the `genesisBlockAssetObject.validatorSubstore.validators` array has length 32.
  * Check that the `blsKey` property of every entry in the `genesisBlockAssetObject.validatorSubstore.validators` array has length 48.
  * Check that the `proofOfPossession` property of every entry in the `genesisBlockAssetObject.validatorSubstore.validators` array has length 96.

* Checks for the `genesisBlockAssetObject.snapshotSubstore`: 
  * Check that the `address` properties of entries in the `genesisBlockAssetObject.snapshotSubstore.activeValidators` are pairwise distinct.
  * Check that entries in the `genesisBlockAssetObject.snapshotSubstore.activeValidators` array are ordered lexicographically according to `address`.
  * Check that for every element `activeValidator` in the `genesisBlockAssetObject.snapshotSubstore.activeValidators` array, there is an entry with `address == activeValidator.address` in the `genesisBlockAssetObject.validatorSubstore.validators` array.
  * Check that the `weight` property of every entry in the `genesisBlockAssetObject.snapshotSubstore.activeValidators` array is a positive integer.
  * Let `totalWeight` be the sum of the `weight` property of every entry in `genesisBlockAssetObject.snapshotSubstore.activeValidators`. Then, check that `totalWeight` is less than or equal to `MAX_UINT64`.
  * Check that the value of `genesisBlockAssetObject.snapshotSubstore.threshold` is within the following range:
     * Minimum value: ⌊ ⅓ × `totalWeight`⌋ + 1
     * Maximum value: `totalWeight`
...
```python
chainProperties.roundEndHeight = g.header.height + len(snapshotStore(0).validators)
...
for every validator element in genesisBlockAssetObject.validatorSubstore.validators:
    registerValidatorKeys(validator.address, validator.proofOfPossession, validator.generatorKey, validator.blsKey) == True
...
[lip-weights]: https://research.lisk.com/t/add-weights-to-lisk-bft-consensus-protocol/289#validator-changes-8
