...
* Notation: Let `snapshotStore(roundNumber)` be the substore entry with prefix `SUBSTORE_PREFIX_SNAPSHOT`, deserialized using the `snapshotStoreSchema` schema,  and key `uint32be(roundNumber)`.
