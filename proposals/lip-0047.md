...
* Notation: Let `snapshotStore(roundNumber)` be the substore entry with prefix `SUBSTORE_PREFIX_SNAPSHOT` and key `roundNumber.to_bytes(4, byteorder='big')`.
...
Let `netID` be the byte array corresponding to the network ID of the chain. A transaction with module name `MODULE_NAME_POA` and command name `COMMAND_UPDATE_AUTHORITY` is processed as follows:
