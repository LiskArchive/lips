...
| `COMMAND_ID_REGISTRATION_AUTHORITY` | bytes    | `0x0000`                        |
| `COMMAND_ID_UPDATE_KEY`             | bytes    | `0x0001`                        |
| `COMMAND_ID_UPDATE_AUTHORITY`       | bytes    | `0x0002`                        |
| `TYPE_ID_UPDATE_AUTHORITY`          | bytes    | `0x0001`                        |
...
* Each substore key is a byte array `address` of length `NUM_BYTES_ADDRESS`, where `address` is the address of the user account registered as validator, either in the genesis block or with an [authority registration command](#authority-registration-command).
* Each substore value is the serialization of an object following the JSON schema `validatorObjectSchema` defined below.
* Let `validators(address)` denote the validator substore entry with the key `address`.
...
- `name` is a string representing the validator name, its value is set in the genesis block or with an [authority registration command](#authority-registration-command)
...
* Let `name(nameBytes)` denote the entry in the name substore with the key `nameBytes`.
...
- `address` is a byte array with the address of the user account registered as validator in the genesis block or with an [authority registration command](#authority-registration-command).
...
* Each store key is `roundNumber.to_bytes(4, byteorder='big')`, where `roundNumber` can be 0, 1 or 2 corresponding to the current round, the next round and in two rounds respectively.
* Each store value is the serialization of an object following `snapshotStoreSchema`.
* Notation: Let `snapshotStore(roundNumber)` be the store entry with prefix `STORE_PREFIX_SNAPSHOT` and key `roundNumber.to_bytes(4, byteorder='big')`.
...
This event has `typeID = TYPE_ID_UPDATE_AUTHORITY` and is emitted during update authority command execution. The event has no topics and records only the result status in data. The possible result values are: `[UPDATE_AUTHORITY_SUCCESS, UPDATE_AUTHORITY_FAIL_INVALID_SIGNATURE]`.

##### Data

```java
updateAuthorityDataSchema = {
    "type": "object",
    "properties": {
        "result": {
            "dataType": "uint32",
...
This command is equivalent to the [delegate registration command][lip-0057#delegate-registration] in the DPoS module and has the same schema and similar validity rules. The command ID of this transaction is `COMMAND_ID_REGISTRATION_AUTHORITY`.
...
registrationTransactionParamsSchema = {
    ...
    "required": ["name", "blsKey", "proofOfPossession", "generatorKey", "extraCommandFee"]
}
```

##### Verification

A transaction with module ID `MODULE_ID_POA` and command ID `COMMAND_ID_REGISTRATION_AUTHORITY` is verified as follows:

```python
def verify(trs: Transaction) -> None:
    if trs.params does not satisfy registrationTransactionParamsSchema:
        raise Exception()
    if trs.params.name is empty or (not trs.params.name.regexMatch("[a-z0-9!@$&_.]*")):
        raise Exception()
...
A transaction with module ID `MODULE_ID_POA` and command ID `COMMAND_ID_REGISTRATION_AUTHORITY` is executed as follows:
...
def execute(trs: Transaction) ->  None:
    ...
    name(bytes(trs.params.name, 'utf-8')) = encode(validatorAddressSchema, nameEntry)

    Validators.registerValidatorKeys(senderAddress, trs.params.proofOfPossession,
                                    trs.params.generatorKey, trs.params.blsKey)
```
...
A transaction with module ID `MODULE_ID_POA` and command ID `COMMAND_ID_UPDATE_KEY` is verified as follows:

```python
def verify(trs: Transaction) -> None:
    if trs.params does not satisfy updateGeneratorKeyParamsSchema:
        raise Exception()
    senderAddress = SHA-256(trs.senderPublicKey)[NUM_BYTES_ADDRESS]
...
A transaction with module ID `MODULE_ID_POA` and command ID `COMMAND_ID_UPDATE_KEY` is executed as follows:
...
A transaction with module ID `MODULE_ID_POA` and command ID `COMMAND_ID_UPDATE_AUTHORITY` is verified as follows:

```python
def verify(trs: Transaction) -> None:
    newValidators = trs.params.newValidators
...
Let `netID` be the byte array corresponding to the network ID of the chain. A transaction with module ID `MODULE_ID_POA` and command ID `COMMAND_ID_UPDATE_AUTHORITY` is processed as follows:
...
def execute(trs: Transaction) -> None:
    ...
    m = encode(validatorSignatureMessageSchema, {
                "newValidators": trs.params.newValidators,
                "threshold": trs.params.threshold,
                "validatorsUpdateNonce": trs.params.validatorsUpdateNonce
            })
    validatorInfos = []
    ...
    if verified != VALID:
        emitPersistentEvent(
            moduleID = MODULE_ID_POA,
            typeID = TYPE_ID_UPDATE_AUTHORITY,
    ...
    # set new authorities
    snapshotStore(2).validators = trs.params.newValidators
    snapshotStore(2).threshold = trs.params.threshold
    chainProperties.validatorsUpdateNonce = trs.params.validatorsUpdateNonce + 1
    emitEvent(
        moduleID = MODULE_ID_POA,
        typeID = TYPE_ID_UPDATE_AUTHORITY,
...
[rounds]: https://github.com/LiskHQ/lips/blob/main/proposals/lip-0057.md#round-number-and-end-of-rounds
[ethereum:poa]: https://github.com/ethereum/guide/blob/main/poa.md
