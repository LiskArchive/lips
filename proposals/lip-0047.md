...
Let `genesisBlockAssetBytes` be the bytes included in the `data` property of a deserialized entry in the block assets for the PoA module and let `assetPoA` be the deserialization of `genesisBlockAssetBytes` according to the `genesisPoAStoreSchema` schema, then: 
...
* Checks for the `assetPoA.snapshotSubstore`: 
  ...
  * Check that for every element `activeValidator` in the `assetPoA.snapshotSubstore.activeValidators` array, there is an entry with `address == activeValidator.address` in the `assetPoA.validators` array.
