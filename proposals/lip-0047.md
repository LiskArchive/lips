...
Let `genesisBlockAssetBytes` be the bytes included in the block assets for the PoA module and let `genesisBlockAssetObject` be the deserialization of `genesisBlockAssetBytes` according to the `genesisPoAStoreSchema` schema, given below. 

* Check that `genesisBlockAssetObject.PoADataSubstore.storePrefix` equals `STORE_PREFIX_VALIDATOR`.

* Check that every entry in !!!!!!!  contains only characters from the set `[a-z0-9!@$&_.]`, must not be empty and has to be at most `MAX_LENGTH_NAME` characters long.
* `tx.params.generatorKey` must have length 32.
* `tx.params.blsKey` must have length 48.
* `tx.params.proofOfPossession` must have length 96.

* Create  entry in the validator substore as:
  * `storeKey`: `address`, where `address` is the address of the sender of `trs`.
  * `storeValue`: The serialization of the object `validatorObject` following `validatorObjectSchema` with `validatorObject.name = trs.params.name`. 	

* Create  entry in the name substore as:
  * `storeKey`: `trs.params.name` serialized as a utf-8 encoded string.
  * `storeValue`: The serialization of the object `validatorAddress` following `validatorAddressSchema` with `validatorAddress.address = address` where `address` is the address of the sender of `trs`.

* Create entries in the snapshot substore as:

* Create an entry in the chain properties substore as specified above.
  * The properties in the serialized object as `storeValue` are initialized to their defaul values. 
