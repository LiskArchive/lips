...
This method can be invoked by other modules to charge an additional fee when a state increase is triggered during the execution of a transaction, e.g., when a new validator is registered in the PoS module, a new sidechain is registered, or a [user account is initialized][lip-0051#initializeuseraccountinternal] in the Token module: 
```python
...
Since managing message fees requires checking and updating the escrow balances for the message fee token, this task is assigned to the Token module. In particular, paying for message fees is called for in the Interoperability module, whose [`sendInternal`][lip-0045#sendinternal] method uses the [`payMessageFee`][lip-0051#paymessagefee] of the Token module. After the Token module has ensured that the message fee has been credited to the relayer's account, the Fee module handles the transaction fee processing in the receiving chain.

For protocol reasons (see, e.g., [`bounce`][lip-0045#bounce] and [`terminateChain`][lip-0045#terminatechain] methods in LIP 0045), the Interoperability module allows the message fee to be zero. Consequently, the choice of setting a minimum message fee is delegated to each module that provides its own cross-chain commands. Nevertheless, there is a minimum fee in case the user wants the failed CCM to be returned (see [`bounce`][lip-0045#bounce] method in LIP 0045). Thus, even though this fee is not mandatory, it is advisable to treat it as a suggested minimum fee. 

Furthermore, the CCM sender should ensure to include any additional fees necessary for the CCM execution, e.g., to pay for the user account initialization on the receiving chain. 

The fee processing is similar to transaction fees; note that the consumed part of the message fee contains only the so-called additional fees,  as there is no mandatory minimum fee. Similarly to transaction fees, the consumed part can either be burned or transferred to a fee pool, depending on the chain configuration. The available part of the message fee is left to the relayer to cover the cost of CCU execution (note the relayer pays transaction fees for posting the CCU in the receiving chain, since a CCU is a regular transaction) and to incentivize its participation in CCM relaying. However, in contrast to including transactions in a block, a relayer cannot prioritize CCMs with higher fees as the interoperability protocol requires them to be sent in order. This also implies that users have no reason to pay higher message fees than are required for successful CCM execution.
...
[lip-0045#sendinternal]: https://github.com/LiskHQ/lips/blob/main/proposals/lip-0045.md#sendinternal
