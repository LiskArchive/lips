...
Cross-chain messages, or CCMs, facilitate interoperability by enabling cross-chain transfer of data. They require an associated fee, distinct from the usual transaction fee. Similar to transaction fees, the consumed fee can either be burned or transferred to a fee pool depending on the configuration of the chain. The available fee is rewarded to a CCM relayer to cover the cost of CCM execution and to incentivize participation in CCM relaying.
