...
def verify(trs: Transaction) -> None:
    ...
    if not Token.userSubstoreExists(senderAddress, TOKEN_ID_FEE) or trs.fee > Token.getAvailableBalance(senderAddress, TOKEN_ID_FEE):
        raise Exception("Account not initialized or insufficient balance")
```
