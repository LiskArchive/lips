...
| `ZERO_FEE_PER_BYTE_MAX_BLOCK_HEIGHT`                   | uint32 | 0                                   | Block height ending the `MIN_FEE_PER_BYTE = 0` initial period. |
...
```python
def finalizeGenesisState(g: Block) -> None:
    if ADDRESS_FEE_POOL is not None:
        Token.initializeUserAccount(ADDRESS_FEE_POOL, TOKEN_ID_FEE)
```
...
def verify(trs: Transaction) -> None:
    ...
    h = b.header.height
    if h < ZERO_FEE_PER_BYTE_MAX_BLOCK_HEIGHT:
	    # Set MIN_FEE_PER_BYTE = 0 for the first ZERO_FEE_PER_BYTE_MAX_BLOCK_HEIGHT blocks after the genesis block has been generated.
        MIN_FEE_PER_BYTE = 0
...
def afterCommandExecute(trs: Transaction) -> None:
    ...
    if ADDRESS_FEE_POOL is not None and Token.userSubstoreExists(ADDRESS_FEE_POOL, TOKEN_ID_FEE):
        # Transfer the fees to the pool account address, if it exists and it is initialized for the fee token. 
        if not Token.userSubstoreExists(generatorAddress, TOKEN_ID_FEE):
            # If block generator account is not initialized for the fee token, all fees are transferred to the pool account address.
            ctx.availableTransactionFee = 0
...
def afterCrossChainCommandExecution(trs: Transaction, ccm: CCM) -> None:
    ...
    if ADDRESS_FEE_POOL is not None and Token.userSubstoreExists(ADDRESS_FEE_POOL, messageFeeTokenID):
