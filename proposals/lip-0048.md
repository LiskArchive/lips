...
```
The minimum fee is always subtracted from the part of the fee that goes to the block generator, as we explain below. Therefore, the minimum fee discourages validators from sending zero-cost transactions in the blocks they generate.
...
```
In addition, developers may decide to call the `payFee` method for transactions that require extensive computation, such as when verifying an inclusion proof.

### Fee Processing

We refer to the total of the minimum fee and the fees processed by the `payFee` method as the "consumed" part of the fee. The remainder of the fee, called the "available" fee, is given to the validator as a reward. In this way, validators are assumed to prioritize transactions with a higher available fee in case of network congestion. [After the transaction is executed](#after-command-execution), consumed fee is either burned or transferred to a dedicated pool address. 
