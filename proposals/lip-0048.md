...
In principle, any transaction that results in a state increase should require a fee. This consideration is implemented in the [`payFee`](#payfee) method, which can be invoked by other modules to charge an additional fee when a state increase is triggered during the execution of a transaction, e.g., when a new validator is registered in the PoS module, a new sidechain is registered, or a user account is initialized in the Token module. 
...
Cross-chain messages, or CCMs, facilitate interoperability by enabling cross-chain transfer of data. They require an associated fee, distinct from the usual transaction fee, to be paid by the relayer. Similar to transaction fees, the minimum CCM fee can either be burned or transferred to a fee pool depending on the configuration of the chain.
