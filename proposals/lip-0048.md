...
Processing of message fees is similar to that of transaction fees. It should be noted that the consumed part of the message fee contains only the so-called additional fees, as there is no mandatory minimum fee. The consumed part can either be burned or transferred to a fee pool, depending on the chain configuration. 
