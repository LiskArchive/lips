...
def apply(ccu: Transaction, ccm: CCM) -> None:
    try:
        ...
        bounce(ccm, ERROR_CODE_INVALID_CCM)
        return

    if ccm.module is supported and ccm.crossChainCommand is supported:
    ...
    if ccm.module is not supported:
        bounceWithoutFee(ccm, ERROR_CODE_MODULE_NOT_SUPPORTED)
    elif crossChainCommand is not supported:
        bounceWithoutFee(ccm, ERROR_CODE_CCCOMMAND_NOT_SUPPORTED)
    else:
        # Create a second state snapshot.
        ...
        try:
            ...
            bounceWithoutFee(ccm, ERROR_CODE_FAILED_CCM)
...
def forward(ccu: CCU, ccm: CCM) -> None:
    ...
    if chainAccount(ccm.receivingChainID) does not exist:
        return
    # If the chain is not active, do not continue
...
```python
def execute(ccu: CCU, ccm: CCM) -> None:
...
##### Verification
```python
def verify(ccu: CCU, ccm: CCM) -> None:
