...
def execute(ccu: CCU, ccm: CCM) -> None:
    ...
    if terminatedStateAccount(ccm.chainID) exists:
        if terminatedStateAccount(ccm.chainID).initialized == True:
            return
        terminatedStateAccount(ccu.params.chainID).stateRoot = ccmParams.stateRoot
        terminatedStateAccount(ccu.params.chainID).mainchainStateRoot = EMPTY_HASH
        terminatedStateAccount(ccu.params.chainID).initialized = True
    else:
