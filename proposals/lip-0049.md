...
def apply(ccu: Transaction, ccm: CCM) -> None:
    ...
    try:
        ...
        emitEvent(
            ...
            data = {"ccmID": ccmID, "result": ERROR_CODE_INVALID_CCM},
    ...
    if ccm.module is not supported:
        bounce(ccm, ERROR_CODE_MODULE_NOT_SUPPORTED)
        return
    elif crossChainCommand is not supported:
        bounce(ccm, ERROR_CODE_CCCOMMAND_NOT_SUPPORTED)
        return
    ...
    try:
        ...
        emitEvent(
            module = MODULE_NAME_INTEROPERABILITY,
            name = EVENT_NAME_CCM_PROCESSED,
            data = {"ccmID": ccmID, "result": ERROR_CODE_FAILED_CCM},
            topics = [ccm.sendingChainID, ccm.receivingChainID]
        )
        bounce(ccm, ERROR_CODE_FAILED_CCM)
...
def forward(ccu: Transaction, ccm: CCM) -> None:
    ...
    if chainAccount(ccm.receivingChainID) does not exist:
        bounce(ccm, ERROR_CODE_CHANNEL_UNAVAILABLE, ERROR_CODE_CHANNEL_UNAVAILABLE)
        return
    ...
    if not isLive(ccm.receivingChainID):
        ...
        # If the chain is active, it means it violated the liveness condition and we terminate it.
        # If the chain is already terminated, terminateChain does nothing.
    ...
    baseSnapshot = snapshot of the current state
    try:  
        # Call the beforeCrossChainCommandForwarding functions from other modules
        # For example, the Token module transfers the fee from escrow to escrow.
        for each module mdl for which beforeCrossChainCommandForwarding exists:
            mdl.beforeCrossChainCommandForwarding(ccu, ccm)
    except:
        revert state to baseSnapshot
        terminateChain(ccm.sendingChainID)
        return
...
The `params` schema for the channel terminated CCM is:
...
* `stateRoot`: last state root of the terminated sidechain,
* `inboxSize`: size of the channel inbox on the terminated chain.
...
def execute(ccu: Transaction, ccm: CCM) -> None:
    if not isLive(ccm.sendingChainID):
        createTerminatedStateAccount(ccm.sendingChainID, ccm.params.stateRoot)
        createTerminatedOutboxAccount(
            ccm.sendinChainID,
            channel(ccm.params.chainID).outbox.root,
            channel(ccm.params.chainID).outbox.size,
            ccm.params.inboxSize
        )
```
...
The role of the registration message is to guarantee that a registration transaction was included in the partner chain when activating the cross-chain channel. It also serves to guarantee that the channel was opened on the sidechain with the correct chain ID, name, and message fee token ID.
...
def verify(ccu: Transaction, ccm: CCM) -> None:
    ...
    if ccm.params.chainID != OWN_CHAIN_ID:
...
This cross-chain message can only be sent by the mainchain and therefore can only be processed on a sidechain.
...
The `createTerminatedStateAccount` and `terminateChain` functions are defined in [LIP 0045][lip-0045].
