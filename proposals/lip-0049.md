...
| `CROSS_CHAIN_COMMAND_ID_SIDECHAIN_STATUS_TERMINATED`  | bytes | 0x0002      |
...
def apply(ccu: CCU, ccm: CCM) -> None:
    ...
    if (ccm.moduleID, ccm.crossChainCommandID) is supported: 
        eventStatus = CCM_PROCESSED_STATUS_COMMAND_APPLIED  
        ...
        try: 
            ...
            eventStatus = CCM_PROCESSED_STATUS_COMMAND_FAILED  
    ...
    # If the status is not ok and the fee is sufficient, we send back an error message.
        ...
        # Calculate ID of original CCM, to emit an event later.
        ...
        sendInternal(
            ...
            errorStatus,
            ccm.params,
            ccmID # The original CCM ID is passed so that it can be added to the error CCM event topics.
        )
...
def forward(ccu: CCU, ccm: CCM) -> None:
    ...
    if (chainAccount(ccm.receivingChainID) exists
        ...
        emitEvent(
            moduleID = MODULE_ID_INTEROPERABILITY,
            typeID = TYPE_ID_CCM_PROCESSED,
            data = {"ccmID": originalCCMID, "status": CCM_PROCESSED_STATUS_FORWARDED},
            topics=[ccm.sendingChainID, ccm.receivingChainID, originalCCMID]
        )
    ...
    if ccm.status != CCM_STATUS_OK:
        ...
        emitEvent(
            moduleID = MODULE_ID_INTEROPERABILITY,
            typeID = TYPE_ID_CCM_PROCESSED,
            data = {"ccmID": originalCCMID, "status": CCM_PROCESSED_STATUS_DISCARDED},
            topics=[ccm.sendingChainID, ccm.receivingChainID, originalCCMID]
        )
        return
    ...
    emitEvent(
        moduleID = MODULE_ID_INTEROPERABILITY,
        typeID = TYPE_ID_CCM_PROCESSED,
        data = {"ccmID": originalCCMID, "status": CCM_PROCESSED_STATUS_CHANNEL_UNAVAILABLE},
        topics=[ccm.sendingChainID, ccm.receivingChainID, originalCCMID]
    )
    ...
    stmParams = encode(sidechainTerminatedCCMParamsSchema, {
...
```python
def execute(ccm: CCM) -> None:
    if (channel(ccm.sendingChainID).inbox.size != 1
    ...
    if ownChainAccount.ID == MAINCHAIN_ID:
        if ccm.nonce != 0 or ccm.receivingChainID != MAINCHAIN_ID:
            terminateChainInternal(ccm.sendingChainID)
