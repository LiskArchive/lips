...
def apply(ccu: CCU, ccm: CCM) -> None:
    ...
    # Else, we discard the message.
        ...
        eventStatus = CCM_PROCESSED_STATUS_COMMAND_DISCARDED  
        
    # Finally emit an event for the CCM processing.
    emitEvent(
        moduleID = MODULE_ID_INTEROPERABILITY,
        typeID = TYPE_ID_CCM_PROCESSED,
        data = {
...
def forward(ccu: CCU, ccm: CCM) -> None:
    if (chainAccount(ccm.receivingChainID) exists
        ...
        emitEvent(
            ...
            data = {
                "crossChainMessage": ccm,
                "status": CCM_PROCESSED_STATUS_FORWARDED
            },
            topics=[ccmID]
        )
    ...
    # Emit CCM processed event, using the ccmID of the original message as event topic.
    originalCCMID = SHA-256(encode(crossChainMessageSchema, ccm))
    emitEvent(
        moduleID = MODULE_ID_INTEROPERABILITY,
        typeID = TYPE_ID_CCM_PROCESSED,
        data = {
            "crossChainMessage": ccm,
            "status": CCM_PROCESSED_STATUS_CHANNEL_UNAVAILABLE
        },
        topics=[originalCCMID]
    )
