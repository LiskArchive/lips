...
def apply(ccu: Transaction, ccm: CCM) -> None:
    ...
    try:
        ...
        emitEvent(
            ...
            data = {"ccmID": ccmID, "result": CCM_PROCESSED_RESULT_DISCARDED, "code": INVALID_CCM},
    ...
    if ccm.module is not supported:
        bounce(ccm, STATUS_CODE_MODULE_NOT_SUPPORTED)
        return
    elif crossChainCommand is not supported:
        bounce(ccm, STATUS_CODE_CROSS_CHAIN_COMMAND_NOT_SUPPORTED)
        return
    ...
    try:
        ...
        emitEvent(
            module = MODULE_NAME_INTEROPERABILITY,
            name = EVENT_NAME_CCM_PROCESSED,
            data = {"ccmID": ccmID, "result": CCM_PROCESSED_RESULT_APPLIED, "code": STATUS_CODE_OK},
            topics = [ccm.sendingChainID, ccm.receivingChainID]
        )
    except:
        revert state to executionSnapshot
        bounce(ccm, STATUS_CODE_FAILED_CCM)
...
def forward(ccu: Transaction, ccm: CCM) -> None:
    ...
    if chainAccount(ccm.receivingChainID) does not exist:
        bounce(ccm, STATUS_CODE_CHANNEL_UNAVAILABLE)
        return
    ...
    if not isLive(ccm.receivingChainID):
        emitEvent(
            module = MODULE_NAME_INTEROPERABILITY,
            name = EVENT_NAME_CCM_PROCESSED,
            data = {"ccmID": ccmID, "result": CCM_PROCESSED_RESULT_DISCARDED, "code": STATUS_CODE_CHANNEL_UNAVAILABLE},
            topics = [ccm.sendingChainID, ccm.receivingChainID]
        )
    ...
    emitEvent(
        module = MODULE_NAME_INTEROPERABILITY,
        name = EVENT_NAME_CCM_PROCESSED,
        data = {"ccmID": ccmID, "result": CCM_PROCESSED_RESULT_FORWARDED, "code": STATUS_CODE_OK},
        topics = [ccm.sendingChainID, ccm.receivingChainID]
    )
```
