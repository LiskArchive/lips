...
This is enforced by the Interoperability when sending a CCM (see the [send method][lip-0045#send]) and when receiving a CCM via a cross-chain update transaction (see [LIP 0053][lip-0053]).
...
def validateFormat(ccm: CCM) -> None:
    # ccm must satisfy root schema
    if not validateObjectSchema(crossChainMessageSchema, ccm):    
        raise Exception("Cross-chain message does not satisfy schema.")
