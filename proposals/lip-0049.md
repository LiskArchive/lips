...
def apply(ccu: Transaction, ccm: CCM) -> None:
    ...
    if (ccm.module, ccm.crossChainCommand) is supported:
        ...
        try:  
            ...
            emitEvent(
                ...
                data = { "ccmID": ccmID, "status": CCM_PROCESSED_STATUS_INVALID},
        ...
        try:
            ...
            emitEvent(
                module = MODULE_NAME_INTEROPERABILITY,
                name = EVENT_NAME_CCM_PROCESSED,
                data = { "ccmID": ccmID, "status": CCM_PROCESSED_STATUS_FAILED},
                topics = [ccm.sendingChainID, ccm.receivingChainID]
            ) 
    ...
    elif ccm.status == CCM_STATUS_OK and ccm.fee >= MIN_RETURN_FEE * len(encode(crossChainMessageSchema, ccm)):
...
def forward(ccu: CCU, ccm: CCM) -> None:
    ...
    if isLive(ccm.receivingChainID) and chainAccount(ccm.receivingChainID).status == CHAIN_STATUS_ACTIVE:
        # Transfer the fee from escrow to escrow, call Token LIP. Notice that this should not fail.
        Token.forwardMessageFee(ccm.sendingChainID, ccm.receivingChainID, ccm.fee)
    ...
    if ccm.status != CCM_STATUS_OK:
        ...
        emitEvent(
            module = MODULE_NAME_INTEROPERABILITY,
            name = EVENT_NAME_CCM_PROCESSED,
            data = {"ccmID": ccmID, "status": CCM_PROCESSED_STATUS_FORWARDING_FAILED},
            topics = [ccm.sendingChainID, ccm.receivingChainID]
        )
        return
