...
def verify(ccu: CCU, ccm: CCM) -> None:
    ...
    if ccm.sendingChainID != ccu.params.sendingChainID:
...
def execute(ccu: CCU, ccm: CCM) -> None:
    ccmParams = decode(sidechainTerminatedCCMParamsSchema, ccm.params)
    ccuParams = decode(crossChainUpdateTransactionParams, ccu.params)

    if terminatedStateAccount(ccm.chainID) exists:
        if terminatedStateAccount(ccm.chainID).initialized == True:
            return
        terminatedStateAccount(ccuParams.chainID).stateRoot = ccmParams.stateRoot
        terminatedStateAccount(ccuParams.chainID).mainchainStateRoot = EMPTY_HASH
        terminatedStateAccount(ccuParams.chainID).initialized = True
    else:
