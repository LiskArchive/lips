...
| `CROSS_CHAIN_COMMAND_ID_SIDECHAIN_TERMINATED`  | bytes | 0x0002      |
...
```python
def validateFormat(ccm: CCM) -> bool:
...
def apply(ccu: CCU, ccm: CCM) -> None:
    ...
    # ccm is discarded and has no further effect if the status is not ok or the fee is insufficient.
...
def forward(ccu: CCU, ccm: CCM) -> None:
    if (chainAccount(ccm.receivingChainID) exists
        ...
        # If the previous call fails, just return 
        except:
            return
...
def bounce(ccm: CCM) -> None:
    ...
    # and chainAccount(newCCM.sendingChainID).status == CHAIN_ACTIVE
    # so we can call addToOutbox.
    addToOutbox(newCCM.sendingChainID, newCCM)
```
...
```python
def execute(ctm: CCM) -> None:
    createTerminatedStateAccount(ctm.sendingChainID)
```
...
```python
def execute(registrationCCM: CCM) -> None:
...
```python
def execute(stm: CCM) -> None:
    if stm.sendingChainID == MAINCHAIN_ID:
        if there exist an entry in the terminated state substore with storeKey = stm.params.chainID:
            return
