...
* `minReturnFeePerByte`: the minimum value of the CCM fee required for the CCM to be bounced in case of execution errors.
...
def verify(ccu: Transaction, ccm: CCM) -> None:
    ...
    if channel(ccm.sendingChainID).minReturnFeePerByte != ccm.params.minReturnFeePerByte:
