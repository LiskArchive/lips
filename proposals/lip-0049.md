...
def apply(ccu: Transaction, ccm: CCM) -> None:
    ...
    if (ccm.module, ccm.crossChainCommand) is supported:
        ...
        try:  
            ...
            bounce(ccm, STATUS_INVALID, CCM_PROCESSED_STATUS_INVALID)
            return
        ...
        try:
            ...
            emitEvent(
                ...
                data = { "ccmID": ccmID, "status": CCM_PROCESSED_STATUS_APPLIED},
                topics = [ccm.sendingChainID, ccm.receivingChainID]
            )
        except:
            revert state to executionSnapshot
            bounceWithoutFee(ccm, STATUS_FAILED, CCM_PROCESSED_STATUS_FAILED)
    ...
    bounce(ccm, errorStatus, eventStatus)
```
...
def forward(ccu: CCU, ccm: CCM) -> None:
    ...
    if isLive(ccm.receivingChainID) and chainAccount(ccm.receivingChainID).status == CHAIN_STATUS_ACTIVE:
        ...
        try:  
            ...
            revert state to baseSnapshot
            terminateChain(ccm.sendingChainID)
            bounce(ccm, CCM_STATUS_INVALID, CCM_PROCESSED_STATUS_INVALID)
            return
    ...
    bounce(ccm, CCM_STATUS_CHANNEL_UNAVAILABLE, CCM_PROCESSED_STATUS_CHANNEL_UNAVAILABLE)
