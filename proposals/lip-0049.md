...
_Figure 1: A schematic of the steps executed in the `pply` function. Blue boredered boxes indicate the emission of an event. Function called from other modules during the CCM execution (indicated as `mdl.<FUNCTION_NAME>` are repeated for each module in the chain for which the function exists._
