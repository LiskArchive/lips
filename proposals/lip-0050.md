...
def execute(trs: Transaction) -> None:
    ...
    Token.unlock(ADDRESS_LEGACY_RESERVE, MODULE_NAME_LEGACY, Token.getTokenIDLSK(), trs.params.amount)

    Token.transfer(ADDRESS_LEGACY_RESERVE, newAddress, Token.getTokenIDLSK(), trs.params.amount)
...
This command allows migrated legacy validators register the required keys. In particular, this command is used by all migrated validators to register a BLS key. It could also be used to modify an existing BLS key. Transactions executing this command have:
...
def verify(trs: Transaction) -> None:
    ...
    if validatorParams.validators does not contain element with address == validatorAddress:
        raise Exception("No validator account for this address")
```
