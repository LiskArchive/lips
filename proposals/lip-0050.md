...
def execute(trs: Transaction) -> None:
    ...
    Token.unlock(ADDRESS_LEGACY_RESERVE, MODULE_NAME_LEGACY, Token.getMainchainTokenID(), trs.params.amount)

    Token.transfer(ADDRESS_LEGACY_RESERVE, newAddress, Token.getMainchainTokenID(), trs.params.amount)
...
This command allows migrated legacy validators register the required keys. In particular, this command is used by all migrated validators to register a BLS key. This command cannot be used to modify an existing BLS key, as this is forbidden by the Validators module. Transactions executing this command have:
...
- Check if the sum of the `balance` properties of the entries in `genesisBlockAssetObject.accounts` equals `Token.getLockedAmount(ADDRESS_LEGACY_RESERVE, MODULE_NAME_LEGACY, Token.getMainchainTokenID())`.
