...
* For all entries `userEntry` of the `userSubstore` array, the pair value `(userEntry.address, userEntry.tokenID)` must be unique (each pair appears at most once, though a given address can appear multiple times with different token IDs). 
* For any given `(address, tokenID)` pair, the values used as `moduleID` in the `lockedBalances` array are unique.
* For each `tokenID` value such that `tokenID.chainID == CHAIN_ID_ALIAS_NATIVE`, there must exist an entry in  `genesisBlockAssetObject.supplySubstore` corresponding to `tokenID.localID`.
* For all element `supplyEntry` of the `supplySubstore` array, the values given for `supplyEntry.localID` are unique.
* For all `localID`, check that the corresponding `totalSupply` value equals the sum 
   * `amount` in `escrowSubstore` with corresponding `localID`,
   * all amounts in `userSubstore` with corresponding `localID`, available or locked.
...
* For each entry `userEntry` in the `genesisBlockAssetObject.userSubstore`, create an entry in the user substore  with
  ```python
  storeKey = userEntry.address || uint32(userEntry.tokenID.chainID) || uint32(userEntry.tokenID.localID)
...
* Create an entry in the available local ID substore with
  ...
  storeValue = {"nextAvailableLocalID": genesisBlockAssetObject.availableLocalIDSubstore} 
...
Once the module store is initialized, the validity of the state is checked by:
* Check that the total supply is correct by doing
  ```python
  for localID a key of the supply substore:
      corresponding store value = sum of all escrowed, available and locked tokens with this ID
  ```
* Check that the next available local ID is valid by doing
  ```python
  store value of the available local ID > any local ID with initialized supply
  ```
...
genesisTokenStoreSchema = {
    ...
    "properties": {
        ...
        "escrowSubstore": {
            ...
            "items": {
                "type": "object",
                "required": ["escrowChainID", "tokenLocalID", "amount"],
                "properties": {
                    "escrowChainID": {
                        "dataType": "bytes",
                        "fieldNumber": 1
