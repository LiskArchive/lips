...
* Initial checks:
   * Accross all elements of the `userSubstore` array, the pairs `address, tokenID` must be unique (each pair appears at most once, though a given address can appear multiple times with different token IDs). 
   * For each such `address, tokenID` pair, the values used as `moduleID` in the `lockedBalances` array must be unique.
   * Accross all element of the `supplySubstore` array, all values given for `localID` must be unique.
   * Accross all elements of the `escrowSubstore` array, the pairs `escrowChainID, localID` must be unique (each pair appears at most once). 
