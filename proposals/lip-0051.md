...
* For all entries `userEntry` of the `userSubstore` array, the values used for `userEntry.address` are unique. 
* For any given `address`, the values of `token.tokenID` are unique.
* For any given `tokenID`, the values of `lockedBalances.moduleID` are unique.
* For all ` tokenID` value such that `tokenID.chainID == CHAIN_ID_ALIAS_NATIVE`, there must exist an entry in  genesisBlockAssetObject.supplySubstore` corresponding to `tokenID.localID`.
