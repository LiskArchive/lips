...
The choice of supported tokens must abide by two rules: all chains support by default the LSK token and their native tokens, if they have any. Supported non-native tokens can be specified as part of the initial configuration of the Token module at the chain creation. For example:
...
| `CROSS_CHAIN_COMMAND_NAME_TRANSFER` | string | “crossChainTransferCommand” |
...
| `EVENT_NAME_INITIALIZE_TOKEN` | string | “initializeToken” | Used for events emitted during calls to the [initializeToken](#initializetoken-1) function. |
| `EVENT_NAME_INITIALIZE_USER_ACCOUNT` | string | “initializeUserAccount” | Used for events emitted during initialization of user substore entry.|
| `EVENT_NAME_INITIALIZE_ESCROW_ACCOUNT` | string | “initializeEscrowAccount” | Used for events emitted during initialization of escrow substore entry.|
...
| `EVENT_NAME_ALL_TOKENS_SUPPORTED`| string | “allTokensSupported” | Used for the [allTokensSupported](#alltokenssupported) event |
| `EVENT_NAME_ALL_TOKENS_SUPPORT_REMOVED`| string | “allTokensSupportRemoved” | Used for the [allTokensSupportedRemoved](#alltokenssupportedremoved) event |
| `EVENT_NAME_ALL_TOKENS_FROM_CHAIN_SUPPORTED`| string | “allTokensFromChainSupported” | Used for the [allTokensFromChainSupported](#alltokensfromchainsupported) event |
| `EVENT_NAME_ALL_TOKENS_FROM_CHAIN_SUPPORT_REMOVED`| string | “allTokensFromChainSupportRemoved” | Used for the [allTokensFromChainSupportedRemoved](#alltokensfromchainsupportedremoved) event |
| `EVENT_NAME_TOKEN_ID_SUPPORTED` | string | “tokenIDSupported” | Used for the [tokenIDSupported](#tokenidsupported) event |
| `EVENT_NAME_TOKEN_ID_SUPPORT_REMOVED` | string | “tokenIDSupportRemoved”  | Used for the [tokenIDSupportRemoved](#tokenidsupportremoved) event |
| **Result codes** | | | |
...
This section specifies the non-trivial or recommended endpoints of the Token module and does not include all endpoints.

#### getBalances

This function returns available and locked balances for all tokens associated with a given address.
...
This function returns the escrowed amounts and escrowed chain IDs for all escrowed tokens.
