...
For example, for Lisk Mainnet, the chain ID of the Lisk mainchain is `0x00000000` and further the token ID of the native mainchain token is always `0x00000000`. Therefore the LSK token on Mainnet is identified by the token ID `0x0000000000000000`. Note that the Token module has a method [`getMainchainTokenID`](#getMainchainTokenID) to obtain this token ID.
...
def isTokenSupported(tokenID: TokenID) -> bool:
    ...
    if chainID == OWN_CHAIN_ID or tokenID == getMainchainTokenID():
        return True
...
```python
def getMainchainTokenID() -> TokenID:
    # The local ID of the LSK token is 0x00000000.
    return getMainchainID() + b'\0'*4
```

Here [`getMainchainID()`] is a utility function defined in [LIP 0037][lip-0037#getMainchainID].
...
def transferCrossChain(
    ...
    if tokenChainID not in [OWN_CHAIN_ID, receivingChainID, getMainchainID()]:
...
def removeSupport(tokenID: TokenID) -> None:
    ...
    if tokenID = getMainchainTokenID() or chainID == OWN_CHAIN_ID:
...
The following steps are executed as part of the execution of a cross-chain update command, see [LIP 0049][lip-0049] and [LIP 0053][lip-0053] for details.

#### verifyCrossChainMessage

```python
def verifyCrossChainMessage(ccu: Transaction, ccm: CCM) -> None:
    # This is getMainchainTokenID() for channels with the mainchain.
...
def beforeCrossChainMessageForwarding(ccu: Transaction, ccm: CCM) -> None:
    # This should never fail since it is checked in verifyCrossChainMessage.
    messageFeeTokenID = Interoperability.getMessageFeeTokenID(ccm.receivingChainID) # Always getMainchainTokenID().
    if escrowAmount(ccm.sendingChainID, messageFeeTokenID) < ccm.fee:
        emitPersistentEvent(
            module=MODULE_NAME_TOKEN,
    ...
    # Transfer the fee from escrow to escrow. messageFeeTokenID is always getMainchainTokenID().
    escrowAmount(ccm.sendingChainID, messageFeeTokenID) -= ccm.fee
