...
def verify(trs: Transaction):
    ...
    if availableBalance(senderAddress, trsParams.tokenID) < trsParams.amount:
...
```python
def beforeCrossChainCommandExecution(ccu: CCU, ccm: CCM) -> None:
...
```python
def beforeCrossChainMessageForwarding(ccu: CCU, ccm: CCM) -> None:
    # This should never fail since it is checked in verifyCrossChainMessage.
