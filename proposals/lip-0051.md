...
* Check that for each native token the total supply is correct. 
  ...
  totalSupplyCorrect():
      ...
      for (storeKey, storeValue) in user substore:
          ...
          localID = storeKey[20:28] # part of the key corresponding to the local ID of the token
          if chainID == CHAIN_ID_ALIAS_NATIVE:
              ownedSupply[localID] += sum of availableBalance and all locked amounts of storeValue
      ...
      for (storeKey, storeValue) in escrow substore:
          ...
          ownedSupply[localID] += storeValue.amount
      
      recordedSupply = {}
      for (storeKey, storeValue) in supply substore:
          localID = storeKey
          recordedSupply[localID] = storeValue.amount
      
      # check if both dictionary are equal, same keys, same corresponding values
      if recordedSupply == ownedSupply:
          return True
      else:
          return False
  ```
