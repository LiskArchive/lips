...
if (tokenChainID not in [ownChainID, sendingChainID]
    ...
    if (ccm.status == CCM_STATUS_OK
        ...
        interoperability.error(timestamp, ccm, CCM_STATUS_PROTOCOL_VIOLATION)
...
if tokenChainID == ownChainID:
    ...
    if tokenSupported(tokenID) == False:
        if (ccm.fee >= MIN_RETURN_FEE*length(ccm)
            ...
            interoperability.error(timestamp, ccm, CCM_STATUS_TOKEN_NOT_SUPPORTED)
        stop ccm execution
    if ccm.status == CCM_STATUS_OK:
        availableBalance(recipientAddress, tokenID) += amount
    else:
...
* Cross-chain messages that would result in an address `address` with `availableBalance(address, TOKEN_ID_LSK_MAINCHAIN) < MIN_BALANCE` after their execution must be rejected. This is done by calling `interoperability.error(timestamp, ccm, CCM_STATUS_MIN_BALANCE_NOT_REACHED)` on the rejected message `ccm`.
...
transferCrossChain(timestamp,
    ...
    if (chainID not in [CHAIN_ID_ALIAS_NATIVE, receivingChainID]
        and interoperability.isDirectChannelActive(chainID) == False):
        transferCrossChain fails
    ...
    else: # forward the tokens through their native chain
...
forwardMessageFee(ccm):
    fee = ccm.fee
    if (fee < 0
        or getChannel(ccm.sendingChainID).messageFeeTokenID != getChannel(ccm.receivingChainID).messageFeeTokenID):
        forwardMessageFee fails

    feeTokenID = getChannel(ccm.sendingChainID).messageFeeTokenID
...
beforeSendCCM(payFromAddress, ccm):
    ...
    feeTokenID = getChannel(ccm.receivingChainID).messageFeeTokenID
