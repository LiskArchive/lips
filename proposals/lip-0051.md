...
* Initial checks on the properties of `genesisBlockAssetObject`:
  ...
  * The `supportedTokensSubstore` array must adhere to one of the following two mutually exclusive conditions:
    * Have a unique entry with `chainID` of length `0` and no other entries. This indicates that all tokens are supported, i.e., `chainID == ALL_SUPPORTED_TOKENS_KEY`. In this case, the `supportedTokenIds` should be an empty array.
    * Have all entries with `chainID` property of length `CHAIN_ID_LENGTH`. The `supportedTokenIDs` array must be in lexicographic order, and the `supportedTokensSubstore` array must be in lexicographic order of `chainID`.
