...
In particular, the LSK token is native to the Lisk mainchain which has `chainID = 0x 00 00 00 00`, it is also the first (and only) token of this chain and has `localID = 0x 00 00 00 00`. This entails that the LSK token ID is `0x 00 00 00 00 00 00 00 00`. 
...
| `FEE_CCM_INIT_USER_STORE`            | uint64  | 5000000                      |
| `ALL_SUPPORTED_TOKENS_KEY`           | bytes   | 0x 00 00 00 00               |
...
| `USER_SUBSTORE_INITIALIZATION_FEE`      | uint64  | 5000000                      |
| `ESCROW_SUBSTORE_INITIALIZATION_FEE`      | uint64  | 5000000                      |
...
| `EVENT_NAME_INITIALIZE_USER_STORE`   | string  | TBD    | Used for events emitted during initialization of user substore entry.|
| `EVENT_NAME_INITIALIZE_ESCROW_STORE` | string  | TBD    | Used for events emitted during initialization of escrow substore entry.|
...
| `EVENT_NAME_ALL_TOKENS_SUPPORTED_REMOVED`| string  | TBD    | Used for the [allTokensSupportedRemoved](#alltokenssupportedremoved) event  |
...
| `EVENT_NAME_ALL_TOKENS_FROM_CHAIN_SUPPORTED_REMOVED`| string  | TBD    | Used for the [allTokensFromChainSupportedRemoved](#alltokensfromchainsupportedremoved) event  |
...
| `MAX_AVAILABLE_ID_REACHED`           | uint32 | 14 | Used when the initialize Token ID is called and the max value has been reached. |
...
|--------------------|---------|--------------------------------------|---------------------------------------|
| `Address`          | bytes   | Must be of length `ADDRESS_LENGTH`.  | Used for Ed25519 public keys.         |
...
The property `accountInitializationFee` is of interest only in cases where there is no entry `userStore(recipientAddress, tokenID)` and the sender needs to pay a fee for the creation of this entry. In such cases, it should have value `USER_SUBSTORE_INITIALIZATION_FEE`. The token for this fee is `TOKEN_ID_STORE_INITIALIZATION`; by default it is the token used for transaction fees in the chain, i.e., `TOKEN_ID_STORE_INITIALIZATION = TOKEN_ID_FEE`. In case the entry `userStore(recipientAddress, tokenID)` already exists and the sender of the transaction sets a non-zero amount for this property, then sender's account is not debited. 
...
def verify(trs: Transaction):
    ...
    if  userStore(recipientAddress, tokenID) does not exist:
        if trs.params.accountInitializationFee != USER_SUBSTORE_INITIALIZATION_FEE:
            raise Exception("Invalid account initialization fee")
        
        balanceChecks[TOKEN_ID_STORE_INITIALIZATION] = USER_SUBSTORE_INITIALIZATION_FEE
...
```
Here, the [initializeUserStoreInternal](#initializeuserstoreinternal) function creates an entry in the user substore and the [transferInternal](#transferinternal) function debits sender's account and credits recipient's account.
...
The property `escrowInitializationFee` is of interest only in cases where the token sent is native to the chain and there is no entry `escrowStore(receivingChainID, tokenID)`, so the sender needs to pay a fee for the creation of this entry. In such cases, it should have value `ESCROW_SUBSTORE_INITIALIZATION_FEE`. The token for this fee is `TOKEN_ID_STORE_INITIALIZATION`; by default it is the token used for transaction fees in the chain, i.e., `TOKEN_ID_STORE_INITIALIZATION = TOKEN_ID_FEE`. In case the entry `escrowStore(receivingChainID, tokenID)` already exists and the sender of the transaction sets a non-zero amount for this property, then sender's account is not debited.
...
def verify(trs: Transaction) -> None:
    ...
    if tokenChainID == OWN_CHAIN_ID and escrowStore(receivingChainID, tokenID) does not exist:
        if trs.params.escrowInitializationFee != ESCROW_SUBSTORE_INITIALIZATION_FEE:
            raise Exception("Invalid escrow initialization fee")
        
        balanceChecks[TOKEN_ID_STORE_INITIALIZATION] = ESCROW_SUBSTORE_INITIALIZATION_FEE
...
def execute(trs: Transaction) -> None:
    ...
    if tokenChainID == OWN_CHAIN_ID and escrowStore(receivingChainID, tokenID) does not exist:
        initializeEscrowStoreInternal(receivingChainID, tokenID, senderAddress)

    transferCrossChainInternal(senderAddress, tokenID, amount, receivingChainID, recipientAddress, messageFee, data)
```

Here, the [initializeEscrowStoreInternal](#initializeescrowstoreinternal) function creates an entry in the escrow substore and the [transferCrossChainInternal](#transfercrosschaininternal) function debits senders account and calls the interoperability module in order to create a CCM.
...
def execute(ccu: Transaction, ccm: CCM)-> None:
    ...
    if userStore(recipientAddress, tokenID) does not exist:
        if ccm.fee < FEE_CCM_INIT_USER_STORE:
            ...
            burn(relayerAddress, TOKEN_ID_FEE, USER_SUBSTORE_INITIALIZATION_FEE)
...
This event has `name = EVENT_NAME_INITIALIZE_USER_STORE`. This event is emitted when the initialized user store function is called.
...
#### initializeEscrowStore

This event has `name = EVENT_NAME_INITIALIZE_ESCROW_STORE`. This event is emitted when the initialized user store function is called.
...
This event has `name = EVENT_NAME_ALL_TOKENS_SUPPORTED_REMOVED`. This event is emitted when the support to all tokens is removed, e.g., when the function [`removeAllTokensSupport`](#removealltokenssupport) is called.
...
This event has `name = EVENT_NAME_ALL_TOKENS_FROM_CHAIN_SUPPORTED_REMOVED`. This event is emitted when the support of all tokens of a chain is removed, e.g., when the function [`removeAllTokensSupportFromChainID`](#removealltokenssupportfromchainid) is called.
...
#### initializeEscrowStoreInternal

```python
def initializeEscrowStoreInternal(
    chainID: ChainID, 
    tokenID: TokenID, 
    ...
    burn(initPayingAddress, TOKEN_ID_STORE_INITIALIZATION, ESCROW_SUBSTORE_INITIALIZATION_FEE)
    ...
    emitEvent(                                   
        module=MODULE_NAME_TOKEN,
        name=EVENT_NAME_INITIALIZE_ESCROW_STORE,
        data={
            "chainID": chainID,
            "tokenID": tokenID,
            "initPayingAddress": initPayingAddress,
            "initializationFee": ESCROW_SUBSTORE_INITIALIZATION_FEE,
            "result": RESULT_SUCCESSFUL
        },
        topics=[chainID]
    )
...
```python
def initializeUserStoreInternal(
    address: Address, 
    tokenID: TokenID, 
    initPayingAddress: Address
    ) -> None:
    ...
    burn(initPayingAddress, TOKEN_ID_STORE_INITIALIZATION, USER_SUBSTORE_INITIALIZATION_FEE) 
    ...
    emitEvent(
        module=MODULE_NAME_TOKEN,
        name=EVENT_NAME_INITIALIZE_USER_STORE,
        data={
            "address": address,
            "tokenID": tokenID,
            "initPayingAddress": initPayingAddress,
            "initializationFee":USER_SUBSTORE_INITIALIZATION_FEE,
            "result": RESULT_SUCCESSFUL
        },
        topics=[address]
    )
...
def getEscrowedAmount(escrowChainID: ChainID, tokenID: TokenID) -> uint64:
    ...
    if escrowStore(escrowChainID, tokenID) does not exist:
        raise Exception("Escrow store does not exist")
    ...
    if supply substore is non-empty:      # check if the maximum number of native tokens is reached.
        ...
        if intLocalID == 2^(8*LOCAL_ID_LENGTH) - 1:
            emitPersistentEvent(
                ...
                data={
                    "result": MAX_AVAILABLE_ID_REACHED
                },
                topics=[]
            )
...
def mint(
    ...
    if availableBalance(address, tokenID) + amount >= 2^64:
...
def initializeUserStore(
    ...
    if initializationFee != USER_SUBSTORE_INITIALIZATION_FEE:
    ...
    initializeUserStoreInternal(address, tokenID, initPayingAddress)
...
def initializeEscrowStore(
    ...
    # check if initializationFee value is valid. 
    if initializationFee != ESCROW_SUBSTORE_INITIALIZATION_FEE:
    ...
    ###### end of checks #######    
    initializeEscrowStoreInternal(chainID, tokenID, initPayingAddress)
...
def transferCrossChain(
    ...
    if tokenChainID == OWN_CHAIN_ID and escrowStore(tokenChainID, tokenID) does not exist:
    ...
    for tkID in balanceChecks:
        if availableBalance(senderAddress, tkID) < balanceChecks[tkID]:
            emitFailedCrossChainTransferEvent(senderAddress, tokenID, amount, receivingChainID, recipientAddress, RESULT_INSUFFICIENT_BALANCE)
            raise Exception("Insufficient sender available balance")


    # Transfer is only possible for tokens native to either sending, receiving or mainchain
    if tokenChainID not in [OWN_CHAIN_ID, receivingChainID, MAINCHAIN_ID]:
        emitFailedCrossChainTransferEvent(senderAddress, tokenID, amount, receivingChainID, recipientAddress, INVALID_TOKEN_ID)
...
```python
def payMessageFee(payFromAddress: Address, fee: uint64) -> None:
    messageFeeTokenID = Interoperability.getMessageFeeTokenID(receivingChainID)
    ...
    if getChainID(messageFeeTokenID) == OWN_CHAIN_ID:
        escrowAmount(ccm.receivingChainID, messageFeeTokenID) += fee    
...
def beforeCrossChainMessageForwarding(ccu: Transaction, ccm: CCM) -> None:
    ...
    if escrowAmount(ccm.sendingChainID, messageFeeTokenID) < ccm.fee:
        emitFailedCCCForwardEvent(ccm.sendingChainID, ccm.receivingChainID, messageFeeTokenID, ccm.fee, RESULT_INSUFFICIENT_BALANCE) 
        raise Exception("Insufficient balance in the sending chain for the message fee.")
    ...
    if ccm.module == MODULE_NAME_TOKEN and ccm.crossChainCommand == CROSS_CHAIN_COMMAND_NAME_TRANSFER and getChainID(ccm.params.tokenID) == OWN_CHAIN_ID:
        if escrowAmount(ccm.sendingChainID, ccm.params.tokenID) < ccm.params.amount:
            emitFailedCCCForwardEvent(ccm.sendingChainID, ccm.receivingChainID, messageFeeTokenID, ccm.fee, INSUFFICIENT_ESCROW_BALANCE) 
...
def verifyCrossChainMessage(ccu: Transaction, ccm: CCM) -> None:
    ...
    if getChainID(feeTokenID) == OWN_CHAIN_ID:
        if escrowAmount(ccm.sendingChainID, feeTokenID) < fee:
...
This function is called in case where all tokens are supported and we want to remove this support. After calling this function the supported tokens substore becomes empty, therefore the only remaining supported tokens are the ones native to the chain and the LSK token. 

```python
def removeAllTokensSupport() -> None:
    if there does not exist an entry with key = ALL_SUPPORTED_TOKENS_KEY in the supported tokens substore:
        raise Exception('Not all tokens are supported in this chain.')

    remove all entries from the supported tokens substore
    
    emitEvent(
        module=MODULE_NAME_TOKEN,
        name=EVENT_NAME_ALL_TOKENS_SUPPORTED_REMOVED,
        data=EMPTY_BYTES,
        topics=[]
    )
```
...
This function is called in case where all tokens from a specific chain are supported and we want to remove this support. After calling this function no tokens of the specified chain are supported. 
...
def removeAllTokensSupportFromChainID(chainID: ChainID) -> None:
    ...
    if supportedTokens(chainID) does not exist or supportedTokens(chainID).supportedTokenIDs != []:
        raise Exception('Not all tokens for the specified chain are supported')
     
    delete entry supportedTokens(chainID) from the supported tokens substore

    emitEvent(
        module=MODULE_NAME_TOKEN,
        name=EVENT_NAME_ALL_TOKENS_FROM_CHAIN_SUPPORTED_REMOVED,
        data={"chainID": chainID},
        topics=[chainID]
    )
...
def removeSupport(tokenID: TokenID) -> None:
    ...
    if supportedTokens(chainID) exists:
        ...
        if supportedTokens(chainID).supportedTokenIDs is empty:
...
* Initial checks on the properties of `genesisBlockAssetObject`:
  ...
  * The `userSubstore` array must be in lexicographical order of `address`. For a given `address`, the entries must be in ascending order of `tokenID`.
...
* Check that for each native token the total supply is correct.
  ...
  def validateSupplyStoreEntries() -> bool:
      ...
      for (storeKey, storeValue) in escrow substore:
          tokenID = storeKey[:8] # Part of the key corresponding to the token ID of the token
