...
* Initial checks on the properties of `genesisBlockAssetObject`:
  ...
  * The `supportedTokensSubstore` array, if non-empty, must adhere to one of the following two mutually exclusive conditions:
    ...
    * Have all entries unique and with `chainID` property of length `CHAIN_ID_LENGTH`. The `supportedTokenIDs` array must be in lexicographic order, and the `supportedTokensSubstore` array must be in lexicographic order of `chainID`.
