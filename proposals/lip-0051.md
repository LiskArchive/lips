...
* Initial checks on the properties of `genesisBlockAssetObject`:
  ...
  * The `supportedTokensSubstore` array must be in lexicographical order of `chainID` and the `chainID` should be of length `CHAIN_ID_LENGTH` or `0`. For each entry of this array, the `supportedTokenIDs` array should be in lexicographic order.
