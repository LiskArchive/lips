...
| `INCORRECT_RECEIVING_CHAIN` | uint32 | 14 | Used when, during the cross-chain token transfer, the receiving chain is set to be equal to the sending chain. |
...
def transferCrossChain(
    ...
    if receivingChainID == OWN_CHAIN_ID:
        emitFailedCrossChainTransferEvent(senderAddress, tokenID, amount, receivingChainID, recipientAddress, INCORRECT_RECEIVING_CHAIN)
        raise Exception("Receiving chain cannot be the sending chain.")
