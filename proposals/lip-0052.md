...
When an NFT is sent to another chain, the `attributes` properties of the NFT can be modified according to specifications set on the receiving chain. For this reason, custom modules specifying an NFT must also implement the behavior to adopt when an NFT is returned with a modified `attributes` properties.

Once an NFT is transferred back to the native chain, any module can perform custom attributes processing, according to the logic specified in `afterCrossChainCommandExecution`. In particular, a module might update all the attributes to the values received cross-chain, keep the old values, or implement more complex processing.
...
This function is used to destroy NFTs. The NFT will be removed from the NFT substore and cannot be retrieved, except in the case of destroying NFT on a non-native chain: the information about the NFT (e.g., the attributes) will still be available in the corresponding escrow entry of the NFT substore in the native chain.
...
def execute(
    ...
    receivedAttributes = ccm.params.attributesArray
