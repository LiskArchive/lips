...
- Initial checks on the properties of `genesisBlockAssetObject`:
  ...
  - The `supportedNFTsSubstore` array must be ordered lexicographically by `chainID` and the `chainID` should be of length `LENGTH_CHAIN_ID` or `0`. For each entry of this array, the `supportedCollectionIDArray` should be in lexicographical order.
