...
- Initial checks on the properties of `genesisBlockAssetObject`:
  ...
  - The `supportedNFTsSubstore` array must adhere to one of the following two mutually exclusive conditions:
    - Have a unique entry with `chainID` of length `0` and no other entries. This indicates that all NFTs are supported, i.e., `chainID == ALL_SUPPORTED_NFTS_KEY`. In this case, the `supportedCollectionIDArray` should be an empty array.
    - Have all entries with `chainID` property of length `LENGTH_CHAIN_ID`. The `supportedCollectionIDArray` must be in lexicographic order, and the `supportedNFTsSubstore` array must be in lexicographic order of `chainID`.
