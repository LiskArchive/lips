...
| `RECEIVING_CHAIN_NOT_ACTIVE` | uint32 | 16 | Used when, during the cross-chain token transfer, the receiving chain is not active. |
...
def transferCrossChain(
    ...
    if not Interoperability.isChannelActive(receivingChainID):
        emitFailedTransferCrossChainEvent(senderAddress, nftID, amount, receivingChainID, recipientAddress, RECEIVING_CHAIN_NOT_ACTIVE)
        raise Exception("Receiving chain is not active.")    
