...
def execute(
    ...
    if getChainID(nftID) == OWN_CHAIN_ID: # execution on the native chain
        ...
        elif ccm.status <= MAX_RESERVED_ERROR_STATUS: # message is returning to the native chain
        ...
        if ccm.status == CCM_STATUS_OK:
            ...
            createUserEntry(recipientAddress, nftID)
        elif ccm.status <= MAX_RESERVED_ERROR_STATUS: # message is returning to the foreign chain
