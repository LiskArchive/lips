...
```python
def isNFTEscrowed(nftID: NFTID) -> bool:
    return len(getNFTOwner(nftID)) == LENGTH_CHAIN_ID
```
...
```python
def isNFTLocked(nftID: NFTID) -> bool:
    return getLockingModule(nftID) != NFT_NOT_LOCKED
```
