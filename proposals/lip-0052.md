...
- Initial checks on the properties of `genesisBlockAssetObject`:
  ...
  - The `supportedNFTsSubstore` array, if non-empty, must adhere to one of the following two mutually exclusive conditions:
    ...
    - Have all entries unique and with `chainID` property of length `LENGTH_CHAIN_ID`. The `supportedCollectionIDArray` must be in lexicographic order, and the `supportedNFTsSubstore` array must be in lexicographic order of `chainID`.
