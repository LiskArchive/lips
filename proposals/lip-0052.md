...
def getNextAvailableIndex(collectionID: CollectionID) -> uint64:
    collectionNftIDs = sorted([key for key in NFTStore.keys() if key[:LENGTH_CHAIN_ID] == OWN_CHAIN_ID and key[LENGTH_CHAIN_ID:LENGTH_CHAIN_ID+LENGTH_COLLECTION_ID] == collectionID], reverse = True)
    if len(collectionNftIDs) == 0:
        return 0 
    largestKey = collectionNftIDs[0]
    ...
    nextAvailableIndex = largestIndex + 1
