...
def calculateNewActiveValidators(activeValidators: list[ActiveValidator], blsKeysUpdate: list[PublicKeyBLS], bftWeightsUpdate: list[uint64], bftWeightsUpdateBitmap: bytes) -> list[ActiveValidator]:
    ...
    bitmapSize = len(bftWeightsUpdateBitmap)
    # Invert bftWeightsUpdate to be able to poop from the end.
    bftWeightsUpdate = bftWeightsUpdate[::-1]
...
def execute(ccu: Transaction) -> None:
    ...
    if len(ccu.params.blsKeysUpdate) > 0 or len(ccu.params.bftWeightsUpdate) > 0 or ccu.params.bftWeightsUpdateBitmap != EMPTY_BYTES or ccu.params.certificateThreshold != validators(ccu.params.sendingChainID).certificateThreshold:
...
def getActiveValidatorsUpdate(currentValidators: list[ActiveValidator], newValidators: list[ActiveValidator]) -> ActiveValidatorsUpdate:
    ...
    bftWeightsUpdate = [v.bftWeight for validator in validatorsUpdate]
