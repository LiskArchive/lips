...
In the following, we define some accessory functions used to verify cross-chain update commands.
...
def verifyInboxUpdate(ccu: CCU) -> None:
    ...
    messageWitness = ccu.params.inboxUpdate.messageWitness
    if messageWitness != {}:
        # calculateRootFromRightWitness is specified in LIP 0031.
        newInboxRoot = calculateRootFromRightWitness(inboxTree.size, inboxTree.appendPath, messageWitness.siblingHashes)
    ...
    if len(certificateBytes) == 0:
        ...
        proof = {
            siblingHashes: witness.siblingHashes,
            queries: [{
                key: outboxKey,
                value: newInboxRoot,
                bitmap: witness.bitmap
            }],
        }
...
### Mainchain Cross-chain Command Verification
...
### Sidechain Cross-chain Command Verification
...
* For every `ccm` in `inboxUpdate.crossChainMessages`:
  ...
  * If `ccm.receivingChainID == MAINCHAIN_ID`: 
   ...
   * else <code>[apply][lip-0049#apply](ccu, ccm)</code>.
...
* For every `ccm` in `inboxUpdate.crossChainMessages`:
  * If (`ownChainAccount.ID != ccm.receivingChainID` and `ccm.status == CCM_STATUS_OK`) or <code>[validateFormat][lip-0049#validateFormat](ccm) != True</code>:  
  <code>[terminateChainInternal][lip-0045#terminateChainInternal](partnerChainID)</code>.
  * <code>[appendToInboxTree][lip-0045#appendToInboxTree](ccu.sendingChainID, serializedCCM)</code>, where `serializedCCM` is the serialization of `ccm` using [`crossChainMessageSchema`][lip-0049#ccmSchema].
