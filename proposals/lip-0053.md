...
2. **Cross-chain command verification**: The cross-chain command corresponding to the CCM `module`-`crossChainCommand` combination is verified. If an error occurs, the channel with the CCU sending chain is terminated and the CCU execution stops.
