...
The chain account stores an array containing the BLS keys and BFT weights required to validate the certificate signature. This array has to be updated if the validator set changes in the chain sending the certificate. The `newBLSKeys` property contains the BLS keys of new validators.

#### newBFTWeights

The `newBFTWeights` property contains the update to the BFT weights of the chain validators, possibly including the BFT weights of the new validators with BLS keys given in `newBLSKeys`.
...
def verifyValidatorsUpdate(ccu: Transaction) -> None:
    ...
    partnerchainID = ccu.params.sendingChainID
    newBLSKeys = ccu.params.newBLSKeys
    # Validator keys must be in lexicographic order.
    if not all(newBLSKeys[i] < newBLSKeys[i + 1] for i in range(len(newBLSKeys) - 1)):
    ...
    newBFTWeights = ccu.params.newBFTWeights
    if bin(int.from_bytes(bftUpdateBitmap, 'big')).count("1") != len(newBFTWeights):
    ...
    for idx, blsKey in enumerate(allBLSKeys):
        if blsKey in newBLSKeys:
            ...
            digit = (int.from_bytes(bftUpdateBitmap, 'big') >> idx) & 1
            if digit != 1:
                raise Exception("New validators must have a BFT weight update.")
            if newBFTWeights[idx].bftWeight == 0:
                raise Exception("New validators must have a positive BFT weight.")

    # Update validator list.
    newActiveValidators = calculateNewActiveValidators(validators(partnerchainID).activeValidators, newBLSKeys, newBFTWeights, bftUpdateBitmap)
...
```python
def calculateNewActiveValidators(activeValidators: list[ActiveValidator], newBLSKeys: list[PublicKeyBLS], newBFTWeights: list[uint64], bftUpdateBitmap: bytes) -> list[ActiveValidator]:
    currentBLSKeys = [v.blsKey for v in activeValidators]
    newValidators = [{"blsKey": blsKey, "bftWeight": 0} for blsKey in newBLSKeys]
    newActiveValidators = sorted(activeValidators + newValidators, key = lambda v: v.blsKey)

    # Update BFT weights.
    intBitmap = int.from_bytes(bftUpdateBitmap, 'big')
    for idx, val in enumerate(newActiveValidators):
        ...
        if (intBitmap >> idx) & 1 == 1:
            # Get last BFT weight from the list of new BFT weights.
            val.bftWeight = newBFTWeights.pop()

    # No BFT weights should be left.
    assert len(newBFTWeights) == 0
...
def updateValidators(ccu: Transaction) -> None:
    ...
    validators(ccu.params.sendingChainID).activeValidators = calculateNewActiveValidators(
        ...
        ccu.params.newBLSKeys,
        ccu.params.newBFTWeights,
        ccu.params.bftUpdateBitmap
    )
```
...
def verify(ccu: Transaction) -> None:
    ...
    if len(ccu.params.newBLSKeys) > 0 or len(ccu.params.newBFTWeights) > 0 or ccu.params.bftUpdateBitmap != EMPTY_BYTES or ccu.params.certificateThreshold != validators(ccu.params.sendingChainID).certificateThreshold:
    ...
    for ccmBytes in crossChainMessages:
        ...
        try:
            ...
            emitEvent(
                ...
                data = {"ccm": EMPTY_CCM, "result": CCM_PROCESSED_RESULT_DISCARDED, "code": CCM_PROCESSED_CODE_INVALID_CCM_VALIDATION_EXCEPTION},
...
When posting a CCU transaction, the validators hash given in the certificate certifies the new set of validators of the sending chain. The CCU must therefore include the difference between the validators currently stored in the chain account and the validator set authenticated by the certificate. The BLS keys of new validators are contained in the `newBLSKeys` property, while the new BFT weights are contained in the `newBFTWeights` property. These properties can be obtained applying the functions given below.

```python
def getnewBLSKeys(currentValidators: list[ActiveValidator], newValidators: list[ActiveValidator]) -> list[PublicKeyBLS]:
    currentBLSKeys = [v.blsKey for v in currentValidators]
    return sorted([validator.blsKey for validator in newValidators if validator.blsKey not in currentBLSKeys])
```

```python
def getnewBFTWeights(currentValidators: list[ActiveValidator], newValidators: list[ActiveValidator]) -> list[uint64]:
    ...
    for validator in validatorsUpdate:
        newBFTWeights.append(validator.bftWeight)

    return newBFTWeights
```

```python
def getBFTUpdateBitmap(currentValidators: list[ActiveValidator], newValidators: list[ActiveValidator]) -> bytes:
    validatorsUpdate: list[ActiveValidator] = []
    for validator in newValidators:
        # We add the validator to the validatorsUpdate unless an entry with the same BLS key and BFT weight already exists in currentValidators.
        if validator not in currentValidators:
    ...
    validatorsUpdate = sorted(validatorsUpdate, key=lambda v: v.blsKey)
    currentBLSKeys = [v.blsKey for v in currentValidators]
    newBLSKeys = [validator.blsKey for validator in newValidators]
    allBLSKeys = sorted([k for k in set(currentBLSKeys + newBLSKeys)])
    bitmap = 0
    for validator in validatorsUpdate:
        idx = allBLSKeys.index(validator.blsKey)
        bitmap |= 1 << idx

    return bitmap.to_bytes((len(allBLSKeys) + 7) // 8, "big")
```
