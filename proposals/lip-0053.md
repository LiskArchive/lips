...
def verifyPartnerChainOutboxRoot(ccu: Transaction) -> None:
    ...
    if len(ccu.params.certificate) > 0:
        if outboxRootWitness is empty:
            raise Exception("If a certificate is present, the outbox root witness must be non-empty to authenticate the new partnerChainOutboxRoot.")
...
def verify(ccu: Transaction) -> None:
    certificateBytes = ccu.params.certificate
    if len(certificateBytes) == 0 and ccu.params.inboxUpdate is empty: # An empty object has all properties set to their default values.
