...
def verify(trs: Transaction) -> None:
    ...
    if chainAccount(trs.params.sendingChainID).status == CHAIN_STATUS_REGISTERED:
        verifyLivenessConditionForRegisteredChains(trs)
    certificateBytes = trs.params.certificate
    if chainAccount(trs.params.sendingChainID).status == CHAIN_STATUS_REGISTERED and len(certificateBytes) == 0:
        raise Exception("The first CCU must contain a non-empty certificate.")
    if len(certificateBytes) > 0:
        verifyCertificate(trs)
    if len(trs.params.activeValidatorsUpdate) > 0 or trs.params.certificateThreshold != validators(trs.params.sendingChainID).certificateThreshold:
...
def execute(trs: Transaction) -> None:
    ...
    for ccmBytes in crossChainMessages:
        ...
        try:
            ...
            if ccm.status == STATUS_CODE_CHANNEL_UNAVAILABLE:
            ...
            emitEvent(
                ...
                data = {"ccmID": ccmID, "result": CCM_PROCESSED_RESULT_DISCARDED, "code": STATUS_CODE_INVALID_FORMAT},
