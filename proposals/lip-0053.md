...
def verifyValidatorsUpdate(trs: Transaction) -> None:
    if len(trs.params.activeValidatorsUpdate) == 0 and trs.params.newCertificateThreshold == 0:
        return
...
def execute(trs: Transaction) -> None:
    ...
    for idx in range(len(crossChainMessages)):
        ccmBytes = crossChainMessages[idx]
        ccm = decode(crossChainMessageSchema, ccmBytes)

        # For a chain in the CHAIN_STATUS_REGISTERED status, the first message must be the registration message.
        if idx == 0 and sidechainAccount.status == CHAIN_STATUS_REGISTERED:
            if ccm.crossChainCommandID != CROSS_CHAIN_COMMAND_ID_REGISTRATION:
        ...
        # All CCMs must come from the sending chain.
        ...
        # If the receiving chain is the mainchain, process the CCM.
    ...
    if trs.params.newCertificateThreshold > 0:
        validators(sidechainID).certificateThreshold = trs.params.newCertificateThreshold
    else:
...
def execute(trs: Transaction) -> None:
    ...
    for idx in range(len(crossChainMessages)):
        ...
        # All CCMs with status CCM_STATUS_OK must be directed to the sidechain.
        ...
        # Notice that this interrupts the CCU processing also if the sending chain has been terminated
        # by a previous CCM.
        if not isLive(mainchainID):
            return

        # The Token module is called to transfer the fee to the CCU sender.
        # Notice that on a sidechain, this function should not fail,
        # as the message fee token is LSK.
        senderAddress = Address derived from trs.senderPublicKey
        token.assignMessageFee(senderAddress, ccm)
        ...
        # Finally update the inbox
        appendToInboxTree(mainchainID, ccmBytes)
    
    # Update mainchain validators.
    if trs.params.newCertificateThreshold > 0:
        validators(mainchainID).certificateThreshold = trs.params.newCertificateThreshold
    else:
