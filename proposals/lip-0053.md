...
2. **Cross-chain command verification**: The cross-chain command corresponding to the CCM `module`-`crossChainCommand` combination is verified.
...
4. **Cross-chain command execution**: The cross-chain command corresponding to the CCM `module`-`crossChainCommand` combination is executed. 
...
If an error occurs in stages 2 to 5, the cross-chain message is invalid and discarded. All state transitions are reverted and the state is restored to the snapshot before the cross-chain message was processed.
