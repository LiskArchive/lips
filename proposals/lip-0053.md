...
2. **Cross-chain command verification**: The cross-chain command corresponding to the CCM `module`-`crossChainCommand` combination is verified. If an error occurs, the channel with the CCM sending chain is terminated and the CCM execution stops. Note that before verifying the cross-chain command, the parameters are decoded and validated against the schema.
