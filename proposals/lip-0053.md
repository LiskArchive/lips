...
The Interoperability module supports two commands used to certify the state of another chain. Those commands have `moduleID = MODULE_ID_INTEROPERABILITY`. The mainchain cross-chain update, meant to be posted on the mainchain, has `commandID = COMMAND_MAINCHAIN_CCU`, while the sidechain cross-chain update, meant to be posted on sidechains, has `commandID = COMMAND_SIDECHAIN_CCU`.
...
* `moduleID = MODULE_ID_INTEROPERABILITY`,
* `commandID = COMMAND_MAINCHAIN_CCU`.
...
* `moduleID = MODULE_ID_INTEROPERABILITY`,
* `commandID = COMMAND_SIDECHAIN_CCU`.
