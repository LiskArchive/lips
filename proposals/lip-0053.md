...
def verifyValidatorsUpdate(ccu: Transaction) -> None:
    ...
    # New BFT weights must be sorted by blsKeyIndex property.
    newBFTWeights = ccu.params.newBFTWeights
    if newBFTWeights != sorted(newBFTWeights, key = lambda w: w.blsKeyIndex):
        raise Exception("New BFT weights are not sorted by BLS key index.")

    # A BLS key index can only be present once in the BFT weights update.
    newBFTWeightIndeces = [w.blsKeyIndex for w in ccu.params.newBFTWeights]
    if len(newBFTWeightIndeces) != len(set(newBFTWeightIndeces)):
        raise Exception("Duplicate BLS key index in BFT wieghts update.")

    # All new keys must have a positive BFT weight.
    for idx, blsKey in enumerate(allBLSKeys):
        if blsKey in newBLSKeys:
            if idx not in newBFTWeightIndeces:
                raise Exception("New validators must have a BFT weight update.")
            newWeightIndex = newBFTWeightIndeces.index(idx)
            if newBFTWeights[newWeightIndex].bftWeight == 0:
                raise Exception("New validators must have a positive BFT weight.")

    # Update validator list.
    newActiveValidators = calculateNewActiveValidators(validators(partnerchainID).activeValidators, newBLSKeys, newBFTWeights)
...
```python
def calculateNewActiveValidators(activeValidators: list[ActiveValidator], newBLSKeys: list[PublicKeyBLS], newBFTWeights: list[dict[str, int]]) -> list[ActiveValidator]:
    ...
    for w in newBFTWeights:
        newActiveValidators[w.blsKeyIndex].bftWeight = w.bftWeight

    # Remove all validators with 0 BFT weight
    newValidators = [v for v in newValidators if v.bftWeight > 0]
...
crossChainUpdateTransactionParams = {
    ...
    "properties": {
        ...
        "newBFTWeights": {
            ...
            "items": {
                "type": "object",
                "required": ["blsKeyIndex", "bftWeight"],
                "properties": {
                    "blsKeyIndex": {
                        "dataType": "uint32",
                        "fieldNumber": 1
...
def verify(ccu: Transaction) -> None:
    ...
    if len(ccu.params.newBLSKeys) > 0 or len(ccu.params.newBFTWeights) > 0 or ccu.params.certificateThreshold != validators(ccu.params.sendingChainID).certificateThreshold:
...
def getnewBLSKeys(currentValidators: list[ActiveValidator], newValidators: list[ActiveValidator]) -> list[PublicKeyBLS]:
    ...
    newBLSKeys = [blsKey for blsKey in newBLSKeys if blsKey not in currentBLSKeys]

    return newBLSKeys
```

```python
def getnewBFTWeights(currentValidators: list[ActiveValidator], newValidators: list[ActiveValidator]) -> list[dict[str, int]]:
    currentBLSKeys = [v.blsKey for v in currentValidators]
    allBLSKeys = sorted(set(currentBLSKeys + newBLSKeys))
    newBLSKeys = [validator.blsKey for validator in newValidators]
    ...
    for validator in validatorsUpdate:
        ...
        newBFTWeights.append({'blsKeyIndex': idx, 'bftWeight': validator.bftWeight})
