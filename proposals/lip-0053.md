...
```python
def verifyCertificateSignature(trs: Transaction) -> None:
...
```python
def verifyChannel(trs: Transaction) -> None:
    partnerchainID = trs.params.sendingChainID
    ...
    if len(trs.params.certificate) > 0:
        ...
        outboxKey = MODULE_NAME_INTEROPERABILITY + SUBSTORE_PREFIX_OUTBOX_ROOT + SHA-256(partnerchainID)
        proof = {
