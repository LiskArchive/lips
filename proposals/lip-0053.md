...
| `ActiveValidator` | object | Must have `blsKey: BLSKey` and `bftWeight: uint64` properties. | The BLS key and BFT weight of an active validator. |
...
On the mainchain, a CCU transaction is only valid if the sidechain is not terminated and follows the [liveness rule][lip-0045#livenessCondition]. This is done by asserting the two points below:  
...
If `partnerChain.status == CHAIN_REGISTERED`, all proposed CCUs must contain a non-empty `certificate` which must follow the schema defined in  [LIP 0061][lip-0061]. In the following, let `certificate` be the deserialized certificate.
...
```python
timestamp - certificate.timestamp < LIVENESS_LIMIT / 2
```
...
* `params.certificate` follows the schema defined in [LIP 0061][lip-0061]. Again, we write `certificate` for the deserialized certificate.
...
* For every `ccm` in `inboxUpdate.crossChainMessages`:
  ...
  * <code>[appendToInboxTree][lip-0045#appendToInboxTree](ccu.sendingChainID, serializedCCM)</code>, where `serializedCCM` is the serialization of `ccm` using [`crossChainMessageSchema`][lip-0049#ccmSchema].
