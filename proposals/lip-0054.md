...
def verify(trs: Transaction) -> None:
    ...
    if RMTVerify(
        [SHA-256(ccmBytes) for ccmBytes in trs.params.crossChainMessages],
        proof,
...
def execute(trs: Transaction) -> None:
    ...
    for ccm in crossChainMessages:
        ...
        if ccm.receivingChainID == ownChainAccount.ID and (ccm.module, ccm.crossChainCommand) is supported:
        ...
        elif ownChainAccount.ID == CHAIN_ID_MAINCHAIN
...
```python
def verify(trs: Transaction) -> None:
    if trs.params.chainID == CHAIN_ID_MAINCHAIN or trs.params.chainID == ownChainAccount.ID:
    ...
    interopAccKey = MODULE_NAME_INTEROPERABILITY + SUBSTORE_PREFIX_CHAIN_DATA + SHA-256(trs.params.chainID)
...
def verify(trs: Transaction) -> None:
    ...
    channelKey = MODULE_NAME_INTEROPERABILITY + SUBSTORE_PREFIX_CHANNEL_DATA + SHA-256(CHAIN_ID_MAINCHAIN)
