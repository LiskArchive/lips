...
def verify(trs: Transaction) -> None:
    ...
    for ccmBytes in crossChainMessages:
        ...
        validateFormat(ccm)
        # The CCM must come from the current chain.
...
def execute(trs: Transaction) -> None:
    ...
    for ccmBytes in crossChainMessages:
        ...
        set ccmID as default topic to all events emitted in processRecovery insead of trs.id

        ccm = decode(crossChainMessageSchema, ccmBytes)
        processRecovery(trs, ccm)
...
```python
def processRecovery(trs: Transaction, ccm: CCM) -> None:
    ...
    try:
        ...
        emitEvent(
            module = MODULE_NAME_INTEROPERABILITY,
            name = EVENT_NAME_CCM_PROCESSED,
            data = {"ccmID": ccmID, "result": CCM_PROCESSED_RESULT_APPLIED, "code": CCM_PROCESSED_CODE_RECOVERY_SUCCESS},
            topics = [ccm.sendingChainID, ccm.receivingChainID]
        )
    except:
