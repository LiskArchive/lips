...
```python
def verify(trs: Transaction) -> None:
    if trs.params.chainID == getMainchainID() or trs.params.chainID == ownChainAccount.chainID:
    ...
    sidechainAccount = decode(chainDataSchema, trs.params.sidechainAccount)
...
```python
def execute(trs: Transaction) -> None:
    partnerChannel = decode(channelDataSchema, trs.params.channel)
