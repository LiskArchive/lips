...
def execute(trs: Transaction) -> None:
    ...
    stateRoot = decode(chainDataSchema, trsParams.sidechainAccount).lastCertificate.stateRoot
    terminatedStateAccount = terminatedStateAccountSubstore(trsParams.chainID)
    if terminatedStateAccount exists:
        terminatedStateAccount.stateRoot = stateRoot
        terminatedStateAccount.mainchainStateRoot = EMPTY_HASH
        terminatedStateAccount.initialized = True
    else:
        createTerminatedStateAccount(trsParams.chainID, stateRoot)
```
