...
| **Command ID**                                |        |              |
| `COMMAND_ID_STATE_RECOVERY`                   | bytes  | 0x0004       |
| `COMMAND_ID_MESSAGE_RECOVERY`                 | bytes  | 0x0005       |
| `COMMAND_ID_STATE_RECOVERY_INITIALIZATION`    | bytes  | 0x0006       |
| `COMMAND_ID_MESSAGE_RECOVERY_INITIALIZATION`  | bytes  | 0x0007       |
| `CROSS_CHAIN_COMMAND_ID_TRANSFER`             | bytes  | 0x0000       |
| **Other**                                     |        |              |
...
def verify(trs: Transaction) -> None:
    ...
    for entry in trs.params.storeEntries:
        ...
        query = { 
            "key": trs.params.moduleID + entry.entry.storePrefix + entry.storeKey,
...
def execute(trs: Transaction) -> None:
    ...
    for entry in trs.params.storeEntries:
        ...
        recoveryModule = module associated with trs.params.moduleID
        recoveryModule.recover(trs.params.chainID, entry.storePrefix, entry.storeKey, entry.storeValue)
...
```python
def recover(terminatedChainID: ChainID, storePrefix: bytes, storeKey: bytes, storeValue: bytes),
```
...
def execute(trs: Transaction) -> None:
    ...
    for ccm in crossChainMessages:
        ...
        if ccm.receivingChainID == ownChainAccount.ID and (ccm.moduleID, ccm.crossChainCommandID) is supported:
...
* `moduleID = MODULE_ID_INTEROPERABILITY`,
* `commandID = COMMAND_ID_STATE_RECOVERY_INITIALIZATION`.
...
def verify(trs: Transaction) -> None:
    ...
    if sidechainAccount.status != CHAIN_STATUS_TERMINATED
        and chainAccount(MAINCHAIN_ID).lastCertificate.timestamp - sidechainAccount.lastCertificate.timestamp < LIVENESS_LIMIT:
        raise Exception("Sidechain is not terminated.")

    interopAccKey = MODULE_ID_INTEROPERABILITY + STORE_PREFIX_CHAIN_DATA + SHA-256(trs.params.chainID)
    ...
    if terminatedStateAccount(trs.params.chainID) exists:
        ...
        if SMTVerify([interopAccKey], proofOfInclusion, chainAccount(MAINCHAIN_ID).lastCertificate.stateRoot) == False:
            raise Exception("State recovery initialization proof of inclusion is not valid.")
```
...
* `moduleID = MODULE_ID_INTEROPERABILITY`,
* `commandID = COMMAND_ID_MESSAGE_RECOVERY_INITIALIZATION`.
...
def verify(trs: Transaction) -> None:
    ...
    channelKey = MODULE_ID_INTEROPERABILITY + STORE_PREFIX_CHANNEL_DATA + SHA-256(MAINCHAIN_ID)
