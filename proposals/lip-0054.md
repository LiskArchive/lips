...
* `commandID = COMMAND_STATE_RECOVERY`.
...
Processing a transaction `trs` with module ID `MODULE_ID_INTEROPERABILITY` and command ID `COMMAND_STATE_RECOVERY` implies the following logic:
...
* `moduleID = MODULE_ID_INTEROPERABILITY`,
* `commandID = COMMAND_MESSAGE_RECOVERY`.
...
* `moduleID = MODULE_ID_INTEROPERABILITY`,
* `commandID = COMMAND_STATE_RECOVERY_INITIALIZATION`.
...
* `moduleID = MODULE_ID_INTEROPERABILITY`,
* `commandID = COMMAND_MESSAGE_RECOVERY_INITIALIZATION`.
...
def verify(trs: Transaction) -> None:
    ...
    channelKey = MODULE_ID_INTEROPERABILITY + STORE_PREFIX_CHANNEL_DATA + SHA-256(CHAIN_ID_MAINCHAIN)
