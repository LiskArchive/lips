...
def execute(trs: Transaction) -> None:
    ...
    for ccm in crossChainMessages:
        ...
        if ccm.receivingChainID == ownChainAccount.ID and (ccm.moduleID, ccm.crossChainCommandID) is supported:
            eventStatus = CCM_PROCESSED_STATUS_COMMAND_RECOVERY_APPLIED  
            ...
            try: 
                ...
                eventStatus = CCM_PROCESSED_STATUS_COMMAND_RECOVERY_FAILED 
                
        elif ownChainAccount.ID == MAINCHAIN_ID
            and(chainAccount(ccm.receivingChainID) exists
            and chainAccount(ccm.receivingChainID).status == CHAIN_STATUS_ACTIVE
            and isLive(ccm.receivingChainID):
            eventStatus = CCM_PROCESSED_STATUS_COMMAND_RECOVERY_FORWARDED
            addToOutbox(ccm.receivingChainID, ccm)
        else: 
            eventStatus = CCM_PROCESSED_STATUS_COMMAND_RECOVERY_DISCARDED
