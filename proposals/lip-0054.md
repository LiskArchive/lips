...
def verify(trs: Transaction) -> None:
    ...
    timestamp = timestamp of the block containing trs
    if timestamp - sidechainAccount.lastCertificate.timestamp < LIVENESS_LIMIT:
        raise Exception("Sidechain did not violate liveness condition.")
```
...
This command is used to initialize a terminated state account or update the state root stored in it. It can only bu submitted on a sidechain.
