...
The properties `maxHeightPrevoted`, `maxHeightGenerated` and `impliesMaxPrevotes` are related to the [Lisk-BFT protocol][lip-0014#block-header-properties]. They are verified by calling the `verifyBFTProperties` function. This function returns a boolean, indicating the success of the check.
...
def verifyBFTProperties():
  ...
  if bft.isHeaderContradictingChain(block.header):
    return False

  if block.header.impliesMaxPrevotes != bft.impliesMaximalPrevotes(block.header)
    return False
