...
`MODULEID_LENGTH_BYTES`      | integer | 4                     | The length of a module ID.
...
- **Static validation**:
  - Check that each entry in the assets array has `moduleID` with length equal to `MODULEID_LENGTH_BYTES` and set to the ID of a module registered in the chain, while the `data` property has size at most equal to `MAX_ASSET_DATA_SIZE_BYTES`.
