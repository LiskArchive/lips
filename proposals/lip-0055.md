...
In particular, if the error happens after the transaction has been verified and the state transitions happening before the *command execution* have been processed (see the ["Block processing stages"](#block-processing-stages) section below), the transaction is considered to be *failed*. Otherwise, the transaction is *invalid* and it is not included in the block (in a way analogous to the current protocol).
