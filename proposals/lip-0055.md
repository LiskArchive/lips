...
8. **Command verification**: The command corresponding to the `module`/`command` combination is verified. If an error occurs, the transaction is invalid and the whole block is discarded. Note that before verifying the command, the parameters are decoded and validated against the schema.
...
6. **Command verification**: The command corresponding to the `module`/`command` combination is verified. If an error occurs, the transaction is invalid and it is not included in the block. Note that before verifying the command, the parameters are decoded and validated against the schema.
