...
Furthermore, we update the block header schema to include new properties introduced by the ["Define state model and state root"][lip-0040] LIP, the ["Introduce BFT module"][research:bft-module] LIP, and [LIP 0061][lip-0061].
...
- `aggregateCommit`: This property contains the aggregate signature for a certificate for a certain block height. Based on this, any node can create a certificate for the corresponding height. See the [LIP 0061][lip-0061] for more details.
