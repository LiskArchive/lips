...
In the current protocol, a transaction that is included in a block always has its intended consequences. In particular, if an error occurs during its processing as part of the block generation, the transaction is simply invalid and it is not included in the block payload.
...
In particular, if the error happens during the *command execution* (see the ["Block processing stages"](#block-processing-stages) section below), the transaction is considered to be *failed*. Otherwise, the transaction is *invalid* and it is not included in the block (like in the current protocol).
...
9. **Command verification**: The command corresponding to the `moduleID`-`commandID` combination is verified. If an error occurs, the transaction is invalid and it is not included in the block.
...
11. **Command execution**: The command corresponding to the `moduleID`-`commandID` combination is executed. If an error occurs, the transaction is failed and the processing continues.
...
*Figure 2: A schematic depiction of the various stages of a block generation. The steps performed in the generator domain are indicated by blue boxes, while the steps performed in the state-machine domain are indicated by red boxes. Within the state-machine domain, the transactions processing is indicated by purple boxes. The network domain is indicated by green boxes.*
...
7. **Command execution**: The command corresponding to the `moduleID`-`commandID` combination is executed. If an error occurs, the transaction is failed and the processing continues.
