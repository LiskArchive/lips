...
8. **Command verification**: Before verifying the command, the parameters are decoded using `decode` function and validated against the schema using `validateObjectSchema`. Then, the command corresponding to the `module`/`command` combination is verified. If an error occurs either during parameter validation or command verification, the transaction is invalid and the whole block is discarded. 
...
6. **Command verification**: Before verifying the cross-chain command, the parameters are decoded using `decode` function and validated against the schema using `validateObjectSchema`. Then, the command corresponding to the `module`/`command` combination is verified. If an error occurs either during parameter validation or command verification, the transaction is invalid and it is not included in the block. 
