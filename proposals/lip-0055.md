...
Allowing for failing transactions has the advantage that transactions that require a complex and expensive "command execution" stage would still pay the fee in case of errors, as the fee is processed in the "before command execution" stage. 
...
- The application domain checks and applies the module-level logic. All the stages that are part of the application domain (except "command verification" and "command execution") are repeated for each module registered in the chain. The "command verification" and "command execution" stages are called only for the module to which the command belongs (i.e. with module ID matching the `moduleID` property). The transactions processing is performed on the transactions contained in the block as part of the application domain processing. The four stages in the transactions processing are repeated for each transaction.
...
11. **Command execution**: The command corresponding to the `moduleID`-`commandID` combination is executed. If an error occurs, the transaction is failed and all state transitions performed in this stage are reverted. In any case, afterwards the processing continues with the next stage.
...
6. **Before command execution**: Each module can define protocol logic that is processed *before* the command has been executed. If an error occurs, the transaction is invalid and it is not included in the block. All state transitions performed in this stage are reverted.
7. **Command execution**: The command corresponding to the `moduleID`-`commandID` combination is executed. If an error occurs, the transaction is failed and all state transitions performed in this stage are reverted. In any case, afterwards the processing continues with the next stage.
8. **After command execution**: Each module can define protocol logic that is processed *after* the command has been executed. If an error occurs, the transaction is invalid and it is not included in the block. All state transitions induced by the transaction performed up to this stage are reverted.
...
10. **Assets finalization**: Each module can update or insert new information in the block assets *after* the state transitions implied by the block are executed.
...
In this section, we specify in which stages each property is inserted in the block header.

- **Header initialization**:
  - Set the `version`, `timestamp`, `height`, `previousBlockID`, `generatorAddress`, `maxHeightPrevoted`, `maxHeightGenerated` properties.
- **Header finalization**:
  - Set the `transactionRoot`, `assetRoot`, `eventRoot`, `stateRoot`, `validatorsHash`, `aggregateCommit`, and `signature` properties.
