...
- `eventsRoot`: The root of the sparse Merkle tree that is computed from the events emitted during the block processing. See [LIP "Introduce events and event root"][lip-events] LIP for the reason why it needs to be included in a block header.
...
- `aggregateCommit`: This property contains the aggregate signature for a certificate for a certain block height. Based on this, any node can create a certificate for the corresponding height. See [LIP 0061][lip-0061] for more details.
...
In the current protocol, a transaction that is included in a block always has its intended consequences. In particular, if an error occurs during the its processing, the transaction is simply invalid and it is not included in the block payload.

With this LIP, we introduce failing transactions. A failing transaction is one for which an error occurs but which is nevertheless included in a block. 
In particular, if the error happens after the transaction has been verified and the state transitions happening before the command execution have been processed (see the ["Block processing stage"](#block-processing-stages) section, below), the transaction is considered to be *failed*. Otherwise, the transaction is *invalid* and it is not included in the block (in a way analogous to the current protocol).
...
- The state-machine checks and applies the module-level logic. All the steps that are part of the state-machine domain (except "command verification" and "command execution") are repeated for each module registered in the chain. The "command verification" and "command execution" steps are called only for the module to which the command belongs (i.e. with module ID matching the `moduleID` property). The transactions processing is performed on the transactions contained in the block as part of the state-machine processing. The four steps in the transactions processing are repeated for each transaction.
...
8. **Transaction verification**: Each module can define protocol logic that verifies a transaction, possibly by accessing the state store. From this step until the end of the "before command execution" step, if an error occurs the transaction is invalid and the whole block is discarded.
9. **Command verification**: The command corresponding to the `moduleID`-`commandID` combination is verified.
10. **Before command execution**: Each module can define protocol logic that is processed *before* the command has been executed. After this step is completed, if an error occurs in the transaction processing, the transaction is failed (but the block is still valid).
11. **Command execution**: The command corresponding to the `moduleID`-`commandID` combination is executed.
12. **After command execution**: Each module can define protocol logic that is processed *after* the command has been executed.
...
*Figure 2: A schematic depiction of the various stages of a block generation. The steps performed in the consensus domain are indicated by blue boxes, while the steps performed in the state-machine domain are indicated by red boxes. Within the state-machine domain, the transactions processing is indicated by purple boxes. The network domain is indicated by green boxes.*

In this section, we describe the various stages of a block generation.

Similarly to the block processing, the block generation is split between the network, consensus, and state-machine domains.
The full generation of a block is organized as follows.

1. **Header creation**: Block header properties that require access to the state store *before* any state transitions implied by the block are executed are inserted in this stage.
2. **Block initialization**: Block assets properties that require access to the state store *before* any state transitions implied by the block are executed are inserted in this stage.
...
4. **Transaction verification**: Each module can define protocol logic that verifies a transaction, possibly by accessing the state store. From this step until the end of the "before command execution" step, if an error occurs the transaction is invalid and it cannot be included in the block.
5. **Command verification**: The command corresponding to the `moduleID`-`commandID` combination is verified.
6. **Before command execution**: Each module can define protocol logic that is processed *before* the command has been executed. After this step is completed, the transaction is guaranteed to be included in the block payload.
7. **Command execution**: The command corresponding to the `moduleID`-`commandID` combination is executed.
8. **After command execution**: Each module can define protocol logic that is processed *after* the command has been executed.
...
9. **Block sealing**: Block assets properties that require access to the state store *after* any state transitions implied by the block are executed are inserted in this stage.
11. **Result verification**: Block header properties, which require accessing the state store *after* all state transitions implied by the block have been executed, are inserted.
...
The block header signature is calculated using the `calculateBlockSignature` function. This function returns a bytes value with length `SIGNATURE_LENGTH_BYTES`.

```python
calculateBlockSignature():
  generatorKey = validators.getValidatorAccount(block.header.generatorAddress).generatorKey
  let secretKey be the secret key corresponding to the public generatorKey
...
- **Result verification**:
  - Verify the `stateRoot`, `eventsRoot`, and `validatorsHash` properties.
...
The events root is the root of the sparse Merkle tree built from the events emitted during the block processing. It is verified by calling the `verifyEventsRoot` function. This function returns a boolean, indicating the success of the check.

```python
verifyEventsRoot():
  return block.header.eventsRoot == eventsRoot(block.header.height)
```

Here, the function `eventsRoot` calculates the events root of the block with the input height as specified in [LIP "Introduce events and event root"][lip-events].
