...
def getActiveValidators(validatorsTwoRoundsAgo: list[EligibleValidatorObject], roundNumber: uint32) -> list[ValidatorObject]:
    ...
    if roundNumber < initRounds + NUMBER_ACTIVE_VALIDATORS:
        ...
        electedValidatorAddresses = [item.address for item in electedValidators]
        ...
        activeValidators = [{"address": item.address, "bftWeight": ceiling(item.weight, WEIGHT_SCALE_FACTOR) } for item in electedValidators]
        averageElectedValidatorWeight = sum([item.weight for item in electedValidators]) // nbrElectedValidators
        activeValidators+= [{"address": item, "bftWeight": ceiling(averageElectedValidatorWeight, WEIGHT_SCALE_FACTOR)  } for item in chosenInitValidators]
    else:
