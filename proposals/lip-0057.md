...
**Other constatns**                |        |                                                                 |
`MAINNET_V4_SNAPSHOTBLOCK_HEIGHT`  | uint32 | TBD                                                             | The height of the snapshot block used for the mainnet migration from Lisk Core 3 to Lisk Core 4
...
# Lisk mainnet, apply a special rule. This must be done as all validators in 
# this snapshot block do not have a BLS key.
if (b.header.height == MAINNET_V4_SNAPSHOTBLOCK_HEIGHT
    and interoperability.getOwnChainAccount().ID == MAINCHAIN_ID):
