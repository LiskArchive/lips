...
| `TOKEN_ID_DYNAMIC_BLOCK_REWARD` | bytes |`TOKEN_ID_LSK = 0x 00 00 00 00 00 00 00 00`| The [token ID][lip-0051#tokenID] of the token used for block rewards. Specified as part of the [ dynamic block rewards module][lip-dynamic-block-rewards] configuration. |
...
def execute(trs: Transaction) -> None:
    ...
    senderReward = min(DynamicBlockRewards.getDefaultRewardAtHeight(h), Token.getAvailableBalance(punishedAddress,TOKEN_ID_DYNAMIC_BLOCK_REWARD))
