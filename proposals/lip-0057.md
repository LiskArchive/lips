...
Note that further we choose `floor(2/3 * aggregateBFTWeight) + 1` as precommit and certificate threshold for PoS chains, see [LIP 0058][lip-0058] for an explanation of these parameters. Here `aggregateBFTWeight` is the sum of BFT weights of the active validators. This choice of BFT parameters means that the safety and liveness of the BFT consensus protocol is ensured if `<1/3 * aggregateBFTWeight` is malicious (for the case of changing validators the guarantees are slightly weaker). The capping of BFT weights ensures a minimum of decentralization in the validator set, even in the case of a very unbalanced validator weight distribution; for example in the Lisk Mainchain, even if top 3 validators have a huge percentage of total validator weight, they would have at most `0.3 * aggregateBFTWeight <1/3 * aggregateBFTWeight` BFT weight. Similarly, only a set of at least 7 validators could have BFT weight of `>= floor(2/3 * aggregateBFTWeight) + 1` and would therefore be able to generate any certificate. In practice, we expect the capping to only effect very few validators, if any, in particular as long as the reward system incentivizes decentralization and does not lead to concentration of huge staked amounts to few validators (this is the case for both reward modules proposed, see [LIP 0042][lip-0042] and [LIP 0071][lip-0071]).
...
```python
def verify(trs: Transaction) -> None:
    trsParams = decode(unlockTransactionParams, trs.params)
    validateObjectSchema(unlockTransactionParams, trsParams)
```
