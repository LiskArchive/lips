...
def afterTransactionsExecute(b: Block) -> None:
    ...
    validatorList = getActiveValidators(snapshotStore(nextRound), nextRound)
...
def updateSnapshotSubstore(height: uint32) -> None:
    roundNumber = getRoundNumberFromHeight(height+1)
    snapshotRound = roundNumber + 2
    ...
    delete any entries from the snapshot substore snapshotStore(x) for x <= roundNumber - 1
```
