...
* Each store key is `roundNumber.to_bytes(4,'big')`, i.e., the big endian uint32 serialization of `roundNumber`, where `roundNumber` is the number of the round at the end of which the active validators and weights are computed. These values will be used at the end of round `roundNumber + 2` to compute the validator set for round `roundNumber + 3`.
...
def afterTransactionsExecute(b: Block) -> None:
    ...
    currentRound = getRoundNumberFromHeight(height)
    ...
    validatorList = getActiveValidators(snapshotStore(currentRound-2), nextRound)

    # Select standby validators if relevant.
    if nextRound >= initRounds + NUMBER_ACTIVE_VALIDATORS:
        standbyValidators = [validator for validator in snapshotStore(currentRound-2)
...
def updateSnapshotSubstore(height: uint32) -> None:
    ...
    # no need to take a snapshot for the first initRounds - 3
    if currentRound + 3 <= genesisDataStore.initRounds:
        return
    ...
    create an entry in the snapshot substore with
        storeKey = currentRound.to_bytes(4,'big'),
        storeValue = encode(snapshotStoreSchema, snapshotState)
    delete any entries from the snapshot substore snapshotStore(x) for x <= currentRound-3
```
