...
| `COMMAND_NAME_VALIDATOR_REGISTRATION` | string | "validatorRegistration"| The command name of the validator registration transaction. |
...
| `BASE_STAKE_AMOUNT` | uint32 | `10 * (10)^8` | The minimum voting amount. All staked amounts should be multiples of this value. |
...
| `EVENT_NAME_VALIDATOR_STAKED` | string | "stakeValidator" | Used for events related to voting a validator. |
...
| `UnlockObject` | object | Contains 3 elements (`address`, `amount`, `unstakeHeight`) of types `Address`, `uint64` and `uint32` respectively (same as the items in the `pendingUnlocks` array of the [stakerStoreSchema](#json-schema)). | An object containing information regarding unvoting a validator. |
...
* `command = COMMAND_NAME_VALIDATOR_REGISTRATION`.
...
def isUnlockable(unlockObject: UnlockObject, senderAddress: Address, height: uint32) -> bool:
    ...
    if not isPunished(validatorAddress, height):       
        ...
        if height – lastPomHeight < punishmentWindow and lastPomHeight < unlockObject.unstakeHeight + lockingPeriod:
            return False
...
def execute(trs: Transaction) -> None:
    ...
    senderReward = min(POM_REWARD, validatorStore(punishedAddress).selfStakes)
...
def isPunished(address: Address, height: uint32) -> bool:
    ...
    if height <= lastPomHeight + PUNISHMENT_WINDOW_SELF_STAKES:
        return True
...
Returns the total amount locked due to participation in voting (active stakes and pending unlocks) for the given address.
