...
The PoS module handles all aspects of the generator selection, this includes the registration of accounts as validators, the staking process, and potential report of misbehavior.
...
This part of the state store is used to maintain all information regarding the registered validators. It is keyed by address and contains values for the validator name, last generated height, total stake (i.e., the sum of all amounts staked with that validator), misbehavior report heights, and a flag asserting if the validator is banned or not.
...
By setting the BFT weights proportional to validator weight instead of uniform BFT weights for all active validators as done before, we can improve the security guarantees of the BFT consensus protocol. If we neglect the effect of capping, which will likely effect only very few validators by a small percentage, then more than 1/3 of the active validator weight are needed to have more than 1/3 of the BFT weight for an attack of the network. Assuming validators generally have at least 10 % self-staking, this translates to at least 1/3 of the total staked amount for active validators would have to be controlled by the attacker.
...
| `MAX_NUM_BYTES_Q96` | uint32 | 24 | The maximal number of bytes of a serialized fractional number in Q96 format (see [Reward sharing LIP][lip-0070]). |
| **Configurable Constants** | | **Mainchain Value** | |
| `TOKEN_ID_POS` | bytes | `TOKEN_ID_LSK = 0x 00 00 00 00 00 00 00 00` | The [token ID][lip-0051#tokenID] of the token used for staking. |
...
| `MAX_NUMBER_STAKING_SLOTS` | uint32 | `10` | The maximum size of the sentStakes array of a staker substore entry. |
...
| `PUNISHMENT_WINDOW_STAKING` | uint32 | `260,000` | The punishment time for staked tokens with punished validators. |
| `PUNISHMENT_WINDOW_SELF_STAKING` | uint32 | `780,000` | The punishment time for self-staked tokens of punished validators. |
...
| `EVENT_NAME_AMOUNT_STAKED` | string | "stakeValidator" | Used for events related to staking a validator. |
...
* `sentStakes`: stores an array of the current staking information of a user. This array was called `votes` in [LIP 0023][lip-0023].
  Each stake is represented by the address of the validator and the amount of tokens that have been staked with that validator; in case reward sharing is enabled, the stake also includes the [sharing coefficients][lip-0070#efficient-calculation-of-rewards] at the time of staking (or at the last time rewards for this stake were claimed).
...
The following three properties are relevant only in case reward sharing is enabled. See the [Reward sharing LIP][lip-0070] for more details.
...
* `params.stakes` has at most `2 * MAX_NUMBER_STAKING_SLOTS` elements. The reason of choosing this bound on the size is to allow a staker to unstake all current validators (which are at most `MAX_NUMBER_STAKING_SLOTS`) and stake with `MAX_NUMBER_STAKING_SLOTS` new ones in the same transaction.
...
* For all stakes included in `params.stakes`, we have:
  * `amount` value is a multiple of `BASE_STAKING_AMOUNT`, i.e., `amount % BASE_STAKING_AMOUNT == 0`. For the Lisk mainchain, where `BASE_STAKING_AMOUNT = 10^9` and `TOKEN_ID_POS = TOKEN_ID_LSK`, this corresponds to multiples of 10 LSK.
  * `amount != 0`.
...
def execute(trs: Transaction) -> None:
    ...
    for stake in sortedStakes:
        ...
        if stake.amount > 0: # Case of regular stake.
            ...
            if len(stakerStore(senderAddress).sentStakes) > MAX_NUMBER_STAKING_SLOTS:
...
Here, the [`updateValidatorEligibility` function](#updatevalidatoreligibility) updates the eligible validators substore according to the new weight of the validator the user stakes with. The `assignStakeRewards` function is defined in the [reward sharing LIP][lip-0070]. The calls to this function are needed only in case reward sharing is enabled.
...
def isUnlockable(unlockObject: UnlockObject, senderAddress: Address, height: uint32) -> bool:
    ...
    lockingPeriod = LOCKING_PERIOD_SELF_STAKING if validatorAddress == senderAddress else LOCKING_PERIOD_STAKING

    # If validator is not punished, normal locking period for staked/self-staked tokens applies.
    if not isPunished(validatorAddress, height):       
        ...
        punishmentWindow = PUNISHMENT_WINDOW_SELF_STAKING if validatorAddress == senderAddress else PUNISHMENT_WINDOW_STAKING
...
* Initial checks on the properties of `genesisBlockAssetObject`:
  ...
  * For all elements of the `stakers` array:
    ...
    * For each element `sentStake` in the `sentStakes` array, the `sharingCoefficients` must be ordered in lexicographic order of `tokenID`.
    * For each element `sentStake` in the `sentStakes` array, for each item in `sharingCoefficients` array  must be ordered in lexicographic order of `tokenID`.
    * `sentStakes` has size is at most `MAX_NUMBER_STAKING_SLOTS`.
...
def afterTransactionsExecute(b: Block) -> None:
    ...
    eligibleValidators = [
        ...
        eligibleValidatorsStore(key) < height - PUNISHMENT_WINDOW_SELF_STAKING or
