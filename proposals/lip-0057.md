...
def execute(trs: Transaction) -> None:
    ...
    senderReward = min(MISBEHAVIOR_REPORT_REWARD, validatorStore(punishedAddress).selfStake)
...
def isPunished(address: Address, height: uint32) -> bool:
    ...
    if height <= lastMisbehaviorReportHeight + PUNISHMENT_WINDOW_SELF_STAKING:
        return True
