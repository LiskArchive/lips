...
| `STAKE_FAILED_INVALID_UNSTAKE_PARAMETERS`| uint32 | 1 | Used when a stake transaction fails because the unstaked amount exceeds the total stakes sent from staker to validator. |
| `STAKE_FAILED_TOO_MANY_PENDING_UNLOCKS` | uint32 | 2 | Used when a stake transaction fails because it the total number of pending unlocks of staker exceeds `MAX_NUMBER_PENDING_UNLOCKS`. |
| `STAKE_FAILED_TOO_MANY_VALIDATORS` | uint32 | 3 | Used when a stake transaction fails because the total number of validators that the user has staked with exceeds `MAX_NUMBER_STAKING_SLOTS`. |
...
* For all stakes included in `params.stakes`, we have:
  * `validatorAddress` corresponds to a registered validator, i.e., there exists an entry `validatorStore(validatorAddress)` in the validator substore. 
...
#        activeValidators = [{"address": item, "bftWeight": 1 } for item in activeValidatorAddresses]
        ...
        averageElectedValidatorWeight = sum([item.weight for item in validatorsTwoRoundsAgo[:nbrElectedValidators]]) // nbrElectedValidators   
        ...
        activeValidators+= [{"address": item, "bftWeight": ceiling(averageElectedValidatorWeight, WEIGHT_SCALE_FACTOR)  } for item in remainingInitValidators[:nbrInitValidators]]
    else:
