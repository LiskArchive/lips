...
| `WEIGHT_SCALE_FACTOR` | uint32 | 1000 | It determines the factor by which BFT weights are divided. |
...
* Notation: For the rest of this proposal let `voterStore(address)` be the object stored in the voter substore with store key `address`, deserialized as a dictionary.
...
* Notation: For the rest of this proposal let `delegateStore(address)` be the object stored in the delegate substore with store key `address`, deserialized as a dictionary.
...
* Notation: For the rest of this proposal let `nameStore(name)` be the object stored in the name substore with store key `name`, deserialized as a dictionary.
...
* Notation: For the rest of this proposal let `eligibleDelegatesStore(key)` be the object stored in the eligible delegates substore with store key `key`, deserialized as a dictionary.
...
* Notation: For the rest of this proposal let `snapshotStore(roundNumber)` be the object stored in the snapshot substore with store key `roundNumber.to_bytes(4,'big')`, deserialized as a dictionary.
    ...
    for vote in sortedVotes:
        ...
        if vote.amount < 0: # Case of unvote.
            ...
            if len(voterStore(senderAddress).pendingUnlocks) > MAX_NUMBER_PENDING_UNLOCKS:
                emitVoteEvent(senderAddress, votedAddress, vote.amount, VOTE_FAILED_TOO_MANY_PENDING_UNLOCKS)    
        ...
        if vote.amount > 0: # Case of regular vote.
            ...
            if there exists an entry oldVote in voterStore(senderAddress).sentVotes with oldVote.delegateAddress = votedAddress:
                ...
                add {"delegateAddress": votedAddress, "amount": vote.amount,
                     "sharingCoefficients": delegateStore(vote.delegateAddress).sharingCoefficients}
            ...
            if len(voterStore(senderAddress).sentVotes) > MAX_NUMBER_SENT_VOTES:
                emitVoteEvent(senderAddress, votedAddress, vote.amount, VOTE_FAILED_TOO_MANY_SENT_VOTES)
        ...
        emitVoteEvent(senderAddress, votedAddress, vote.amount, VOTE_SUCCESSFUL)
...
def execute(trs: Transaction) -> None:
    ...
    updateDelegateEligibility(punishedAddress, currentWeight)
    ...
    senderReward = min(DynamicBlockRewards.getDefaultRewardAtHeight(h), Token.getAvailableBalance(punishedAddress,TOKEN_ID_REWARD))
...
def unbanDelegate(address: Address)-> None:
    ...
    if len(delegateStore(address).pomHeights) < POM_LIMIT_BANNED and Validators.getValidatorAccount(address).blsKey != INVALID_BLS_KEY:
...
```python
def getDPoSTokenID() -> TokenID:
    return TOKEN_ID_DPOS
```
...
for address a key of the voter substore:
    if there exists an entry in delegate substore with key == address:
        if Token.getLockedAmount(address, MODULE_NAME_DPOS, TOKEN_ID_DPOS) < getLockedVotedAmount(address):
...
[lip-reward-sharing]: https://github.com/LiskHQ/lips-staging/blob/0a48eed84a4f65ae18e77eca046ba5bd996a3cd2/proposals/reward-sharing.md
[lip-reward-sharing#efficient-calculation-of-rewards]: https://github.com/LiskHQ/lips-staging/blob/0a48eed84a4f65ae18e77eca046ba5bd996a3cd2/proposals/reward-sharing.md#efficient-calculation-of-rewards
