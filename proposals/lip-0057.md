...
This part of the state store is used to maintain all information regarding the registered validators. It is keyed by address and contains values for the validator name, last generated height, total stakes received, misbehavior report heights, and a flag asserting if the validator is banned or not.
...
| `FACTOR_SELF_STAKES` | uint32 | `10` | The factor multiplying the self-staked amount of a validator for the validator weight computation. |
...
| `LOCKING_PERIOD_STAKING` | uint32 | `26,000` | The [locking period][lip-0023#explicit-unlock-mechanism] for regular stakes. |
| `LOCKING_PERIOD_SELF_STAKING` | uint32 | `260,000` | The [locking period][lip-0023#explicit-unlock-mechanism] for self-stakes. |
| `PUNISHMENT_WINDOW_STAKING` | uint32 | `260,000` | The punishment time for stakes on punished validators. |
| `PUNISHMENT_WINDOW_SELF_STAKING` | uint32 | `780,000` | The punishment time for self-stakes of punished validators. |
...
* `sentStakes`: stores an array of the current stakes of a user. This array was called `votes` in [LIP 0023][lip-0023].
...
Here, the [`updateValidatorEligibility` function](#updatevalidatoreligibility) updates the eligible validators substore according to the new weight of the staked validator. The `assignStakeRewards` function is defined in the [reward sharing LIP][lip-0070]. The calls to this function are needed only in case reward sharing is enabled.
...
In [LIP 0023][lip-0023] the [locking period][lip-0023#locking-period] for unstaked amounts was set to roughly 5.5 hours (2000 blocks) for the lisk mainchain. The sole protocol change in this LIP is to change this value to roughly 3 days, by setting `LOCKING_PERIOD_STAKING` to 26.000 blocks.    
