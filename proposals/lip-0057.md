...
The bootstrap period, defined in [LIP 0034][lip-0034#bootstrap-period] is the initial period when the PoS validator selection mechanism is not active and blocks are generated by a fixed set of validators (specified in genesis block), called initial validators. This period should be long enough to prevent long range attacks, as explained in [LIP 0034][lip-0034#bootstrap-period]. In LIP 0034, we assumed that after the bootstrap phase ends, then all validators are selected according to the PoS selection mechanism. One drawback of this approach is that after the end of the bootstrap period, the change of validators might be too abrupt, possibly leading to breaking the BFT security assumptions and finalizing contradicting blocks or inability of the chain to produce certificates. To avoid those issues, in LIP 34 we just did not allow blocks to be finalized (and therefore produce certificates) during the bootstrap period. This approach has some drawbacks, for example, it does not allow sidechains to be interoperable during their bootstrap phase, which is not desirable.

To circumvent the aforementioned issues and allow chains to produce certificates at any time, after the bootstrap period we introduce a hybrid period. During the hybrid period, the set of validators is gradually shifting from initial validators to POS-selected validators. In particular, in each round we decrease the number of initial validators by 1 and increase the number of validators selected from the PoS selection; therefore the hybrid phase lasts for `NUMBER_ACTIVE_VALIDATORS - 1` rounds. This way, the change of validators between subsequent rounds will not be too abrupt and the sidechains would be able to finalize blocks and produce certificates during both bootstrap and hybrid periods without additional security risks. 
...
| `MIN_INIT_ROUNDS` | uint32 | `3` | The minimum number of rounds for the [bootstrap period](#bootstrap-and-hybrid-period). |
...
| `LOCKING_PERIOD_STAKING` | uint32 | 25,920 = 3 * 24 * 3600 // BLOCK_TIME | The [locking period][lip-0023#explicit-unlock-mechanism] for regular staked tokens, measured in number of blocks. |
