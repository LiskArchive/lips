...
def afterTransactionsExecute(b: Block) -> None:
    ...
    if roundNumber(height+1) <= genesisDataStore.initRounds:
        return

    updateSnapshotSubstore(height)

    # Calculate the active validators for next round based on the snapshot taken two rounds ago.
    validatorList = getActiveValidators(snapshotStore(roundNumber(height)-2), roundNumber(height+1))

    # Select standby validators if relevant.
    if roundNumber(height+1) >= initRounds + NUMBER_ACTIVE_VALIDATORS:
        standbyValidators = [validator for validator in snapshotStore(roundNumber(height)-2)
...
def updateProductivity(b.header) -> None:
...
def updateSnapshotSubstore(height)-> None:
    ...
    create an entry in the snapshot substore with
        storeKey = roundNumber(height).to_bytes(4,'big'),
        storeValue = encode(snapshotStoreSchema, snapshotState)
    delete any entries from the snapshot substore snapshotStore(x) for x <= roundNumber(height)-3 # check definition of snapshot substore



def shuffleAndSetValidators(validatorList, height) -> None:
