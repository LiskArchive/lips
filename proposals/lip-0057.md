...
In this LIP, we set the BFT weights of active delegates to be proportional to their delegate weight. Additionally, to avoid a large concentration of BFT weight in one or only very few delegates, we introduce a capping threshold for the BFT weight, i.e., we do not allow one delegate to get more than a certain percentage of the total BFT weight. This threshold is specified by the constant `MAX_BFT_WEIGHT_CAP`; for the Lisk mainchain, it is set to 5 %.
...
| `POM_REWARD` | uint32 | `10^8` | The reward (in `TOKEN_ID_DPOS`) for a sender of a proof-of-misbehavior transaction.   |
...
def execute(trs: Transaction) -> None:
    ...
    Token.unlock(punishedAddress, MODULE_NAME_DPOS, TOKEN_ID_DPOS, senderReward)
