...
def getActiveValidators(validatorsTwoRoundsAgo: list[EligibleValidatorObject], roundNumber: uint32) -> list[ValidatorObject]:
    ...
    if roundNumber < initRounds + NUMBER_ACTIVE_VALIDATORS:
        ...
        averageElectedValidatorWeight = sum([item.weight for item in activeValidators]) // nbrElectedValidators
        activeValidators+= [{"address": item, "bftWeight": averageElectedValidatorWeight} for item in chosenInitValidators]
    else:
