...
def updateSnapshotSubstore(height: uint32)-> None:
    ...
    create an entry in the snapshot substore with
        storeKey = getRoundNumberFromHeight(height).to_bytes(4,'big'),
        storeValue = encode(snapshotStoreSchema, snapshotState)
    delete any entries from the snapshot substore snapshotStore(x) for x <= getRoundNumberFromHeight(height)-3
```
