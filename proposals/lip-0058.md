...
The Lisk-BFT consensus protocol introduced in [LIP 0014](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0014.md) specifies two new block properties, `maxHeightGenerated` (renamed from `maxHeightPreviouslyForged`) and `maxHeightPrevoted`, which are validated as part of the block processing. As part of the [new state architecture][lip-0069] and stricter separation between state machine layer and replication layer, the majority of the block processing now happens in the state machine layer and is logically separated into separate modules. The consensus-related logic and verification of the block header properties is performed in the replication layer, however. That is why the consensus participants, their BFT weights, keys and the current state of consensus votes is also maintained as part of the replication layer. In this LIP we therefore define the BFT store for this purpose and specify how this information is updated as part of the block processing. Additionally, we define the concept of block slots and the logic for determining which validator is assigned to a specific block slot.
...
We call the first four inputs above the **BFT parameters**. We assume that these BFT parameters are forwarded from the state machine layer to the replication layer whenever the [setValidatorParams][lip-0044#setValidatorParams] function in the Validators module of the state machine layer is called. For a DPoS chain, the DPoS module would need to call this function with the correct parameters, in particular, notifying the replication layer about any changes of validators. Similarly, for a PoA chain, it is the responsibility of the PoA module to provide the correct parameters as input to `setValidatorParams` function. The block header information can be obtained during the block processing in the replication layer.

All the input parameters above, except the block headers, are stored in the BFT Parameters substore. We use the height at which the parameters are first valid as key. This means that if `h1` and `h2` are two keys in the BFT Parameters substore with `h1 < h2` and there is no key between `h1` and `h2`, then the BFT parameters for the key `h1` are valid at heights `h1, ..., h2-1`. The BFT substore is therefore agnostic of the concept of rounds and the frequency at which these parameters are updated.
...
|-------------------------------|----------|------------------------|----------------------------------------------------------------------------|
| `SUBSTORE_PREFIX_BFT_PARAMETERS` |`bytes`   | `0x0000`               | The store prefix of the BFT Parameters substore.  |
| `SUBSTORE_PREFIX_BFT_VOTES`   |`bytes`   | `0x4000`               | The store prefix of the BFT Votes substore.                                |
| `SUBSTORE_PREFIX_GENESIS_DATA`| bytes | `0x8000` | Substore prefix of the genesis data substore. |
...
| `ED25519_PUBLIC_KEY_LENGTH`   |`uint32`  | 32                     | Length in bytes of a Ed25519 public keys.                                  |
| `BLS_PUBLIC_KEY_LENGTH`       |`uint32`  | 48                     | Length in bytes of BLS public key.                                         |
...
The BFT store is organized in the three substores described below. Note that the BFT Store is not part of the state store of the state machine and therefore does not impact the state root.
...
* Notation: We let `bftParameters(height)` denote the object stored in the BFT store for the store prefix `SUBSTORE_PREFIX_BFT_PARAMETERS` and store key `uint32be(height)`.
...
* Notation: We let `bftVotes` denote the object stored in the BFT store for the store prefix `SUBSTORE_PREFIX_BFT_VOTES` and store key equal to empty bytes.
...
* The store value is set to a data structure holding the timestamp of the genesis block of the chain, serialized using the `genesisDataSchema` schema presented below.
...
```python
def getGeneratorAtTimestamp(timestamp):
...
For an address and height as input, the function returns the validator information of that validator at the given height. The function raises an exception if the validator is not in the validator set at that height.

```python
def getValidator(address, height):
    bftParams = getBFTParameters(height)
    for val in bft.validators:
...
This function allows to set the thresholds, validators and associated BFT weights to be used from the next height onward and updates the BFT module store accordingly. The function should be called with the parameters forwarded to the application layer from the state machine every time the function [setValidatorParams][lip-0044#setValidatorParams] of the Validators module is called.
...
As part of the genesis state initialization of a genesis block `b`, the BFT store is initialized as follows:
...
* The Genesis Data substore is initialized by setting `genesisData.timestamp = h.header.timestamp`.

#### Genesis State Finalization

In this step, the function [setValidatorParams][lip-0044#setValidatorParams] of the Validators module should be called by the PoA module or DPoS module.
These parameters are forwarded to the replication layer and the BFT parameters store is updated accordingly using the function `setBFTParameters` defined above.
...
During the block creation process, the properties `maxHeightPrevoted`, `maxHeightGenerated` and `validatorsHash` in the block header related to the BFT protocol (see also [LIP 0055][lip-0055]) have to be set as follows:

* The value of `maxHeightPrevoted` is obtained by calling `getBFTHeights().maxHeightPrevoted` before executing any of the state transitions defined by the BFT protocol in the section "Block Processing" above.
