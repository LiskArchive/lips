...
The BFT store is organized in the two substores described below. Note that the BFT Store is not part of the state store of the state machine and therefore does not impact the state root.
...
* Notation: We let `bftParameters(height)` denote the object stored in the BFT store for the store prefix `STORE_PREFIX_BFT_PARAMETERS` and store key `uint32be(height)`.
...
* Notation: We let `bftVotes` denote the object stored in the BFT store for the store prefix `STORE_PREFIX_BFT_VOTES` and store key equal to empty bytes.
