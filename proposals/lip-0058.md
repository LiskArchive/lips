...
This LIP defines the BFT store and the block processing logic related to the Lisk-BFT consensus protocol. The BFT store is responsible for maintaining the consensus participants, their BFT weights and all information related to the consensus stakes that have been cast as part of the block headers. In this LIP, we specify how this information is serialized and stored as well as updated as part of the block processing.
...
* Functions that allow to verify the Lisk-BFT related properties in the block header, namely `validatorsHash`, `maxHeightGenerated`, `maxHeightPrevoted` and `impliesMaximalPrevotes`, see also [LIP 0055][lip-0055].
...
| `SUBSTORE_PREFIX_BFT_STAKES` |`bytes` | `0x4000` | The substore prefix of the BFT Stakes substore. |
...
| `MAX_LENGTH_BLOCK_BFT_INFOS` |`uint32` | `3*LSK_BFT_BATCH_SIZE` | The maximum length of the array `blockBFTInfos` in the BFT Stakes substore. |
...
* Notation: We let `bftParameters(height)` denote the object stored in the BFT store for the substore prefix `SUBSTORE_PREFIX_BFT_PARAMETERS` and store key `uint32be(height)`.
...
* The substore prefix is `SUBSTORE_PREFIX_BFT_STAKES`.
* The store key is empty bytes.
* The store value is the serialization of an object following the JSON schema `bftStakesSchema` defined below.
* Notation: We let `bftStakes` denote the object stored in the BFT store for the substore prefix `SUBSTORE_PREFIX_BFT_STAKES` and store key equal to empty bytes.
...
* `h`: Height for which to obtain the BFT parameters. Note that for `h <= bftStakes.maxHeightCertified` the function may fail as among all key-value pairs in the BFT Parameters substore with key less or equal to `bftStakes.maxHeightCertified+1` only the one with largest key is kept and all others are pruned.
...
Object following the schema of the objects in the array `bftStakes.blockBFTInfos`.

##### Execution

```python
def getBlockBFTProperties(blockHeader):
    blockBFTInfo = object following schema of objects in bftStakes.blockBFTInfos
...
def getCurrentHeight():
    if length(bftStakes.blockBFTInfos) > 0:
        return bftStakes.blockBFTInfos[0].height
    else:
        # Only directly after processing the genesis block we have length(bftStakes.blockBFTInfos) == 0.
...
def setBFTParameters(precommitThreshold, certificateThreshold, validatorList):
    ...
    # Update bftStakes.updateActiveValidatorsStakeInfo.
    newActiveValidatorsStakeInfo = []
    for each validator in validatorList:
        if validator.address appears in bftStakes.activeValidatorsStakeInfo:
            validatorStakeInfo = object in bftStakes.activeValidatorsStakeInfo
                                with address equal to validator.address
        else:
            validatorStakeInfo = object following schema of elements
                                in bftStakes.activeValidatorsStakeInfo
            validatorStakeInfo.address = validator.address
            validatorStakeInfo.minHeightActive = nextHeight
            validatorStakeInfo.largestHeightPrecommit = nextHeight-1
        add validatorStakeInfo to newActiveValidatorsStakeInfo
    sort newActiveValidatorsStakeInfo lexicographically by address
    bftStakes.activeValidatorsStakeInfo = newActiveValidatorsStakeInfo
```
...
The function returns whether the block header given as input implies the maximal number of prevotes which shows that the block generator is constructively contributing in the Lisk-BFT protocol. This function needs to be called before the execution of the block with the block header `blockHeader` given as input, as it requires `blockHeader.height` to be one larger than the maximum height stored in `bftStakes.blockBFTInfos`.
...
def impliesMaximalPrevotes(blockHeader):
    ...
    if len(bftStakes.blockBFTInfos) == 0:
        if blockHeader.height > blockHeader.maxHeightGenerated:
            return True
        else:
            return False
    heightCurrentTip = bftStakes.blockBFTInfos[0].height
    ...
    if offset >= length(bftStakes.blockBFTInfos):
        return True
    ...
    if bftStakes.blockBFTInfos[offset].generatorAddress != blockHeader.generatorAddress:
        return False
    return True
```
...
def isHeaderContradictingChain(blockHeader):
    for blockBFTInfo in bftStakes.blockBFTInfos:
...
The function returns the current values of the property `maxHeightPrevoted`, `maxHeightPrecommitted` and `maxHeightCertified` in the BFT Stakes substore.
...
def getBFTHeights():
    return {
        "maxHeightPrevoted": bftStakes.maxHeightPrevoted,
        "maxHeightPrecommitted": bftStakes.maxHeightPrecommitted,
        "maxHeightCertified": bftStakes.maxHeightCertified
    }
```
...
* The BFT Stakes substore is initialized as follows:
  * `bftStakes.maxHeightPrevoted = b.header.height`,
  * `bftStakes.maxHeightPrecommitted = b.header.height`,
  * `bftStakes.maxHeightCertified = b.header.height`,
  * `bftStakes.blockBFTInfos` is an empty array,
  * `bftStakes.activeValidatorsStakeInfo` is an empty array.
...
1. The object `getBlockBFTProperties(b.header)` is added to the beginning of the array `bftStakes.blockBFTInfos`.
2. If `bftStakes.blockBFTInfos[MAX_LENGTH_BLOCK_BFT_INFOS]` exists, then the corresponding value is removed from the array.
3. The function `updatePrevotesPrecommits` from [LIP 0056][lip-0056] is executed to update the prevote and precommit weights.
4. The value of `bftStakes.maxHeightPrevoted` is updated by calling the function `updateMaxHeightPrevoted` specified in [LIP 0056][lip-0056].
5. The value of `bftStakes.maxHeightPrecommitted` is updated by calling the function `updateMaxHeightPrecommitted` specified in [LIP 0056][lip-0056].
6. Let `m` be the aggregate commit included in block `b`. If `m.aggregationBits` is empty bytes and `m.signature` is empty bytes, do nothing. Otherwise, set `bftStakes.maxHeightCertified` to `m.height`.
7. Let `h` be the largest integer such that `h <= min(bftStakes.maxHeightCertified+1, bftStakes.blockBFTInfos[-1].height)` and there is an entry in the BFT Parameters store with key `h`. Then remove any key-value pair from BFT Parameters with a key strictly smaller than `h`.
