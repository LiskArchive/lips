...
The BFT store is organized in the three substores described below. Note that the BFT store is not part of the state store of the application domain and therefore does not impact the state root. Each substore of the BFT store is a simple key-value store similar to the substores in the application domain.
...
* Notation: For the rest of this proposal let `genesisDataStore` be the entry in the Genesis Data substore.
...
def getCurrentHeight():
    if length(bftVotes.blockBFTInfos) > 0:
        ...
        return genesisDataStore.height
```
...
This function returns the address of the generator active at the timestamp `timestamp` for the block at height `currentHeight` where `currentHeight` where `timestamp` and `currentHeight` are given as input. Notice that if the input timestamp corresponds to a time slot before or after the current round, this function may not return the correct generator address.

```python
def getGeneratorAtTimestamp(timestamp, currentHeight):
    elapsedTime = timestamp - genesisDataStore.timestamp
...
def getSlotNumber(timestamp):
    if timestamp < genesisDataStore.timestamp:
