...
#### getBFTParametersActiveValidatorsInternal

This function is same as `getBFTParametersInternal`, with the only difference that it provides information only for validators with positive BFT weight.

##### Execution

```python
def getBFTParametersActiveValidatorsInternal(h):
    for key, value in BFT Parameters substore ordered
        decreasing by key:
        if key <= h:
...
def getBFTParametersActiveValidators(h):
    return getBFTParametersActiveValidatorsInternal(h)
```
