...
| `MIN_CERTIFICATE_HEIGHT`| configurable per chain; for sidechains built with Lisk SDK, set to `genesisHeight` (defined in [LIP 0058][lip-0058#notation-and-functions]). For the mainchain in the Lisk Core v4 migration, set to be the first height for which all active validators are selected by PoS module based on their stake, (i.e., there is no validator selected due to being an initial validator specified in genesis data). | The smallest height for which a certificate can be produced.|
...
def selectAggregateCommit() -> AggregateCommit:
    try:
        ...
        nextHeight = min(max(heightNextBFTParameters -1, MIN_CERTIFICATE_HEIGHT), getBFTHeights().maxHeightPrecommitted)
    except:  # getNextHeightBFTParameters function call returns error, i.e., there are no new bft params since last certificate.
...
def verifyAggregateCommit(blockHeader: BlockHeader)-> bool:
    ...
    if previous function call returns valid height
     and heightNextBFTParameters > MIN_CERTIFICATE_HEIGHT
     and aggregateCommit.height > heightNextBFTParameters-1:
        return False
...
[lip-0058#notation-and-functions]: https://github.com/LiskHQ/lips/blob/main/proposals/lip-0058.md#notation-and-functions
