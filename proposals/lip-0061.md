...
| `MIN_CERTIFICATE_HEIGHT`| configurable per chain; for sidechains built with Lisk SDK, set to `genesisHeight` (defined in [LIP 0058][lip-0058#notation-and-functions]). For the mainchain in the Lisk Core v4 migration, set to be the first block all active validators are selected by PoS module based on their stake, i.e., the initial validators included in genesis data have no effect anymore. | The smallest height for which a certificate can be produced.|
