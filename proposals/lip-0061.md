...
| `MIN_CERTIFICATE_HEIGHT`| configurable per chain; for sidechains built with Lisk SDK, set to `genesisHeight`. For the mainchain in the Lisk Core v4 migration, set to be the beginning of the first round where all validators are selected using based on stake, i.e., no more initial validators are selected. This value is `(POS_INIT_ROUNDS + 100) * LSK_BFT_BATCH_SIZE + 1= 6239741`.  | The smallest height for which a certificate can be produced.|
