...
| `MIN_CERTIFICATE_HEIGHT`| configurable per chain; for sidechains built with Lisk SDK, set to `genesisHeight +1` (defined in [LIP 0058][lip-0058#notation-and-functions]). For the mainchain in the Lisk Core v4 migration, set to be the first height for which all active validators are selected by PoS module based on their stake, (i.e., there is no validator selected due to being an initial validator specified in genesis data). | The smallest height for which a certificate can be produced.|
