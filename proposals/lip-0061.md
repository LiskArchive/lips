...
Here, `MAX_LENGTH_AGGREGATION_BITS` is calculated as `ceiling(MAX_NUM_VALIDATORS, 8)`, where `MAX_NUM_VALIDATORS` is a constant defined in [LIP 0045][lip-0045#notation-and-constants], and the auxiliary function `ceiling` is defined by
...
The `...` notation, borrowed from [JavaScript ES6 data destructuring](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Destructuring_assignment#Object_destructuring), indicates that the corresponding part of the schema should be inserted in place, and it is just used for notational convenience.
