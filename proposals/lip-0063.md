...
Note that if the migration is tested for a different network, e.g., Betanet or Testnet, the first byte of the constants `CHAIN_ID_MAINCHAIN` and `TOKEN_ID_LSK` need to be changed to the appropriate chain-identifier prefix, see [LIP37](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0037.md#chain-identifiers-1) for details.
