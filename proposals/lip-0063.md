...
def createUserSubstoreArray():
    ...
    for every account in accounts:
        # overwrite the legacy reserve account if it exists
    ...
    # append the legacy reserve account
    userSubstore.append(createLegacyReserveAccount(ADDRESS_LEGACY_RESERVE))
    sort userSubstore in the ascending order of (userObj.address + userObj.tokenID)
    return userSubstore

def createLegacyReserveAccount(address):
    legacyReserve = {}
    legacyReserve.address = address
    legacyReserve.tokenID = TOKEN_ID_LSK_MAINCHAIN
    legacyReserve.availableBalance = 0
    legacyReserveAmount = 0
    for every account in legacyAccounts:
        legacyReserveAmount += account.token.balance
    legacyReserve.lockedBalances = [{"moduleID": MODULE_ID_LEGACY, "amount": legacyReserveAmount}]
