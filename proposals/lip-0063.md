...
<table>
  ...
  <tr>
   ...
   <td>A dummy value for a <a href="https://github.com/LiskHQ/lips/blob/main/proposals/lip-0038.md#public-key-registration-and-proof-of-possession">proof of possession</a>. The PoS module requires that each delegate account contained in a snapshot/genesis block has such an entry. However, the proofs of possession are not verified for this particular snapshot block.
<p>
...
8. [Dynamic Block Reward][research:dynamic-block-reward-module]
...
| [LIP 0055](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0055.md#notation-and-constants) | `CHAIN_ID` | `0x 00 00 00 00` |
...
| [PoS Module](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0057.md#notation-and-constants) | `FACTOR_SELF_VOTES` | 10 |
| [PoS Module](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0057.md#notation-and-constants) | `BASE_VOTE_AMOUNT` | `10 * (10)^8` |
| [PoS Module](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0057.md#notation-and-constants) | `MAX_LENGTH_NAME` | 20 |
| [PoS Module](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0057.md#notation-and-constants) | `MAX_NUMBER_SENT_VOTES` | 10 |
...
| [PoS Module](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0057.md#notation-and-constants) | `LOCKING_PERIOD_VOTES` | 26,000 |
| [PoS Module](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0057.md#notation-and-constants) | `LOCKING_PERIOD_SELF_VOTES` | 260,000 |
| [PoS Module](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0057.md#notation-and-constants) | `PUNISHMENT_WINDOW_VOTES` | 260,000 |
| [PoS Module](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0057.md#notation-and-constants) | `PUNISHMENT_WINDOW_SELF_VOTES` | 780,000 |
| [PoS Module](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0057.md#notation-and-constants) | `POM_LIMIT_BANNED` | 5 |
...
| [PoS Module](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0057.md#notation-and-constants) | `NUMBER_ACTIVE_DELEGATES` | 101 |
| [PoS Module](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0057.md#notation-and-constants) | `NUMBER_STANDBY_DELEGATES` | 2 |
| [PoS Module](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0057.md#notation-and-constants) | `TOKEN_ID_POS` | `TOKEN_ID_LSK` |
| [PoS Module](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0057.md#notation-and-constants) | `DELEGATE_REGISTRATION_FEE` | 10 x (10^8) |
...
def getLockedBalances(account):
    amount = 0
    for vote in account.pos.sentVotes:
        amount += vote.amount
...
def addPoSModuleEntry():
    ...
    PoSObj.voters = createVotersArray()
...
def createValidatorsArray():
    ...
    for every account in accounts:
        if account.pos.delegate.username == "":
            continue
        validator = {}
        validator.address = account.address
        validator.name = account.pos.delegate.username
        ...
        validator.lastGeneratedHeight = account.pos.delegate.lastForgedHeight
        validator.isBanned = True
        validator.pomHeights = account.pos.delegate.pomHeights
        validator.consecutiveMissedBlocks = account.pos.delegate.consecutiveMissedBlocks
...
# i.e., the accounts for which account.pos.delegate.username is not
...
def getSentVotes(account):
    sentVotes = account.pos.sentVotes
    for every vote in sentVotes:
...
def createVotersArray():
    voters = []
    for every account in accounts:
        if account.pos.sentVotes == [] and account.pos.unlocking == []:
            continue
        voter = {}
        voter.address = account.address
        voter.sentVotes = getSentVotes(account)
        voter.pendingUnlocks = account.pos.unlocking
        voters.append(voter)
    return voters
...
There will be a bootstrap period in which a fixed set of delegates will constitute the set of active delegates. This period will last for one week assuming no missed blocks. This is done by setting the [`initRounds`](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0057.md#genesis-assets-schema) property in the snapshot block to the corresponding value, which is given by `POS_INIT_ROUNDS`. The reason to have this period is to achieve that the PoS module selects only delegates with a valid BLS key outside of the bootstrap period. And this is realized by 1) banning every delegate in the snapshot block, and by 2) enabling the unbanning by a _[register keys](https://github.com/LiskHQ/lips/blob/main/proposals/lip-0050.md#register-keys)_ transaction. This way, delegates are forced to submit a _register keys_ transaction, and therefore to register a valid BLS key and a (new) generator key, in order to become an active delegate. Notice that with this approach, the PoS module does not need to check the validity of the BLS key of a delegate when selecting the active delegates. Therefore, it also does not need to store them.

The bootstrap period allows each delegate to submit a _register keys_ transaction in a sufficiently large time window. The expectation is that most delegates are unbanned after the bootstrap period. In the unexpected case that there are less than 101 non-banned delegates, the PoS module would simply select a smaller set of delegates that get the block slots of a round assigned round robin.
...
[research:dynamic-block-reward-module]: https://research.lisk.com/t/introduce-dynamic-block-rewards-module/387
