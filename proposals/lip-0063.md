...
def addLegacyModuleEntry():
    ...
    # mint all the tokens for legacy accounts here
    Token.mint(ADDRESS_LEGACY_RESERVE, TOKEN_ID_LSK_MAINCHAIN, legacyReserveAmount)
    Token.lock(ADDRESS_LEGACY_RESERVE, MODULE_ID_LEGACY, TOKEN_ID_LSK_MAINCHAIN, legacyReserveAmount)
```
