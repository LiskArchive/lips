...
However, events emitted during the [processing of a cross-chain message][lip-0053#ccu-processing] contain the ID of the cross-chain message as default topic rather than the ID of the cross-chain update transaction that contained the cross-chain message.
...
* `EVENT_TOPIC_FINALIZE_GENESIS_STATE`, for events emitted during the 'genesis state finalization' stage, 
* `EVENT_TOPIC_BEFORE_TRANSACTIONS`, for events emitted during the 'before transactions execution' stage, 
