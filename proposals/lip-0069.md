...
Application hooks are module methods which are called during the block execution. These hooks can access the currently executing block header and the block assets, and make state mutations.

#### Block Generation hooks

Block generation hooks are module methods which are called only during the block generation. These hooks cannot introduce state changes, but they have access to the currently generating block and they can add information to the block assets.
