...
Application hooks are module methods which are called during the block execution. These hooks can access the block header being currently executed and its block assets, and make state mutations.

#### Block Generation hooks

Block generation hooks are module methods which are called only during the block generation. These hooks cannot introduce state changes, but they have access to the block being currently generated and they can add information to the block assets.
...
In some modules, it requires methods from other modules, and for that, the dependency needs to be resolved before registering the module to the application.
