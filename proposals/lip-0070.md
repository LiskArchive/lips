...
def getLockedRewards(delegateAddress: Address, tokenID: TokenID) -> uint64:
    if tokenID == TOKEN_ID_POS:
        return Token.getLockedAmount(delegateAddress, MODULE_NAME_POS, tokenID) - getLockedVotedAmount(delegateAddress)
    return Token.getLockedAmount(delegateAddress, MODULE_NAME_POS, tokenID)
```
