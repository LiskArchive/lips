...
| `Stake` | object | Contains 3 elements (address, amount, sharingCoefficients) of types Address, uint64 and array respectively (same as the items in the sentStakes array of the [stakerStoreSchema][lip-0057.md#json-schema]). | Object containing information regarding a stake. |
...
This function calculates the current reward of a particular stake. The input is a `Stake` object, i.e., an object similar to the ones stored in the sentStakes array of the staker substore, containing `validatorAddress`, `amount` and `sharingCoefficients`.
