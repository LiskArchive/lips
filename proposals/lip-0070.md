...
def getClaimableRewards(stakerAddress: Address) -> dict[TokenID, uint64]:
    ...
    for i in range(len(stakerStore(stakerAddress).stakes)):
        ...
        if validatorAddress != stakerAddress:
            for elem in validatorStgore(validatorAddress).sharingCoefficients:
...
```python
def getExpectedStakingBlockReward(validatorAddress: Address, validatorReward: uint64, stake: uint64) -> uint64:
