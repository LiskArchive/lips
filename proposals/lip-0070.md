...
* `stakeAmount`: The amount the user is considering to stake with the specified validator.


```python
def getClaimableRewards(validatorAddress: Address, validatorReward: uint64, stakeAmount: uint64) -> uint64:

     if validatorStore(validatorAddress).totalStake == 0: # If sharing coefficient can not be defined, we return.
        return
    # Use Q96 numbers.
    ...
    totalStake = Q96(validatorStore(validatorAddress).totalStake)
