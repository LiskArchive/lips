...
​​The main purpose of the Dynamic Reward module is to compute the block rewards according to the validator weight for active validators and induces specific logic after the application of a block:
...
* Technically, each time an active validator generates a block they get a fraction of stake reward (depending on the current number of active validators), proportionally to their BFT weight. As a consequence, if active validators generate several blocks in a round then the total reward paid out in a round could slightly exceed the estimate of `defaultReward * roundLength`. This could happen if there are less than 101 active validators on the Lisk mainchain and validators with higher than average weight generate more blocks, however this should not be a persistent problem in practice since new active validators will appear.
