...
def getValidatorBlockReward(blockHeader: BlockHeader) -> uint64:
    ...
    minimumRewardActiveValidators = (FACTOR_MINIMUM_REWARD_ACTIVE_VALIDATORS * getDefaultRewardAtHeight(height))// DECIMAL_PERCENT_FACTOR
    ...
    numberActiveValidators = len([item for item in validatorParams.validators if item.bftWeight > 0])
    ...
    if validator.bftWeight > 0: #active validator
        stakeRewardActiveValidators = numberActiveValidators * (getDefaultRewardAtHeight(height) -  minimumRewardActiveValidators) 
...
def getExpectedValidatorRewards(validatorAddress: Address, height: uint32) -> tuple[uint64, uint64, uint64, uint64]:
    ...
    dailyReward =   86400 * blockReward // (numberActiveValidators * BLOCK_TIME)
    monthlyReward = 2592000 * blockReward // (numberActiveValidators * BLOCK_TIME) 
    yearlyReward = 31536000 * blockReward // (numberActiveValidators * BLOCK_TIME)
