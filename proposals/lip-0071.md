...
def beforeTransactionsExecute(b: Block) -> None:
    ...
    if ctx.blockReward > 0 and not Token.userSubstoreExists(b.header.generatorAddress, TOKEN_ID_DYNAMIC_BLOCK_REWARD):
        Token.initializeUserAccountForBlockGenerator(b.header.generatorAddress, TOKEN_ID_DYNAMIC_BLOCK_REWARD)
```
...
After the reward for the block generator is calculated, we initialize the block generator account for `TOKEN_ID_DYNAMIC_BLOCK_REWARD`, if it has not been previously initialized.  
