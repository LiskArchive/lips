...
The `processQueue` job will call the functions `validateReceivedTransactions` and `processVerifyTransactions`. Firstly, the `validateReceivedTransactions` function performs schema validation and signature validation on all transactions in the received queue. The transactions for which the validation check fails are removed from the transaction pool. Whereas, the transactions for which the validation check passes are moved to the `validated` queue. Secondly, the `processVerifiedTransactions` function performs verification of all transactions in the `validated` queue against other transactions in the transaction pool (this is explained later in a separate section) and against the blockchain state. The transactions for which the verification check fails are removed from the transaction pool. The transactions for which the verification check passes and that require further signatures for the multi-signature processing are moved to the `pending` queue. The transactions for which the verification check passes and that do not require further signatures are moved to the `verified` queue.
